...
/*lenovo-sw yexm1 20140318 : add "proc/lcm_info" to get lcm info */
#ifdef LENOVO_LCM_INFO_SHOW
...
static int mtkfb_set_lcm_feature_mode(lenovo_disp_feature_state_t *states)
{
	...
	/*Lenovo-sw yexm1 modify, 20140512 begin*/
	if (is_early_suspended){
	    LCM_Feature_set_resume = TRUE;
	    printk("%s set !!! but FB has been suspended\n",__func__);
	    goto End;
	}
	/*Lenovo-sw yexm1 modify, 20140512 end*/
...
////////////////////////////////////////////////
	...
	case MTKFB_SET_DISPLAY_FEATURE_STATE:
	{
		...
		if(state.cabc_mode>=0)
			disp_feature_state[displayid].cabc_mode= state.cabc_mode;
		if(state.inverse_mode>=0)
			disp_feature_state[displayid].inverse_mode= state.inverse_mode;
		if(state.gamma_mode>=0)
...
/*lenovo-sw yexm11 20140318 : add "proc/lcm_info" to get lcm info*/
#ifdef LENOVO_LCM_INFO_SHOW
...
static int mtkfb_probe(struct device *dev)
{
	...
	do{
		...
		if(lcm_drv==NULL){
			printk("[yexm1] lcm_drv is NULL\n");
			break;
		}
		lcm_info = create_proc_entry("lcm_info",0444,NULL);
		if(lcm_info==NULL){
			printk("[yexm1] Creat lcm_info failed!!!\n");
		}
		else{
			printk("[yexm1] Create lcm_info entry success!\n");
	...
	/*Lenovo-sw yexm1 modify, open the esd check machine 20140514 begin*/
    if(DISP_EsdRecoverCapbility())
	/*Lenovo-sw yexm1 modify, open the esd check machine 20140514 end*/
    {
...
/* Lenovo-sw yexm1 add 20140522 begin */
	...
	if (lcm_drv->setdimming_off)
	{
		lcm_drv->setdimming_off();
	}
	#endif
/* Lenovo-sw yexm1 add 20140522 end */
	...
	/*Lenovo-sw yexm1 modify 20140423 begin*/
        msleep(20*100000/lcd_fps); // Delay 20 frames.
        /*Lenovo-sw yexm1 modify 20140423 end*/
...
static void mtkfb_late_resume(struct early_suspend *h)
{
	...
	/*Lenovo-sw yexm1 modify, 20140513 begin*/
	#ifdef LENOVO_LCM_EFFECT
	...
	#endif
	/*Lenovo-sw yexm1 modify, 20140513 ennd*/
