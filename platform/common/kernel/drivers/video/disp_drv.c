...
static void _DISP_WaitMemWriteDone (void) {
    ...
    //Must before calling @DISP_SwapBuffer()
    _DISP_DumpOverlay();
...
DISP_STATUS DISP_Init(UINT32 fbVA, UINT32 fbPA, BOOL isLcmInited)
{
   ...
   DISP_InitVSYNC((100000000/lcd_fps) + 1);//us
	...
	if(lcm_drv)
	{
		...
		if(lcm_drv->set_gammamode)
			disp_feature_info[MTKFB_DISPIF_PRIMARY_LCD].gamma_support = 1;
		if(lcm_drv->set_iemode)
			disp_feature_info[MTKFB_DISPIF_PRIMARY_LCD].ie_support = 1;
	}
	else
	{
		printk("[yexm1] DISP Info: Fatal Error!!, lcm_drv is null\n");
	}
	...
	DISP_SetGammaMode(disp_feature_state[MTKFB_DISPIF_PRIMARY_LCD].gamma_mode);
	DISP_SetIeMode(disp_feature_state[MTKFB_DISPIF_PRIMARY_LCD].ie_mode);
	#endif
#if 0
	{
		unsigned int i = 0;
...
static void _DISP_DumpOverlay(void)
{
    ...
    if (DISP_IsDecoupleMode())
    {
        paddr = DISP_GetBufferAddress(false);   //mem_write buffer
    } else {
        if (init == 0)
        {
...
static int _DISP_MergeOVLDirty(disp_path_config_dirty *dirty_flag) {
	...
	if (try_cnt < 5000) {
		...
		if (dirty_flag->ovl_dirty) {
			...
			// Assert layer is disabled by someone when reboot
...
DISP_STATUS DISP_SetCabcMode(unsigned int mode)
{
	...
	disp_drv_init_context();
	//LCD_WaitForNotBusy();//for mt6589
	disphal_wait_not_busy();//for mt6582/92
	if (!lcm_drv->set_cabcmode) {
		ret = DISP_STATUS_NOT_IMPLEMENTED;
		goto End;
	}
...
DISP_STATUS DISP_SetGammaMode(unsigned int mode)
{
	DISP_STATUS ret = DISP_STATUS_OK;
	...
	lcm_drv->set_gammamode(mode);
End:
	return ret;
}
DISP_STATUS DISP_SetIeMode(unsigned int mode)
{
	DISP_STATUS ret = DISP_STATUS_OK;
