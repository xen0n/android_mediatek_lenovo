...
#if defined (CONFIG_EARLY_LINUX_PORTING) || defined (CONFIG_MTK_FPGA) //for FPGA early proting users
S32 pwrap_init_kernel ( void )
{
  u32 pwrap_ret=0,i=0;
  PWRAPFUC();
  for(i=0;i<3;i++)
  {
    ...
    if(pwrap_ret!=0)
    {
      PWRAPREG("[PMIC_WRAP]wrap_init fail,the return value=%x.\n",pwrap_ret);
    }
    else
    {
      PWRAPREG("[PMIC_WRAP]wrap_init pass,the return value=%x.\n",pwrap_ret);
      break;//init pass
    }
  }
 return 0;
...
static int __init mt_pwrap_init(void)
{
   ...
   #if defined (CONFIG_EARLY_LINUX_PORTING) || defined (CONFIG_MTK_FPGA)
	pwrap_init_kernel();  //add for FPGA early porting PMIC_WRAP user in kernel only(In Bring up default do wrapper init)
   #endif
	return ret;
}
