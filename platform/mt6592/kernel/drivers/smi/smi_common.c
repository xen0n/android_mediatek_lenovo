...
#define SMI_ADJ_POLICY_DDR1066 0
#define SMI_ADJ_POLICY_DDR1333 1

// SMI_ADJ_POLICY_DDR1333
...
// SMI_ADJ_POLICY_DDR1066
#define SF_HWC_PIXEL_MAX_NORMAL_DDR1066  (1280 * 720 * 3)
#define SF_HWC_PIXEL_MAX_VR_DDR1066  (1280 * 720 * 4)
#define SF_HWC_PIXEL_MAX_VP_DDR1066  (1280 * 720 * 3)
#define SF_HWC_PIXEL_MAX_ALWAYS_GPU_DDR1066 (1280 * 720)

#define SF_HWC_PIXEL_BYTE_MAX_NORMAL_DDR1066  (SF_HWC_PIXEL_MAX_NORMAL_DDR1066 * 4 )
#define SF_HWC_PIXEL_BYTE_MAX_VR_DDR1066  (SF_HWC_PIXEL_MAX_VR_DDR1066 * 4 )
#define SF_HWC_PIXEL_BYTE_MAX_VP_DDR1066  (SF_HWC_PIXEL_MAX_VP_DDR1066 * 4 )

#define DISP_PIXEL_MAX_720P_BENCHMARK (1280 * 720 * 1 + 120 * 720)
#define DISP_PIXEL_MAX_720P_LAUNCHER  (1280 * 720 * 2)
#define DISP_PIXEL_MAX_720P_KEYGUARD  (1280 * 720 * 3)
#define DISP_PIXEL_MAX_FHD_BENCHMARK  (1920 * 1080 * 1 + 120 * 1080)
#define DISP_PIXEL_MAX_FHD_LAUNCHER   (1920 * 1080 * 2 + 100 * 1080)
#define DISP_PIXEL_MAX_FHD_CAMERA_PREVIEW  (1920 * 1080 * 2 + 150 * 1080)


#define DISP_PIXEL_BYTE_MAX_720P_BENCHMARK ((DISP_PIXEL_MAX_720P_BENCHMARK) *4)
#define DISP_PIXEL_BYTE_MAX_720P_LAUNCHER  ((DISP_PIXEL_MAX_720P_LAUNCHER) *4)
#define DISP_PIXEL_BYTE_MAX_720P_KEYGUARD  ((DISP_PIXEL_MAX_720P_KEYGUARD) *4)
#define DISP_PIXEL_BYTE_MAX_FHD_BENCHMARK  ((DISP_PIXEL_MAX_FHD_BENCHMARK) *4)
#define DISP_PIXEL_BYTE_MAX_FHD_LAUNCHER   ((DISP_PIXEL_MAX_FHD_LAUNCHER) *4)
#define DISP_PIXEL_BYTE_MAX_FHD_CAMERA_PREVIEW  ((DISP_PIXEL_MAX_FHD_CAMERA_PREVIEW) *4)


#define GPU_SMI_L1_LIMIT_0 (0)
#define GPU_SMI_L1_LIMIT_1 ((0x1 <<23) + (0x2<<18)+ (0x4 <<13)+ (0x1 << 12) + (0x0 << 11)+ 0x0)
...
// Special Setting for FHD fine tune
...
static unsigned int smi_adj_policy = SMI_ADJ_POLICY_DDR1066;
...
static void vrSetting(void){
    ...
    M4U_WriteReg32(REG_SMI_L1ARB3 , 0 , GPU_SMI_L1_LIMIT_2);
    M4U_WriteReg32(REG_SMI_L1ARB4 , 0 , GPU_SMI_L1_LIMIT_2);
...
static void initSetting(void)
{
    ...
    // GPU OSTD limittion: Low 
    //M4U_WriteReg32(REG_SMI_L1ARB3 , 0 , (0x1 <<23) + (0x1<<18)+ (0x2 <<13)+ (0x1 << 12) + (0x0 << 11)+ 0x0 );
    //M4U_WriteReg32(REG_SMI_L1ARB4 , 0 , (0x1 <<23) + (0x1<<18)+ (0x2 <<13)+ (0x1 << 12) + (0x0 << 11)+ 0x0 );
        
    // GPU OSTD limitation - Medium
    // M4U_WriteReg32(REG_SMI_L1ARB3 , 0 , ((0x1 <<23) + (0x2<<18)+ (0x3 <<13)+ (0x1 << 12) + (0x0 << 11)+ 0x0));
    // M4U_WriteReg32(REG_SMI_L1ARB4 , 0 , ((0x1 <<23) + (0x2<<18)+ (0x3 <<13)+ (0x1 << 12) + (0x0 << 11)+ 0x0));
    
    // GPU OSTD limitation - High
    // M4U_WriteReg32(REG_SMI_L1ARB3 , 0 , ((0x1 <<23) + (0x2<<18)+ (0x4 <<13)+ (0x1 << 12) + (0x0 << 11)+ 0x0));
    // M4U_WriteReg32(REG_SMI_L1ARB4 , 0 , ((0x1 <<23) + (0x2<<18)+ (0x4 <<13)+ (0x1 << 12) + (0x0 << 11)+ 0x0));
...
static void smi_dynamic_adj_ddr1333(unsigned int dsi2smi_total_pixel){
    if(smi_profile == SMI_BWC_SCEN_VR || smi_profile == SMI_BWC_SCEN_VR_SLOW){
        if(dsi2smi_total_pixel >= SF_HWC_PIXEL_BYTE_MAX_VR){
            M4U_WriteReg32(REG_SMI_L1ARB3 , 0 , GPU_SMI_L1_LIMIT_VR_GP_OPT);
            M4U_WriteReg32(REG_SMI_L1ARB4 , 0 , GPU_SMI_L1_LIMIT_VR_PP_OPT); 
        }else{
            if(dsi2smi_total_pixel < DISP_PIXEL_BYTE_MAX_FHD_CAMERA_PREVIEW){
                M4U_WriteReg32(REG_SMI_L1ARB3 , 0 , GPU_SMI_L1_LIMIT_VR_GP_OPT);
                M4U_WriteReg32(REG_SMI_L1ARB4 , 0 , GPU_SMI_L1_LIMIT_VR_PP_OPT);  
            }else{
                M4U_WriteReg32(REG_SMI_L1ARB3 , 0 , GPU_SMI_L1_LIMIT_VR_GP_OPT);
    ...
    }else if(smi_profile == SMI_BWC_SCEN_VP){
        if(wifi_disp_transaction == 1){
            M4U_WriteReg32(REG_SMI_L1ARB3 , 0 , GPU_SMI_L1_LIMIT_WFDVP_GP_OPT );
            M4U_WriteReg32(REG_SMI_L1ARB4 , 0 , GPU_SMI_L1_LIMIT_WFDVP_PP_OPT );              
        }else{
            if(dsi2smi_total_pixel >= SF_HWC_PIXEL_BYTE_MAX_VP){
                M4U_WriteReg32(REG_SMI_L1ARB3 , 0 , GPU_SMI_L1_LIMIT_VP_GP_OPT);
                M4U_WriteReg32(REG_SMI_L1ARB4 , 0 , GPU_SMI_L1_LIMIT_VP_PP_OPT);       
            }else{
                if(dsi2smi_total_pixel < DISP_PIXEL_BYTE_MAX_FHD_BENCHMARK){
                    M4U_WriteReg32(REG_SMI_L1ARB3 , 0 , GPU_SMI_L1_LIMIT_VP_GP_OPT);
                    M4U_WriteReg32(REG_SMI_L1ARB4 , 0 , GPU_SMI_L1_LIMIT_VP_PP_OPT); 
                }else if(dsi2smi_total_pixel < DISP_PIXEL_BYTE_MAX_FHD_CAMERA_PREVIEW){
                    M4U_WriteReg32(REG_SMI_L1ARB3 , 0 , GPU_SMI_L1_LIMIT_VP_GP_OPT);
                    M4U_WriteReg32(REG_SMI_L1ARB4 , 0 , GPU_SMI_L1_LIMIT_VP_PP_OPT); 
			          }else{
                    M4U_WriteReg32(REG_SMI_L1ARB3 , 0 , GPU_SMI_L1_LIMIT_VP_GP_OPT);
        ...
        if(dsi2smi_total_pixel >= SF_HWC_PIXEL_BYTE_MAX_NORMAL){  
            M4U_WriteReg32(REG_SMI_L1ARB3 , 0 , GPU_SMI_L1_LIMIT_2);
            M4U_WriteReg32(REG_SMI_L1ARB4 , 0 , GPU_SMI_L1_LIMIT_2);
        }else{
            if(dsi2smi_total_pixel <= (1920 * 1080 * 4 * 1)){
                // 1 FHD layer pixels
                M4U_WriteReg32(REG_SMI_L1ARB3 , 0 , GPU_SMI_L1_LIMIT_OPT_1);
                M4U_WriteReg32(REG_SMI_L1ARB4 , 0 , GPU_SMI_L1_LIMIT_OPT_1); 
            }else if(dsi2smi_total_pixel <= (1920 * 1080 * 4 * 2)){
                // 2 FHD layer pixels
                M4U_WriteReg32(REG_SMI_L1ARB3 , 0 , GPU_SMI_L1_LIMIT_1);
                M4U_WriteReg32(REG_SMI_L1ARB4 , 0 , GPU_SMI_L1_LIMIT_1); 
            }else{
                // over 2 FHD layer pixels
                M4U_WriteReg32(REG_SMI_L1ARB3 , 0 , GPU_SMI_L1_LIMIT_2);
                M4U_WriteReg32(REG_SMI_L1ARB4 , 0 , GPU_SMI_L1_LIMIT_2); 
            }
        }  
        ...
        if(dsi2smi_total_pixel >= SF_HWC_PIXEL_BYTE_MAX_NORMAL){  
            ...
            if(dsi2smi_total_pixel <= (1920 * 1080 * 4 * 1)){
                // 1 FHD layer pixels
                M4U_WriteReg32(REG_SMI_L1ARB3 , 0 , GPU_SMI_L1_LIMIT_OPT_0);
                M4U_WriteReg32(REG_SMI_L1ARB4 , 0 , GPU_SMI_L1_LIMIT_OPT_0); 
            }else if(dsi2smi_total_pixel <= (1920 * 1080 * 4 * 2)){
                // 2 FHD layer pixels
                M4U_WriteReg32(REG_SMI_L1ARB3 , 0 , GPU_SMI_L1_LIMIT_OPT_2);
                M4U_WriteReg32(REG_SMI_L1ARB4 , 0 , GPU_SMI_L1_LIMIT_OPT_2); 
            }else{
                // over 2 FHD layer pixels
                M4U_WriteReg32(REG_SMI_L1ARB3 , 0 , GPU_SMI_L1_LIMIT_1);
                M4U_WriteReg32(REG_SMI_L1ARB4 , 0 , GPU_SMI_L1_LIMIT_1); 
...
static void smi_dynamic_adj_ddr1066(unsigned int dsi2smi_total_pixel){
    if(smi_profile == SMI_BWC_SCEN_VR || smi_profile == SMI_BWC_SCEN_VR_SLOW){
        if(dsi2smi_total_pixel >= SF_HWC_PIXEL_BYTE_MAX_VR){
            M4U_WriteReg32(REG_SMI_L1ARB3 , 0 , GPU_SMI_L1_LIMIT_2);
            M4U_WriteReg32(REG_SMI_L1ARB4 , 0 , GPU_SMI_L1_LIMIT_2); 
        ...
        if(dsi2smi_total_pixel >= SF_HWC_PIXEL_BYTE_MAX_VP){
            ...
            M4U_WriteReg32(REG_SMI_L1ARB4 , 0 , GPU_SMI_L1_LIMIT_2);         
                ...
                M4U_WriteReg32(REG_SMI_L1ARB3 , 0 , GPU_SMI_L1_LIMIT_3);
                M4U_WriteReg32(REG_SMI_L1ARB4 , 0 , GPU_SMI_L1_LIMIT_3);
            }
			  }    
    }else if (smi_profile == SMI_BWC_SCEN_VENC){
         if(dsi2smi_total_pixel >= SF_HWC_PIXEL_BYTE_MAX_NORMAL){
        ...
        if(dsi2smi_total_pixel >= SF_HWC_PIXEL_BYTE_MAX_NORMAL){  
            ...
            if(dsi2smi_total_pixel < DISP_PIXEL_BYTE_MAX_720P_BENCHMARK){
                M4U_WriteReg32(REG_SMI_L1ARB3 , 0 , GPU_SMI_L1_LIMIT_0);
                M4U_WriteReg32(REG_SMI_L1ARB4 , 0 , GPU_SMI_L1_LIMIT_0); 
            }else if(dsi2smi_total_pixel < DISP_PIXEL_BYTE_MAX_720P_LAUNCHER){
                M4U_WriteReg32(REG_SMI_L1ARB3 , 0 , GPU_SMI_L1_LIMIT_1);
                M4U_WriteReg32(REG_SMI_L1ARB4 , 0 , GPU_SMI_L1_LIMIT_1); 
            }else if(dsi2smi_total_pixel < DISP_PIXEL_BYTE_MAX_720P_KEYGUARD){
                M4U_WriteReg32(REG_SMI_L1ARB3 , 0 , GPU_SMI_L1_LIMIT_1);
                M4U_WriteReg32(REG_SMI_L1ARB4 , 0 , GPU_SMI_L1_LIMIT_1); 
            }else if(dsi2smi_total_pixel < DISP_PIXEL_BYTE_MAX_FHD_BENCHMARK){
                M4U_WriteReg32(REG_SMI_L1ARB3 , 0 , GPU_SMI_L1_LIMIT_1);
                M4U_WriteReg32(REG_SMI_L1ARB4 , 0 , GPU_SMI_L1_LIMIT_1); 
            }else{
                M4U_WriteReg32(REG_SMI_L1ARB3 , 0 , GPU_SMI_L1_LIMIT_2);
...
{
    // -------------------- Runtime debug suppport -------------------- 
    if(smi_tuning_mode >= 1){
        if(smi_tuning_mode == 2){
            M4U_WriteReg32(REG_SMI_L1ARB3 , 0 , GPU_SMI_L1_LIMIT_0);
            M4U_WriteReg32(REG_SMI_L1ARB4 , 0 , GPU_SMI_L1_LIMIT_0);
        }
        if(smi_debug_level >= 1){
            smi_pixel_count = (dsi2smi_total_pixel>>2);
            smi_common_larb_regs[3] = M4U_ReadReg32(REG_SMI_L1ARB3 , 0);
            smi_common_larb_regs[4] = M4U_ReadReg32(REG_SMI_L1ARB4 , 0);
        }
        return;
    ...
    if(smi_adj_policy == SMI_ADJ_POLICY_DDR1333){
        smi_dynamic_adj_ddr1333(dsi2smi_total_pixel);
    }else{
        smi_dynamic_adj_ddr1066(dsi2smi_total_pixel);
    }
...
static void set_sf_pixel_limitation_ddr1333(){
    if(mhl_disp_transaction == 1){
        if(smi_profile == SMI_BWC_SCEN_VP){
            hwc_max_pixel_count = SF_HWC_PIXEL_MAX_VP;
        }else if(smi_profile == SMI_BWC_SCEN_VR || smi_profile == SMI_BWC_SCEN_VR_SLOW){
            hwc_max_pixel_count = SF_HWC_PIXEL_MAX_VR;
        }else{
            hwc_max_pixel_count = SF_HWC_PIXEL_MAX_ALWAYS_GPU;
        }   
    }else if(wifi_disp_transaction == 1){
        if(smi_profile == SMI_BWC_SCEN_VP){
            hwc_max_pixel_count = SF_HWC_PIXEL_MAX_ALWAYS_GPU;
        }else if(smi_profile == SMI_BWC_SCEN_VR || smi_profile == SMI_BWC_SCEN_VR_SLOW){
            hwc_max_pixel_count = SF_HWC_PIXEL_MAX_ALWAYS_GPU;
...
// Set hwc_max_pixel_count according to the current profile
static void set_sf_pixel_limitation_ddr1066(){
    if(mhl_disp_transaction == 1){
        if(smi_profile == SMI_BWC_SCEN_VP){
            hwc_max_pixel_count = SF_HWC_PIXEL_MAX_VP_DDR1066;
        }else if(smi_profile == SMI_BWC_SCEN_VR || smi_profile == SMI_BWC_SCEN_VR_SLOW){
            hwc_max_pixel_count = SF_HWC_PIXEL_MAX_VR_DDR1066;
        }else{
            hwc_max_pixel_count = SF_HWC_PIXEL_MAX_NORMAL_DDR1066;
        }   
    }else if(wifi_disp_transaction == 1){
        if(smi_profile == SMI_BWC_SCEN_VP){
            hwc_max_pixel_count = SF_HWC_PIXEL_MAX_VP_DDR1066;
...
static void set_sf_pixel_limitation(){
    ...
    if(smi_adj_policy == SMI_ADJ_POLICY_DDR1333){
        set_sf_pixel_limitation_ddr1333();
    }else{
        set_sf_pixel_limitation_ddr1066();
    }
}
...
static void detect_dram_type(void)
{

  unsigned int clk_read = mt_get_emi_freq();
	if( clk_read >= 300000) {
	    smi_adj_policy = SMI_ADJ_POLICY_DDR1333;
	}else{
	    smi_adj_policy = SMI_ADJ_POLICY_DDR1066;
	}
	SMIMSG("clk_read = %d\n", clk_read);
}
