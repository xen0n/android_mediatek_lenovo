...
GPIO_TDM_REQ_INFO mtk_wcn_tdm_req_info_tbls[] = {
	{0,  47,  0x81060010},
	{1,  48,  0x81060010},	
	{2,  49,  0x81060010},
	{3,  14,  0x81060010},
...
wmt_plat_dump_pin_conf (VOID)
{
    WMT_PLAT_INFO_FUNC( "[WMT-PLAT]=>dump wmt pin configuration start<=\n");
        ...
        WMT_PLAT_INFO_FUNC( "FM digital mode is not set, no need for I2S GPIOs\n");
                ...
                WMT_PLAT_INFO_FUNC( "GPS_LNA(GPIO%d)\n", (GPIO_GPS_LNA_PIN & 0x7fffffff));
    #else
                WMT_PLAT_INFO_FUNC( "GPS_LNA(not defined)\n");
    #endif

	#ifdef GPIO_TDM_REQ
	            WMT_PLAT_INFO_FUNC( "GPIO_TDM_REQ(GPIO%d)\n", (GPIO_TDM_REQ & 0x7fffffff));
	#else
	            WMT_PLAT_INFO_FUNC( "GPIO_TDM_REQ(not defined)\n");
	#endif
...
INT32 wmt_plat_i2s_ctrl(ENUM_PIN_STATE state)
{
    ...
    // TODO: provide a translation table in board_custom.h for different ALPS project customization.
		        ...
		        case PIN_STA_DEINIT:
		            ...
		            break;
		        default:
		            WMT_PLAT_DBG_FUNC("WMT-PLAT:Warnning, invalid state(%d) on I2S Group\n", state);
		            break;
		        }
		    #else
		        WMT_PLAT_ERR_FUNC ("[MT662x]Error:FM digital mode set, but no I2S GPIOs defined\n");
		    #endif
		#else
		        WMT_PLAT_INFO_FUNC( "[MT662x]warnning:FM digital mode is not set, no I2S GPIO settings should be modified by combo driver\n");
		#endif
    return 0;
}
...
INT32 wmt_plat_soc_paldo_ctrl(ENUM_PALDO_TYPE ePt,ENUM_PALDO_OP ePo)
{
	...
	case GPS_PALDO:
		...
		break;
	case PMIC_CHIPID_PALDO:
		iRet = mtk_wcn_consys_hw_pmic_id_ctrl();
		break;
	case WIFI_5G_PALDO:
		iRet = mtk_wcn_consys_hw_paldo_wirte_ctrl(ePo);
		break;
    default:
