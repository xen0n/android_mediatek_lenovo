...
static unsigned int brightness_mapping_to_255(unsigned int level)
{
	unsigned int mapped_level;

	//printk("yexm1--brightness_mapping_to_255: level is %d\n", level);
	mapped_level = level /( 1023 / 255);

	//printk("yexm1--brightness_mapping_to_255: mapped_level is %d\n", mapped_level);
	...
	//spin_lock_irqsave(&leds_lock, flags);
		...
		if(level >> LED_RESERVEBIT_SHIFT)
		{
			...
			if(MT65XX_LED_MODE_CUST_BLS_PWM != led_data->cust.mode)
			{
				...
				/* Lenovo-sw yexm1 add 20140402 begin */
			#ifndef LENOVO_LCD_BACKLIGHT_CONTROL_BY_LCM
			...
			/* Lenovo-sw yexm1 add 20140402 begin */
			#ifdef LENOVO_LCD_BACKLIGHT_CONTROL_BY_LCM
			level = brightness_mapping_to_255(level);
			#endif
			/* Lenovo-sw yexm1 add 20140402 end */
