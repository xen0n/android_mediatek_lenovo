...
static unsigned int simp_msdc_init(struct simp_mmc_host *mmc_host)
{
    ...
    switch (host->id) {
		case 0:
			sdr_set_field(MSDC0_GPIO_CLK_BASE, GPIO_MSDC_DRVN, msdc0_hw.clk_drv); /* feifei.wang -- not use hardcode here */
			sdr_set_field(MSDC0_GPIO_CMD_BASE, GPIO_MSDC_DRVN, msdc0_hw.cmd_drv);
			sdr_set_field(MSDC0_GPIO_DAT_BASE, GPIO_MSDC_DRVN, msdc0_hw.dat_drv);
			printk(KERN_EMERG "host0 clk= %d, cmd= %d, dat= %d.\r\n", msdc0_hw.clk_drv, msdc0_hw.cmd_drv, msdc0_hw.dat_drv);
			break;
        case 1:
			#if defined(CFG_DEV_MSDC1)
            sdr_set_field(MSDC1_GPIO_CLK_BASE, GPIO_MSDC_DRVN, msdc1_hw.clk_drv); /* feifei.wang -- not use hardcode here */
            sdr_set_field(MSDC1_GPIO_CMD_BASE, GPIO_MSDC_DRVN, msdc1_hw.cmd_drv);
            sdr_set_field(MSDC1_GPIO_DAT_BASE, GPIO_MSDC_DRVN, msdc1_hw.dat_drv);
			printk(KERN_EMERG "host1 clk= %d, cmd= %d, dat= %d.\r\n", msdc1_hw.clk_drv, msdc1_hw.cmd_drv, msdc1_hw.dat_drv);
			#endif
			break;
		default:
			break;
