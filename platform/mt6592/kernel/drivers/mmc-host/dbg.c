...
static int msdc_help_proc_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    p += sprintf(p, "          [mode]               mode 0:stop, 1:read, 2:write \n");
...
#if (5 == HOST_MAX_NUM) || (4 == HOST_MAX_NUM) || (3 == HOST_MAX_NUM)
    p += sprintf(p, "-> MSDC[2] Zone: 0x%.8x\n", sd_debug_zone[2]);
#endif 
#if (5 == HOST_MAX_NUM) || (4 == HOST_MAX_NUM)
...
static int rwThread(void *data)
{
	...
	while (read_write_state != 0) {
		...
		if (id==0) {
			error = sd_multi_rw_compare_slave(0, p, COMPARE_ADDRESS_MMC);
			if(error) {
				printk("[%s]: failed data id0, error=%d\n", __func__, error);
				break;
			}
		} else if(id==1) {
			error = sd_multi_rw_compare_slave(1, p, COMPARE_ADDRESS_SD);
			if(error) {
		...
		i++;
		if(i==100){
			printk("[***rwThread %s***]", read_write_state==1 ? "read":"write");
			i=0;
		}
	}
...
static int msdc_debug_proc_write(struct file *file, const char *buf, unsigned long count, void *data)
{
	...
	if(cmd == SD_TOOL_ZONE) {
		...
		if(id >= HOST_MAX_NUM || id < 0)
		    ...
		    offset = (unsigned int)p3;
                    if (offset < 0 || offset > 0x104){
                        printk("[****SD_Debug****]register offset error when modify msdc reg\n");
                        return count;
                    }    
#if defined(CFG_DEV_MSDC0)
			if(id == 0)
		...
		if (mode == 0) {
			...
			rw_thread = kthread_create(rwThread, id|mode<<4, "msdc_rw_thread");
