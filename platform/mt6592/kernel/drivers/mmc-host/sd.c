...
/*
bit31~24: RDSEL (use 6bits)
bit23~20: TDSEL
bit19/18/17: IES/SMT/SR
bit16
bit10~8: PUPD/R1/R0
BIT2~0: E8/E4/E2
*/
...
//    struct mmc_command *cmd = host->cmd;
    ...
    if(cmd == NULL) {
        xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "[%s] cmd == NULL\n", __func__);
        return -1;
    }
...
extern int DFS_Detection(void); //for MT6592 only 
static unsigned int msdc_online_tuning(struct msdc_host *host, unsigned fn, unsigned addr)
{
    u32 base = host->base;
    u32 rxdly0;
                                  ...
                                  struct mmc_command *sbc)
{
        ...
        ((cmd->error == -EIO) || (data->error == -EIO)))
    {
        ...
        ERR_MSG("XXX CPU_Temperature<%d>", cur_temperature);
    }
#endif
