...
#define mt6333_SLAVE_ADDR_Read    0xD7
    ...
    #define mt6333_BUSNUM 0
#else
    #ifdef MT6333_I2C_CHANNEL_CHANGED_2
        #define mt6333_BUSNUM 2
    #else
...
kal_uint32 thermal_mt6333_buck_int=0;
...
/**********************************************************
  ...
  *
  *********************************************************/
DEFINE_MUTEX(mt6333_i2c_access);
...
//only for DVFS
int mt6333_read_byte_polling(kal_uint8 cmd, kal_uint8 *returnData)
{
#if 1
    char     cmd_buf[1]={0x00};
    char     readData = 0;
    ...
    //mutex_lock(&mt6333_i2c_access);//only for DVFS
    
    new_client->ext_flag=((new_client->ext_flag ) & I2C_MASK_FLAG ) | I2C_WR_FLAG | I2C_DIRECTION_FLAG| I2C_POLLING_FLAG;
    ...
    {
        new_client->ext_flag=0;
        //mutex_unlock(&mt6333_i2c_access);//only for DVFS
        return 0;
    }
...
//only for DVFS
int mt6333_write_byte_polling(kal_uint8 cmd, kal_uint8 writeData)
{
#if 1
    char    write_data[2] = {0};
    int     ret=0;
    ...
    new_client->ext_flag=((new_client->ext_flag ) & I2C_MASK_FLAG ) | I2C_DIRECTION_FLAG | I2C_POLLING_FLAG;
...
//only for DVFS
kal_uint32 mt6333_read_interface_polling (kal_uint8 RegNum, kal_uint8 *val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    kal_uint8 mt6333_reg = 0;
    int ret = 0;
    ...
    ret = mt6333_read_byte_polling(RegNum, &mt6333_reg);
    //printk("[mt6333_read_interface] Reg[%x]=0x%x\n", RegNum, mt6333_reg);
...
//only for DVFS
kal_uint32 mt6333_config_interface_polling (kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    kal_uint8 mt6333_reg = 0;
    int ret = 0;
    ...
    ret = mt6333_write_byte_polling(RegNum, mt6333_reg);
    //printk("[mt6333_config_interface] write Reg[%x]=0x%x\n", RegNum, mt6333_reg);
...
#define CUST_EINT_MT6333_PMIC_TYPE             8 //CUST_EINTF_TRIGGER_LOW
#define CUST_EINT_MT6333_PMIC_DEBOUNCE_EN      1 //CUST_EINT_DEBOUNCE_ENABLE
//----------------------------------------------------------------------
#else
#include <cust_eint.h>
#endif
...
struct wake_lock mt6333_thread_lock;
...
void cust_mt6333_interrupt_en_setting(void)
{
    ...
    mt6333_set_rg_int_en_buck_thermal(1);
...
void set_thermal_mt6333_buck_int_status(kal_uint32 val)
{
	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "set_thermal_mt6333_buck_int_status\n");
	thermal_mt6333_buck_int = val;
}

kal_uint32 get_thermal_mt6333_buck_int_status(void)
{
	//xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "get_thermal_mt6333_buck_int_status\n");
	return thermal_mt6333_buck_int ;
}
...
void buck_thermal_int_handler(void)
{
    ...
    set_thermal_mt6333_buck_int_status(1);
...
static int mt6333_thread_kthread(void *x)
{
    ...
    while (1) {
        ...
        xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[6333 INT] int_status_val_0=0x%x\n", int_status_val_0);
        ...
        xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[6333 INT] int_status_val_1=0x%x\n", int_status_val_1);

        if( (((int_status_val_1)&(0x0001))>>0) == 1 )  { chrwdt_flag_int_handler();       }
        if( (((int_status_val_1)&(0x0002))>>1) == 1 )  { buck_vcore_oc_int_handler();     }
        if( (((int_status_val_1)&(0x0004))>>2) == 1 )  { buck_vmem_oc_int_handler();      }
        if( (((int_status_val_1)&(0x0008))>>3) == 1 )  { buck_vrf18_oc_int_handler();     }
        if( (((int_status_val_1)&(0x0010))>>4) == 1 )  { buck_thermal_int_handler();      }
        if( (((int_status_val_1)&(0x0020))>>5) == 1 )  { flash_en_timeout_int_handler();  }
        if( (((int_status_val_1)&(0x0040))>>6) == 1 )  { flash_vled_short_int_handler();  }
        if( (((int_status_val_1)&(0x0080))>>7) == 1 )  { flash_vled_open_int_handler();   }
        //--------------------------------------------------------------------------------
        int_status_val_2 = mt6333_get_reg_value(MT6333_INT_STATUS2);
        xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[6333 INT] int_status_val_2=0x%x\n", int_status_val_2);

        if( (((int_status_val_2)&(0x0001))>>0) == 1 )  { chr_plug_in_flash_int_handler(); }
        if( (((int_status_val_2)&(0x0002))>>1) == 1 )  { chr_oc_int_handler();            }
        //--------------------------------------------------------------------------------
        ...
        wake_unlock(&mt6333_thread_lock);
...
void MT6333_PMIC_INIT_SETTING_V1_hpm(void)
{
    int ret=0;

    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[MT6333_PMIC_INIT_SETTING_V1_hpm] PMIC Chip = %x\n",g_mt6333_cid);
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[MT6333_PMIC_INIT_SETTING_V1_hpm] 20130902\n");

    ret = mt6333_config_interface(0x67,0x0,0x3,6); // [7:6]: QI_VCORE_VSLEEP; VCORE sleep mode voltage=0.7V for 6592

    //from JT for MT6333 init setting 20131217
	ret = mt6333_config_interface(0x69,0x0,0x1,0); // VCORE_SFCG_REN
	ret = mt6333_config_interface(0x69,0x2,0x7F,1); // VCORE_SFCHG_RRATE
	ret = mt6333_config_interface(0x69,0x1,0x1,0); //  VCORE_SFCHG_REN

    ret = mt6333_config_interface(0x6D,0x0,0x7F,0); // [6:0]: VCORE_VOSEL_SLEEP; VCORE sleep voltage =0.7V for 6592
    ret = mt6333_config_interface(0x75,0x20,0x7F,0); // [6:0]: VCORE_VOSEL_ON_SPM; VCORE_SPM=0.9V for 6592
    ret = mt6333_config_interface(0x9F,0x0,0x1,7); // [7:7]: VRF18_EN_CTRL; SW control only
    ret = mt6333_config_interface(0xA0,0x1,0x1,0); // [0:0]: RG_VRF18_MODESET_SPM; Only FPWM mode
    ret = mt6333_config_interface(0xA0,0x1,0x1,1); // [1:1]: VRF18_EN_SPM; SPM control

    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "after MT6333 init : Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x\n",
    ...
    // DCT
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[MT6333_PMIC_INIT_SETTING_V1_hpm] pmu_drv_tool_customization_init\n");
...
static int mt6333_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    MT6333_PMIC_INIT_SETTING_V1_hpm();
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[MT6333_PMIC_INIT_SETTING_V1_hpm] Done\n");
...
static int __init mt6333_init(void)
{
    ...
    wake_lock_init(&mt6333_thread_lock, WAKE_LOCK_SUSPEND, "mt6333_thread wakelock");
