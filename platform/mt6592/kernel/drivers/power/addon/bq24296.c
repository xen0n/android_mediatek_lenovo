...
kal_uint32 bq24296_read_interface (kal_uint8 RegNum, kal_uint8 *val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    *val = (bq24296_reg >> SHIFT);    
    printk("[bq24296_read_interface] Val=0x%x\n", *val);
...
kal_uint32 bq24296_config_interface (kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    ret = bq24296_read_byte(RegNum, &bq24296_reg);
    //printk("[bq24296_config_interface] Reg[%x]=0x%x\n", RegNum, bq24296_reg);
    
    bq24296_reg &= ~(MASK << SHIFT);
    bq24296_reg |= (val << SHIFT);

    ret = bq24296_write_byte(RegNum, bq24296_reg);
    //printk("[bq24296_config_interface] Write Reg[%x]=0x%x\n", RegNum, bq24296_reg);

    // Check
    //bq24296_read_byte(RegNum, &bq24296_reg);
    //printk("[bq24296_config_interface] Check Reg[%x]=0x%x\n", RegNum, bq24296_reg);
...
void bq24296_dump_register(void)
{
    ...
    for (i=0;i<bq24296_REG_NUM;i++)
    {
        ...
        printk("[bq24296_dump_register] Reg[0x%X]=0x%X\n", i, bq24296_reg[i]);        
    }
}
...
{
    printk("[show_bq24296_access] 0x%x\n", g_reg_value_bq24296);
...
static ssize_t store_bq24296_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    printk("[store_bq24296_access] \n");
    
    if(buf != NULL && size != 0)
    {
        printk("[store_bq24296_access] buf is %s and size is %d \n",buf,size);
        reg_address = simple_strtoul(buf,&pvalue,16);
        
        if(size > 3)
        {        
            reg_value = simple_strtoul((pvalue+1),NULL,16);        
            printk("[store_bq24296_access] write bq24296 reg 0x%x with value 0x%x !\n",reg_address,reg_value);
        ...
        {    
            ...
            printk("[store_bq24296_access] read bq24296 reg 0x%x with value 0x%x !\n",reg_address,g_reg_value_bq24296);
            printk("[store_bq24296_access] Please use \"cat bq24296_access\" to get value\r\n");
        }        
    }    
    return size;
}
...
{    
    ...
    printk("******** bq24296_user_space_probe!! ********\n" );
...
static struct i2c_board_info __initdata i2c_bq24296 = { I2C_BOARD_INFO("bq24296", (0xd6>>1))};

static int __init bq24296_init(void)
{    
    int ret=0;
    
    printk("[bq24296_init] init start\n");
    
    i2c_register_board_info(BQ24296_BUSNUM, &i2c_bq24296, 1);

    if(i2c_add_driver(&bq24296_driver)!=0)
    {
        printk("[bq24296_init] failed to register bq24296 i2c driver.\n");
    }
    else
    {
        printk("[bq24296_init] Success to register bq24296 i2c driver.\n");
    }

    // bq24296 user space access interface
    ret = platform_device_register(&bq24296_user_space_device);
    if (ret) {
        printk("****[bq24296_init] Unable to device register(%d)\n", ret);
        return ret;
    }    
    ret = platform_driver_register(&bq24296_user_space_driver);
    if (ret) {
        printk("****[bq24296_init] Unable to register driver (%d)\n", ret);
        return ret;
    }
