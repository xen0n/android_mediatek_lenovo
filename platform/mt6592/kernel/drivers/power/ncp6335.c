...
static int ncp6335_driver_suspend(struct i2c_client *client, pm_message_t mesg);
static int ncp6335_driver_resume(struct i2c_client *client);
...
static struct i2c_driver ncp6335_driver = {
    ...
    .suspend     = ncp6335_driver_suspend,
    .resume      = ncp6335_driver_resume,
...
{    
    ...
    ret = ncp6335_config_interface(0x12,0x00,0xFF,0); 
    ret = ncp6335_config_interface(0x13,0x19,0xFF,0); 
    ret = ncp6335_config_interface(0x15,0x80,0xFF,0); 
...
static int ncp6335_driver_suspend(struct i2c_client *client, pm_message_t mesg)
{
    ...
    if(g_ncp6335_hw_exist==1)
    {
        ncp6335_config_interface(0x13,0x19,0xFF,0);
        ncp6335_read_interface(0x13,&val,0xFF,0);
        printk("[ncp6335_driver_suspend] Reg[0x13]=0x%x\n",val);
    }
    else
    {
        printk("[ncp6335_driver_suspend] no HW\n");
    }
    return 0;
}
