...
int disphal_process_dbg_opt(const char *opt)
{
    ...
    else if (0 == strncmp(opt, "merge:", 6))
    {
		if (0 == strncmp(opt + 6, "cmd", 3))
		{
			DISP_Set_MergeTrigger_Mode(COMMAND_DONE);
        }
        else if (0 == strncmp(opt + 6, "tl", 2))
        {
...
{
    // DPI0
    mt_irq_set_sens(MT_DISP_DPI0_IRQ_ID, MT_LEVEL_SENSITIVE);
    mt_irq_set_polarity(MT_DISP_DPI0_IRQ_ID, MT_POLARITY_LOW);

    // DSI
    mt_irq_set_sens(MT_DISP_DSI_IRQ_ID, MT_LEVEL_SENSITIVE);
    mt_irq_set_polarity(MT_DISP_DSI_IRQ_ID, MT_POLARITY_LOW);
...
int disphal_allocate_fb(struct resource* res, unsigned int* pa, unsigned int* va, unsigned int* dma_pa)
{
    ...
    *va = (unsigned int) ioremap_nocache(res->start, res->end - res->start + 1);
    if (disp_use_mmu)
    {
		m4u_alloc_mva_fix(M4U_CLNTMOD_LCDC_UI, *pa, (res->end - res->start + 1), 0,	0, dma_pa);
