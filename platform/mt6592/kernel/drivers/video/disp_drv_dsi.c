...
void init_dsi(BOOL isDsiPoweredOn)
{
	...
	if (lcm_params->dsi.mode == CMD_MODE || (!isDsiPoweredOn))
	{
	...
	if(1 == lcm_params->dsi.ufoe_enable)		
		DSI_CHECK_RET(DSI_PS_Control(lcm_params->dsi.PS, lcm_params->height, (lcm_params->width + lcm_params->width % 4)/2 * dsiTmpBufBpp));
	else
...
static DISP_STATUS dsi_init(UINT32 fbVA, UINT32 fbPA, BOOL isLcmInited)
{
	...
	fbconfig_if_drv = disphal_fbconfig_get_if();

	if (!disp_drv_dsi_init_context()) 
		return DISP_STATUS_NOT_IMPLEMENTED;

#if 0	
	// this is for DSI Video Mode, to avoid SMI clock is disable in the flow of lk->kernel
	if(clk_is_force_on(MT_CG_DISP0_SMI_LARB0)==0 ||
		clk_is_force_on(MT_CG_DISP0_SMI_COMMON)==0)
	 {
		 printk("[DDP] error, MT_CG_DISP0_SMI_LARB0 is not force on\n");
	 }
	 else
	 {
	...
	if(1 == lcm_params->dsi.ufoe_enable){
		UFOE_para ufoe_config;
		ufoe_config.bypass = 0;
		ufoe_config.ufoe_cfg_0 = 1;
		ufoe_config.ufoe_cfg_1 = 0x5;
		ufoe_config.width = lcm_params->width;
		ufoe_config.height = lcm_params->height;
		UFOE_Init(isLcmInited);
		UFOE_Config(ufoe_config);		
	}
	else{
		UFOE_PowerOn();
		UFOE_PowerOff();
