    ...
    while(((DISP_REG_GET(DISP_REG_CONFIG_MUTEX(mutexID))& DISP_INT_MUTEX_BIT_MASK) != DISP_INT_MUTEX_BIT_MASK))
    {
        ...
        if(cnt>10000)
        {
			...
			if(DISP_IsDecoupleMode() && mutexID == gMemOutMutexID)
		    {
                ...
                disp_print_reg_driectly(DISP_MODULE_OVL);
                disp_print_reg_driectly(DISP_MODULE_WDMA0);
    ...
    g_dst_module = pConfig->dstModule;
       ...
       
        {
	        ...
	        else if(pConfig->srcModule==DISP_MODULE_OVL)
	        {
	            ...
	            DISP_REG_SET(DISP_REG_CONFIG_MUTEX_MOD(mutexId), (1<<3)|(1<<10)|(1<<7)|(1<<9)|((pConfig->enableUFOE?1:0)<<13)); //ovl, rdma1, color, bls, ufoe(if enabled)

	        }
	        else if(pConfig->srcModule==DISP_MODULE_RDMA)
	        {

	            DISP_REG_SET(DISP_REG_CONFIG_MUTEX_MOD(mutexId), (1<<10)|(1<<7)|(1<<9)|((pConfig->enableUFOE?1:0)<<13)); //rdma, color, bls, ufoe(if enabled)
        ...
        switch(pConfig->dstModule)
        {
            ...
            case DISP_MODULE_DSI_CMD:
                ...
                DISP_REG_SET(DISP_REG_CONFIG_DISP_OUT_SEL, 0);        // BLS --> UFOE(default)
                DISP_REG_SET(DISP_REG_CONFIG_DISP_OUT_SEL, 0<<1);     // RDMA1 --> UFOE(default)
                DISP_REG_SET(DISP_REG_CONFIG_DPI0_SEL, 0);        	  // from BLS(default)
            break;
            ...
            case DISP_MODULE_DPI0:
                ...
                //DISP_REG_SET(DISP_REG_CONFIG_DISP_OUT_SEL, 0);         // BLS --> UFOE
                DISP_REG_SET(DISP_REG_CONFIG_DISP_OUT_SEL, 1<<1);      // RDMA1 --> DPI0
                DISP_REG_SET(DISP_REG_CONFIG_DPI0_SEL, 1);        	   // from RDMA1
            break;
...
// Ultra config
    ...
    DISP_REG_SET(DISP_REG_OVL_DATAPATH_CON, gOvlReq);
...
{
    // set low freq if FHD or higher resolution
//    if(1) //fix me by query resolution
	if(DISP_GetScreenWidth() * DISP_GetScreenHeight() >= 1080*1920)
    {
