...
void config_misc_info(int md_id, unsigned int base[], unsigned int size)
{
	...
	if(NULL != base) {
		...
		{
			CCCI_MSG_INF(md_id, "ctl", "MISC_32K_LESS no support, crystal_exist_status 1\n");
		...
		CCCI_MSG_INF(md_id, "ctl", "random_seed=%x,misc_info.feature_2_val[0]=%x,d=%d\n", random_seed,misc_info.feature_2_val[0],random_seed);
		misc_info.support_mask |= (FEATURE_SUPPORT<<(MISC_RAND_SEED*2));
		
		//--- Feature 3, GPS&MD co-clk setting
		#ifdef ENABLE_GPS_MD_COCLK
		misc_info.feature_3_val[0] = wmt_get_coclock_setting_for_ccci();
		misc_info.support_mask |= (FEATURE_SUPPORT<<(MISC_MD_COCLK_SETTING*2));
		#else
		misc_info.support_mask |= (FEATURE_NOT_SUPPORT<<(MISC_MD_COCLK_SETTING*2));
		#endif		
...
static int ungate_md1(void)
{
	...
	ccci_write32(md1_boot_slave_Key, 0x3567C766); 	//MT6592 no change
	ccci_write32(md1_boot_slave_Vector, 0x0); 	//MT6592 no change
	ccci_write32(md1_boot_slave_En, 0xA3B66175); 	//MT6592 no change
...
int set_ap_smem_remap(int md_id, unsigned int src, unsigned int des)
{
	...
	if(!smem_remapped) {
		...
		remap1_val =(((des>>24)|0x1)&0xFF)
				  + ((((INVALID_ADDR[md_id] + INVALID_OFFSET*14)>>16)|1<<8)&0xFF00)
				  + ((((INVALID_ADDR[md_id] + INVALID_OFFSET*15)>>8)|1<<16)&0xFF0000)
				  + ((((INVALID_ADDR[md_id] + INVALID_OFFSET*16)>>0)|1<<24)&0xFF000000);
		
		remap2_val =((((INVALID_ADDR[md_id] + INVALID_OFFSET*17)>>24)|0x1)&0xFF)
				  + ((((INVALID_ADDR[md_id] + INVALID_OFFSET*18)>>16)|1<<8)&0xFF00)
...
int set_md_smem_remap(int md_id, unsigned int src, unsigned int des)
{
	...
	switch(md_id)
	{
		case MD_SYS1:
			remap1_val =(((des>>24)|0x1)&0xFF)
					  + ((((INVALID_ADDR[md_id] + INVALID_OFFSET*0)>>16)|1<<8)&0xFF00)
					  + ((((INVALID_ADDR[md_id] + INVALID_OFFSET*1)>>8)|1<<16)&0xFF0000)
					  + ((((INVALID_ADDR[md_id] + INVALID_OFFSET*2)>>0)|1<<24)&0xFF000000);
			remap2_val =((((INVALID_ADDR[md_id] + INVALID_OFFSET*3)>>24)|0x1)&0xFF)
					  + ((((INVALID_ADDR[md_id] + INVALID_OFFSET*4)>>16)|1<<8)&0xFF00)
					  + ((((INVALID_ADDR[md_id] + INVALID_OFFSET*5)>>8)|1<<16)&0xFF0000)
					  + ((((INVALID_ADDR[md_id] + INVALID_OFFSET*6)>>0)|1<<24)&0xFF000000);
...
int set_md_rom_rw_mem_remap(int md_id, unsigned int src, unsigned int des)
{
	...
	switch(md_id)
	{
		case MD_SYS1:
			remap1_val =(((des>>24)|0x1)&0xFF)
					  + ((((INVALID_ADDR[md_id] + INVALID_OFFSET*7)>>16)|1<<8)&0xFF00)
					  + ((((INVALID_ADDR[md_id] + INVALID_OFFSET*8)>>8)|1<<16)&0xFF0000)
					  + ((((INVALID_ADDR[md_id] + INVALID_OFFSET*9)>>0)|1<<24)&0xFF000000);
			remap2_val =((((INVALID_ADDR[md_id] + INVALID_OFFSET*10)>>24)|0x1)&0xFF)
					  + ((((INVALID_ADDR[md_id] + INVALID_OFFSET*11)>>16)|1<<8)&0xFF00)
					  + ((((INVALID_ADDR[md_id] + INVALID_OFFSET*12)>>8)|1<<16)&0xFF0000)
					  + ((((INVALID_ADDR[md_id] + INVALID_OFFSET*13)>>0)|1<<24)&0xFF000000);
...
void ccci_dump_md_register(int md_id)
{
	...
	map_ptr=md1_mcu_ptr;
	for(ptr=(unsigned char *)MD1_MCU_ADDR_BASE,idx=0x100;idx<=0x464;idx+=4)
	{
		CCCI_MSG_INF(md_id, "cci", "[%08X]->[%08X]=%08x",(unsigned int)(ptr+idx), (unsigned int)(map_ptr+idx) ,*((unsigned int*) (map_ptr+idx)));
	}
...
EXPORT_SYMBOL(ccci_dump_md_register);
#endif
