...
#include "mach/mt_cpufreq.h" // for DVFS
...
#define MT6592_VCODEC_DEV_MAJOR_NUMBER 160   //189
//#define VDEC_USE_L2C 

static dev_t vcodec_devno = MKDEV(MT6592_VCODEC_DEV_MAJOR_NUMBER,0);
...
static DEFINE_MUTEX(LivePhotoEMILock);
...
static int MT6592Driver_Open_Count;         //mutex : DriverOpenCountLock
...
static VAL_UINT32_T gu4LivePhotoEMICounter = 0;   //mutex : LivePhotoEMILock
...
static VAL_UINT32_T gu4WFDRefCount = 0; //mutex : VScenLock
static VAL_UINT32_T gu4VENCRefCount = 0; //mutex : VScenLock
static VAL_UINT32_T gu4VDECRefCount = 0; //mutex : VScenLock

static VAL_UINT32_T gLockTimeOutCount = 0;
//#define MT6592_VCODEC_DEBUG
#ifdef MT6592_VCODEC_DEBUG
#undef VCODEC_DEBUG
...
static long vcodec_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd) {
        case VCODEC_SET_SCENARIO:
            {
                VAL_CURRENT_SCENARIO_T  _temp;
                mutex_lock(&VScenLock);
                user_data_addr = (VAL_UINT8_T *)arg;
                ret = copy_from_user(&_temp, user_data_addr, sizeof(VAL_CURRENT_SCENARIO_T));
                if (ret) {
                    MFV_LOGE("[ERROR] VCODEC_SET_SCENARIO, copy_from_user failed: %d\n", ret);
                    mutex_unlock(&VScenLock);
                    return -EFAULT;
                }
                if (_temp.u4Scenario & VAL_VCODEC_SCENARIO_VENC_1080P) {
                    if (_temp.u4OnOff) {
                        if (gu4VENCRefCount == 0) {
                            MFV_LOGE("Set DVFS VENC on %d", gu4VENCRefCount);
                            mt_soc_dvfs(SOC_DVFS_TYPE_VENC, 1);
                        }
                        ++gu4VENCRefCount;
                    }
                    else {
                        if (gu4VENCRefCount == 1) {
                            MFV_LOGE("Set DVFS VENC off %d", gu4VENCRefCount);
                            mt_soc_dvfs(SOC_DVFS_TYPE_VENC, 0);
                        }
                        --gu4VENCRefCount;
                    }
                }
                else if (_temp.u4Scenario & VAL_VCODEC_SCENARIO_VDEC_1080P) {
                    if (_temp.u4OnOff) {
                        if (gu4VDECRefCount == 0 && gu4WFDRefCount > 0) {
                            MFV_LOGE("Set DVFS WFD on %d %d", gu4VDECRefCount, gu4WFDRefCount);
                            mt_soc_dvfs(SOC_DVFS_TYPE_WIFI_DISPLAY, 1);
                        }
                        ++gu4VDECRefCount;
                    }
                    else {
                        if (gu4VDECRefCount == 1 && gu4WFDRefCount > 0) {
                            MFV_LOGE("Set DVFS WFD off %d %d", gu4VDECRefCount, gu4WFDRefCount);
                            mt_soc_dvfs(SOC_DVFS_TYPE_WIFI_DISPLAY, 0);
                        }
                        --gu4VDECRefCount;
                    }
                }
                else if (_temp.u4Scenario & VAL_VCODEC_SCENARIO_VENC_WFD) {
                    if (_temp.u4OnOff) {
                        if (gu4WFDRefCount == 0 && gu4VDECRefCount > 0) {
                            MFV_LOGE("Set DVFS WFD on %d %d", gu4VDECRefCount, gu4WFDRefCount);
                            mt_soc_dvfs(SOC_DVFS_TYPE_WIFI_DISPLAY, 1);
                        }
                        ++gu4WFDRefCount;
                        ...
                        if (gu4WFDRefCount == 1 && gu4VDECRefCount > 0) {
                            MFV_LOGE("Set DVFS WFD off %d %d", gu4VDECRefCount, gu4WFDRefCount);
                            mt_soc_dvfs(SOC_DVFS_TYPE_WIFI_DISPLAY, 0);
                        }
                        --gu4WFDRefCount;
        ...
        case VCODEC_GET_SCENARIO_CNT:
            {
                VAL_CURRENT_SCENARIO_CNT_T  _temp = {0};
                mutex_lock(&VScenLock);
                user_data_addr = (VAL_UINT8_T *)arg;

                ret = copy_from_user(&_temp, user_data_addr, sizeof(VAL_CURRENT_SCENARIO_CNT_T));
                if (ret) {
                    MFV_LOGE("[ERROR] VCODEC_GET_SCENARIO_CNT, copy_from_user failed: %d\n", ret);
                    mutex_unlock(&VScenLock);
                    return -EFAULT;
                }

                if (_temp.u4Scenario & VAL_VCODEC_SCENARIO_VENC_1080P) {
                    _temp.u4ScenarioRefCount = gu4VENCRefCount;
                }
                else if (_temp.u4Scenario & VAL_VCODEC_SCENARIO_VDEC_1080P) {
                    _temp.u4ScenarioRefCount = gu4VDECRefCount;
                }
                else if (_temp.u4Scenario & VAL_VCODEC_SCENARIO_VENC_WFD) {
                    _temp.u4ScenarioRefCount = gu4WFDRefCount;
                }

                ret = copy_to_user(user_data_addr, &_temp, sizeof(VAL_CURRENT_SCENARIO_CNT_T));
                if (ret) {
                    MFV_LOGE("[ERROR] VAL_CURRENT_SCENARIO_CNT_T, copy_to_user failed: %d\n", ret);
                    mutex_unlock(&VScenLock);
                    return -EFAULT;
                }
                mutex_unlock(&VScenLock);
        ...
        case VCODEC_SET_THREAD_ID:
            MFV_LOGE("[MT6592] VCODEC_SET_THREAD_ID [EMPTY] + tid = %d\n", current->pid);

            MFV_LOGE("[MT6592] VCODEC_SET_THREAD_ID [EMPTY] - tid = %d\n", current->pid);
        break;

        case VCODEC_ALLOC_NON_CACHE_BUFFER:
            MFV_LOGE("[MT6592][M4U]! VCODEC_ALLOC_NON_CACHE_BUFFER + tid = %d\n", current->pid);
        ...
        case VCODEC_FREE_NON_CACHE_BUFFER:
            MFV_LOGE("[MT6592][M4U]! VCODEC_FREE_NON_CACHE_BUFFER + tid = %d\n", current->pid);
        ...
        case VCODEC_INC_DEC_EMI_USER:
            MFV_LOGD("[MT6592] VCODEC_INC_DEC_EMI_USER + tid = %d\n", current->pid);
        ...
        case VCODEC_DEC_DEC_EMI_USER:
            MFV_LOGD("[MT6592] VCODEC_DEC_DEC_EMI_USER + tid = %d\n", current->pid);
        ...
        case VCODEC_INC_ENC_EMI_USER:
            MFV_LOGD("[MT6592] VCODEC_INC_ENC_EMI_USER + tid = %d\n", current->pid);
        ...
        case VCODEC_DEC_ENC_EMI_USER:
            MFV_LOGD("[MT6592] VCODEC_DEC_ENC_EMI_USER + tid = %d\n", current->pid);
        ...
        case VCODEC_LOCKHW:
            MFV_LOGD("[MT6592] VCODEC_LOCKHW + tid = %d\n", current->pid);
            user_data_addr = (VAL_UINT8_T *)arg;
        ...
        case VCODEC_UNLOCKHW:
            MFV_LOGD("[MT6592] VCODEC_UNLOCKHW + tid = %d\n", current->pid);
            user_data_addr = (VAL_UINT8_T *)arg;
            ret = copy_from_user(&rHWLock, user_data_addr, sizeof(VAL_HW_LOCK_T));
            if (ret) {
        ...
        case VCODEC_INC_PWR_USER:
            MFV_LOGD("[MT6592] VCODEC_INC_PWR_USER + tid = %d\n", current->pid);
            user_data_addr = (VAL_UINT8_T *)arg;
        ...
        case VCODEC_DEC_PWR_USER:
            MFV_LOGD("[MT6592] VCODEC_DEC_PWR_USER + tid = %d\n", current->pid);
            user_data_addr = (VAL_UINT8_T *)arg;
            ret = copy_from_user(&rPowerParam, user_data_addr, sizeof(VAL_POWER_T));
            if (ret) {
        ...
        case VCODEC_WAITISR:
            ...
            MFV_LOGD("[MT6592] VCODEC_WAITISR + tid = %d\n", current->pid);
            user_data_addr = (VAL_UINT8_T *)arg;
        ...
        {
            MFV_LOGE("[MT6592] VCODEC_INITHWLOCK [EMPTY] + - tid = %d\n", current->pid);
        ...
        {
            MFV_LOGE("[MT6592] VCODEC_DEINITHWLOCK [EMPTY] + - tid = %d\n", current->pid);
        ...
        case VCODEC_GET_CPU_LOADING_INFO:
        {
            ...
            MFV_LOGD("[MT6592] VCODEC_GET_CPU_LOADING_INFO +\n");
            user_data_addr = (VAL_UINT8_T *)arg;
            // TODO:
        ...
        {
            MFV_LOGD("[MT6592] VCODEC_GET_CORE_LOADING + - tid = %d\n", current->pid);
        ...
        {
            MFV_LOGD("[MT6592] VCODEC_GET_CORE_NUMBER + - tid = %d\n", current->pid);

            user_data_addr = (VAL_UINT8_T *)arg;
            temp_nr_cpu_ids = num_possible_cpus();
        ...
        case VCODEC_SET_CPU_OPP_LIMIT:
            MFV_LOGE("[MT6592] VCODEC_SET_CPU_OPP_LIMIT [EMPTY] + - tid = %d\n", current->pid);
            user_data_addr = (VAL_UINT8_T *)arg;
		...
		case VCODEC_INC_LIVEPHOTO_EMI_USER:
			{
				MFV_LOGD("[MT6592] VCODEC_INC_LIVEPHOTO_EMI_USER + tid = %d\n", current->pid);

				mutex_lock(&LivePhotoEMILock);
				gu4LivePhotoEMICounter++;
				MFV_LOGE("LIVEPHOTO_EMI_USER = %d\n", gu4LivePhotoEMICounter);
				user_data_addr = (VAL_UINT8_T *)arg;
				ret = copy_to_user(user_data_addr, &gu4LivePhotoEMICounter, sizeof(VAL_UINT32_T));
				if (ret) {
					MFV_LOGE("[ERROR] VCODEC_INC_LIVEPHOTO_EMI_USER, copy_to_user failed: %d\n", ret);
					mutex_unlock(&LivePhotoEMILock);
					return -EFAULT;
				}
				mutex_unlock(&LivePhotoEMILock);
  	...
  	case VCODEC_DEC_LIVEPHOTO_EMI_USER:
	  	{
			MFV_LOGD("[MT6592] VCODEC_DEC_LIVEPHOTO_EMI_USER + tid = %d\n", current->pid);
			...
			ret = copy_to_user(user_data_addr, &gu4LivePhotoEMICounter, sizeof(VAL_UINT32_T));
			if (ret) {
				MFV_LOGE("[ERROR] VCODEC_DEC_LIVEPHOTO_EMI_USER, copy_to_user failed: %d\n", ret);
				mutex_unlock(&LivePhotoEMILock);
				return -EFAULT;
			}
			mutex_unlock(&LivePhotoEMILock);
...
static int vcodec_open(struct inode *inode, struct file *file)
{
    ...
    MT6592Driver_Open_Count++;

    MFV_LOGE("vcodec_open pid = %d, MT6592Driver_Open_Count %d\n", current->pid, MT6592Driver_Open_Count);
...
static int vcodec_flush(struct file *file, fl_owner_t id)
{
    ...
    MFV_LOGD("[VCODEC_DEBUG] vcodec_flush(e), curr_tid =%d\n", current->pid);
    return 0;
}
...
static int vcodec_release(struct inode *inode, struct file *file)
{
    ...
    mutex_lock(&DriverOpenCountLock);
    MFV_LOGE("vcodec_release pid = %d, MT6592Driver_Open_Count %d\n", current->pid, MT6592Driver_Open_Count);

    MT6592Driver_Open_Count--;

    if (MT6592Driver_Open_Count == 0) {
...
MODULE_DESCRIPTION("MT6592 Vcodec Driver");
MODULE_LICENSE("GPL");
