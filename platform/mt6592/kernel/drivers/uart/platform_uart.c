...
void dump_uart_reg(void)
{
	...
	if (uart->poweron_count > 0)      
	{
		...
		case 0:
	    		...
	    		rx_pin = mt_get_gpio_in(GPIO_UART_URXD1_PIN);
	    		#endif
	    		break;
		case 1:
	    		#ifdef GPIO_UART_URXD2_PIN
	    		rx_pin = mt_get_gpio_in(GPIO_UART_URXD2_PIN);
	    		#endif
	    		break;
		case 2:
	    		#ifdef GPIO_UART_URXD3_PIN
	    		rx_pin = mt_get_gpio_in(GPIO_UART_URXD3_PIN);
	    		#endif
	    		break;
		case 3:
	    		#ifdef GPIO_UART_URXD4_PIN
	    		rx_pin = mt_get_gpio_in(GPIO_UART_URXD4_PIN);
	    		#endif
	    		break;
		default:
	    		break;
		}
	    printk(KERN_ERR "[UART%d] LSR=0x%x rx_pin=%d ", uart->nport, lsr, rx_pin);
  	}else
	    printk(KERN_ERR "[UART%d] clk off ", uart->nport);	
    }
    printk(KERN_DEBUG "\n");
}
/*---------------------------------------------------------------------------*/
