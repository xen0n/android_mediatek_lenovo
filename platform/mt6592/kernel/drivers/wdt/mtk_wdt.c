...
int mtk_wdt_swsysret_config(int bit,int set_value)
{
	...
	case 0x10000000:
			...
			printk("aee need config ddr reserve mode in 92 \n");
...
static int mtk_wdt_probe(struct platform_device *dev)
{
	...
	mtk_wdt_deglitch_en();
	//set kernel init ok flag
	interval_val = DRV_Reg32(MTK_WDT_NONRST_REG2);
	interval_val = interval_val | (0x1<<2); //set bit 2
	DRV_WriteReg32(MTK_WDT_NONRST_REG2,interval_val);
	...
	printk("mtk_wdt_probe : done MTK_WDT_REQ_IRQ_EN(%x),MTK_WDT_NONRST_REG2(%x)\n",DRV_Reg32(MTK_WDT_REQ_IRQ_EN),DRV_Reg32(MTK_WDT_NONRST_REG2));
