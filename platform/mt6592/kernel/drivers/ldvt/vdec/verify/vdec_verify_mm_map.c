...
#if VMMU_SUPPORT
static const UCHAR *g_pu1AllocSA = (UCHAR *)VMMU_SA; // need 64k align
#else
static const UCHAR *g_pu1AllocSA = (UCHAR *)FILELIST_SA;
#endif
...
void * x_alloc_aligned_verify_mem_ex(UINT32 u4Size, UINT32 u4Align, BOOL fgChannelA, const char *szFunction, INT32 i4Line)
{
    ...
    p = (UCHAR *)(((UINT32)p + u4Align-1) & (~(u4Align - 1)));
	...
	p = ioremap_nocache((unsigned long)p, u4Size + 1024); // extra 1K to avoid m4u_user_v2p failed.
	memset(p, 0, u4Size+1024);

    printk("<vdec> ioremap %uM %uK %uB return 0x%p (0x%08X) (%s, %d)\n", u4MB, u4KB, u4B, p, u4PhyAddr, szFunction, i4Line);
    DBG_H264_PRINTF("<vdec> ioremap %uM %uK %uB return 0x%08X (0x%p) (%s, %d)\n", u4MB, u4KB, u4B, p, u4PhyAddr, szFunction, i4Line);
...
void vMemoryAllocate_RM(UINT32 u4InstID)
{
  ...
  printk("_pucRMGoldenDataBuf[u4InstID] Address :%p!!!\n",_pucRMGoldenDataBuf[u4InstID]);
  DBG_H264_PRINTF("_pucRMGoldenDataBuf[u4InstID] Address :%p!!!\n",_pucRMGoldenDataBuf[u4InstID]);

  _pucDumpYBuf_1[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(GOLD_Y_SZ_1, 2048, WORKING_AREA_CHANEL_A);
  printk("_pucDumpYBuf_1[u4InstID] Address :%p!!!\n",_pucDumpYBuf_1[u4InstID]);
  DBG_H264_PRINTF("_pucDumpYBuf_1[u4InstID] Address :%p!!!\n",_pucDumpYBuf_1[u4InstID]);
  _pucDumpCBuf_1[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(GOLD_C_SZ_1, 2048, WORKING_AREA_CHANEL_A);
  printk("_pucDumpCBuf_1[u4InstID] Address :%p!!!\n",_pucDumpCBuf_1[u4InstID]);
  DBG_H264_PRINTF("_pucDumpCBuf_1[u4InstID] Address :%p!!!\n",_pucDumpCBuf_1[u4InstID]);
  ...
  printk("_pucVP6VLDWrapperWorkspace[u4InstID] = 0x%p\n", _pucVP6VLDWrapperWorkspace[u4InstID]);
  DBG_H264_PRINTF("_pucVP6VLDWrapperWorkspace[u4InstID] = 0x%p\n", _pucVP6VLDWrapperWorkspace[u4InstID]);
  printk("_pucVP6PPWrapperWorkspace[u4InstID] = 0x%p\n", _pucVP6PPWrapperWorkspace[u4InstID]);
  DBG_H264_PRINTF("_pucVP6PPWrapperWorkspace[u4InstID] = 0x%p\n", _pucVP6PPWrapperWorkspace[u4InstID]);

#ifdef CAPTURE_ESA_LOG
       _pucESALog[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(ESALOG_SZ, 1024, 1);
      printk("_pucESALog[u4InstID] = 0x%p\n", _pucESALog[u4InstID]);
      ...
      printk("_pucESATotalBufpu4InstId] = 0x%p\n", _pucESATotalBuf[u4InstID]);
#endif
...
//   printk("Enter m4u_user_v2p()! 0x%x\n", va);
 ...
 //  printk("pgd_none=%d, pgd_bad=%d\n", pgd_none(*pgd), pgd_bad(*pgd));
   ...
   {    
       printk("Error: m4u_user_v2p(), virtual addr=0x%x, pud invalid! \n", va); 
       return 0;
   }
 ...
 //  printk("pmd_none=%d, pmd_bad=%d, pmd_val=0x%x\n", pmd_none(*pmd), pmd_bad(*pmd), pmd_val(*pmd));
   ...
   if (( (unsigned int)pmd_val(*pmd) & PMD_TYPE_MASK) == PMD_TYPE_TABLE)
   {
      if(pmd_none(*pmd)||pmd_bad(*pmd))
      {
         printk("Error: m4u_user_v2p(), virtual addr 0x%x, pmd invalid a! \n", va);
         return 0;
      }
 ...
 //     printk("m4u_user_v2p(), pte 0x%x\n", pte);
  ...
  //       printk("PA = 0x%8x\n", pa);
   ...
   else /* Only 1 level page table */
   {
      if(pmd_none(*pmd))
      {
         printk("Error: m4u_user_v2p(), virtual addr 0x%x, pmd invalid b! \n", va);
         return 0;
      }
...
void* vMemoryAllocateLoop(UINT32 u4Size)
{
    ...
    if (u4Size > KMALLOC_SZ)
    	...
    	for (j=0; j<u4Count; j++)
    		...
    		printk("allocate memory SA = 0x%p\n", pucmem);
    		}
    	if (u4Res)
    		{
      		pucmem = (UCHAR *)x_alloc_aligned_verify_mem(u4Res,1024, 1); // 4M
    		printk("allocate Res memory SA = 0x%p\n", pucmem);
  		}
    	}
    else
    	{
...
void* vMemoryFreeLoop(void *pvAddr, UINT32 u4Size)
{
    ...
    if (u4Size > KMALLOC_SZ)
    	...
    	if (u4Res)
    		{
    		pucmem = pvAddr;
    		printk("Free Res memory SA = 0x%p\n", pucmem);
...
//    printk("CRCBUF_SA = 0x%x\n", CRCBUF_SA);
...
#endif
  {
    ...
    DBG_H264_PRINTF("_pucFileListSa[u4InstID] = 0x%p\n", _pucFileListSa[u4InstID]);    
  }
  ...
  else
  {
    ...
    DBG_H264_PRINTF("_pucVFifo[u4InstID] = 0x%p\n", _pucVFifo[u4InstID]);
  }
  
  {
        ...
        printk("_pucDumpYBuf[u4InstID] = 0x%p\n", _pucDumpYBuf[u4InstID]);
        DBG_H264_PRINTF("_pucDumpYBuf[u4InstID] = 0x%p\n", _pucDumpYBuf[u4InstID]);
  }

  {
        ...
        printk("_pucDumpCBuf[u4InstID] = 0x%p\n", _pucDumpCBuf[u4InstID]);
        DBG_H264_PRINTF("_pucDumpCBuf[u4InstID] = 0x%p\n", _pucDumpCBuf[u4InstID]);
  }
  ...
  printk("_pucCRCBuf[u4InstID] = 0x%p\n", _pucCRCBuf[u4InstID]);
  DBG_H264_PRINTF("_pucCRCBuf[u4InstID] = 0x%p\n", _pucCRCBuf[u4InstID]);
  ...
  printk("_pucRegister[0] = 0x%p\n", _pucRegister[0]);
  DBG_H264_PRINTF("_pucRegister[0] = 0x%p\n", _pucRegister[0]);
  ...
  printk("_pucH264CRCYBuf[u4InstID] = 0x%p\n", _pucH264CRCYBuf[u4InstID]);
  DBG_H264_PRINTF("_pucH264CRCYBuf[u4InstID] = 0x%p\n", _pucH264CRCYBuf[u4InstID]);
#endif
  ...
  printk("_pucCRCYBuf[u4InstID] = 0x%p\n",_pucCRCYBuf[u4InstID]);
  DBG_H264_PRINTF("_pucCRCYBuf[u4InstID] = 0x%p\n",_pucCRCYBuf[u4InstID]);
  printk("_pucCRCCBCRBuf[u4InstID] = 0x%p\n",_pucCRCCBCRBuf[u4InstID]);
  DBG_H264_PRINTF("_pucCRCCBCRBuf[u4InstID] = 0x%p\n",_pucCRCCBCRBuf[u4InstID]);
  #endif
#ifdef CAPTURE_ESA_LOG
       _pucESALog[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(ESALOG_SZ, 1024, 1);
      printk("_pucESALog[u4InstID] = 0x%p\n", _pucESALog[u4InstID]);
  ...
  printk("_pucRegisterLog[u4InstID] = 0x%p\n", _pucRegisterLog[u4InstID]);
  DBG_H264_PRINTF("_pucRegisterLog[u4InstID] = 0x%p\n", _pucRegisterLog[u4InstID]);
#endif
...
BOOL fgVDecAllocWorkBuffer(UINT32 u4InstID)
{
  ...
  if(_u4CodecVer[u4InstID] == VDEC_H264)
  {
      ...
      printk("_pucVLDWrapperWrok[u4InstID] = 0x%p\n", _pucVLDWrapperWrok[u4InstID]);
      DBG_H264_PRINTF("_pucVLDWrapperWrok[u4InstID] = 0x%p\n", _pucVLDWrapperWrok[u4InstID]);
      printk("_pucPPWrapperWork[u4InstID] = 0x%p\n", _pucPPWrapperWork[u4InstID]);
      DBG_H264_PRINTF("_pucPPWrapperWork[u4InstID] = 0x%p\n", _pucPPWrapperWork[u4InstID]);
      #endif
     ...
     printk("_pucDPB[u4InstID] = 0x%p\n", _pucDPB[u4InstID]);
     DBG_H264_PRINTF("_pucDPB[u4InstID] = 0x%p\n", _pucDPB[u4InstID]);

     _pucPredSa[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(AVC_PRED_SZ,1024, WORKING_AREA_CHANEL_A);
     printk("_pucPredSa[u4InstID] = 0x%p\n", _pucPredSa[u4InstID]);
     DBG_H264_PRINTF("_pucPredSa[u4InstID] = 0x%p\n", _pucPredSa[u4InstID]);
     ...
     {
          _pucAddressSwapBuf[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(ADDSWAP_BUF_SZ, 2048,  WORKING_AREA_CHANEL_A);
          printk("_pucAddressSwapBuf[u4InstID] = 0x%p\n", _pucAddressSwapBuf[u4InstID]);
          DBG_H264_PRINTF("_pucAddressSwapBuf[u4InstID] = 0x%p\n", _pucAddressSwapBuf[u4InstID]);
     }
     ...
     if (u4InstID == 0)
     {
	   ...
	   printk("_pucAVCMVBuff_Main[%d] = 0x0%p !\n",i,_pucAVCMVBuff_Main[i]);
       }
     #endif
	 //15/9/2010 mtk40343
      ...
      DBG_H264_PRINTF("_pucAVCMVBuff_Main[%d] = 0x%p !\n",i,_pucAVCMVBuff_Main[0]);
       for (i=1; i<17; i++)
       {
     ...
     else
     {
	   ...
	   printk("_pucAVCMVBuff_Sub[%d] = 0x0%p !\n",i,_pucAVCMVBuff_Sub[i]);
       DBG_H264_PRINTF("_pucAVCMVBuff_Sub[%d] = 0x0%p !\n",i,_pucAVCMVBuff_Sub[i]);
         }
     }
#endif
      ...
      _pucVDSCLWork3Sa[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(VDSCL_SW_WORK3_SZ, 1024, VDSCL_CHANEL_A);
      _pucVDSCLWork4Sa[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(VDSCL_SW_WORK4_SZ, 1024, VDSCL_CHANEL_A);
      printk("_pucVDSCLBuf[u4InstID] = 0x%p\n", _pucVDSCLBuf[u4InstID]);
      printk("_pucVDSCLWork1Sa[u4InstID] = 0x%p\n", _pucVDSCLWork1Sa[u4InstID]);
      printk("_pucVDSCLWork2Sa[u4InstID] = 0x%p\n", _pucVDSCLWork2Sa[u4InstID]);
      printk("_pucVDSCLWork3Sa[u4InstID] = 0x%p\n", _pucVDSCLWork3Sa[u4InstID]);
      printk("_pucVDSCLWork4Sa[u4InstID] = 0x%p\n",_pucVDSCLWork4Sa [u4InstID]);
#endif
...
//~AVC Working Buffer
  ...
  // WMV Part
      ...
      _pucPic2Y[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(PIC_Y_SZ,2048, WORKING_AREA_CHANEL_A);
      _pucPic2C[u4InstID] = (UCHAR *)x_alloc_aligned_verify_mem(PIC_C_SZ,2048, WORKING_AREA_CHANEL_A);
      printk("_pucPic0Y[u4InstID] = 0x%p\n", _pucPic0Y[u4InstID]);
      printk("_pucPic0C[u4InstID] = 0x%p\n", _pucPic0C[u4InstID]);
      printk("_pucPic1Y[u4InstID] = 0x%p\n", _pucPic1Y[u4InstID]);
      printk("_pucPic1C[u4InstID] = 0x%p\n", _pucPic1C[u4InstID]);
      printk("_pucPic2Y[u4InstID] = 0x%p\n",_pucPic2Y [u4InstID]);
      printk("_pucPic2C[u4InstID] = 0x%p\n", _pucPic2C[u4InstID]);
     ...
     if (_u4WmvMode[u4InstID] == 0)
     {
      ...
      printk("_pucDcac[u4InstID] = 0x%p\n", _pucDcac[u4InstID]);
      printk("_pucMv_1[u4InstID] = 0x%p\n", _pucMv_1[u4InstID]);
      printk("_pucMv_2[u4InstID] = 0x%p\n", _pucMv_2[u4InstID]);
     }
      ...
      printk("_pucBp_1[u4InstID] = 0x%p\n", _pucBp_1[u4InstID]);
      printk("_pucBp_2[u4InstID] = 0x%p\n", _pucBp_2[u4InstID]);
      printk("_pucBp_3[u4InstID] = 0x%p\n", _pucBp_3[u4InstID]);
      printk("_pucBp_4[u4InstID] = 0x%p\n", _pucBp_4[u4InstID]);
   ...
   if (_u4WmvMode[u4InstID] == 0)
   {
      ...
      printk("_pucMv_3[u4InstID] = 0x%p\n", _pucMv_3[u4InstID]);
      printk("_pucMv_1_2[u4InstID] = 0x%p\n", _pucMv_1_2[u4InstID]);
      printk("_pucDcac_2[u4InstID] = 0x%p\n", _pucDcac_2[u4InstID]);
   }
   else
   {
      ...
      printk("_pucBp0New[u4InstID] = 0x%p\n", _pucBp0New[u4InstID]); //sun for temp
      printk("_pucBp1New[u4InstID] = 0x%p\n", _pucBp1New[u4InstID]);
      printk("_pucBp2New[u4InstID] = 0x%p\n", _pucBp2New[u4InstID]);
    }
  ...
  // MPEG part
      ...
      printk("_pucPp_1[u4InstID] = 0x%p\n", _pucPp_1[u4InstID]);
      printk("_pucPp_2[u4InstID] = 0x%p\n", _pucPp_2[u4InstID]);
      printk("_pucPpYSa[u4InstID] = 0x%p\n", _pucPpYSa[u4InstID]);
      printk("_pucPpCSa[u4InstID] = 0x%p\n", _pucPpCSa[u4InstID]);
      printk("_pucMp4Dcac[u4InstID] = 0x%p\n", _pucMp4Dcac[u4InstID]);
      printk("_pucMp4Mvec[u4InstID] = 0x%p\n", _pucMp4Mvec[u4InstID]);
      printk("_pucMp4Bmb1[u4InstID] = 0x%p\n", _pucMp4Bmb1[u4InstID]);
      printk("_pucMp4Bmb2[u4InstID] = 0x%p\n", _pucMp4Bmb2[u4InstID]);
      printk("_pucMp4Bcode[u4InstID] = 0x%p\n", _pucMp4Bcode[u4InstID]);
      printk("_pucDumpSRAMBuf[u4InstID]  = 0x%p\n", _pucDumpSRAMBuf[u4InstID]);
      printk("_pucMp4DataPartition[u4InstID]  = 0x%p\n", _pucMp4DataPartition[u4InstID]);
      printk("_pucMp4NotCoded[u4InstID]  = 0x%p\n", _pucMp4NotCoded[u4InstID]);
      printk("_pucMp4MvDirect[u4InstID]  = 0x%p\n", _pucMp4MvDirect[u4InstID]);
  }
  else
...
*/
      ...
      printk("_pucSizeFileBuf[u4InstID] = 0x%p\n", _pucSizeFileBuf[u4InstID]);
      printk("_pucWorkYBuf[u4InstID] = 0x%p\n", _pucWorkYBuf[u4InstID]);
    //  printk("_pucWorkCBuf[u4InstID] = 0x%p\n", _pucWorkCBuf[u4InstID]);
      printk("_pucDumpArfYBuf[u4InstID] = 0x%p\n", _pucDumpArfYBuf[u4InstID]);
      printk("_pucDumpGldYBuf[u4InstID] = 0x%p\n", _pucDumpGldYBuf[u4InstID]);
      printk("_pucDumpLstYBuf[u4InstID] = 0x%p\n", _pucDumpLstYBuf[u4InstID]);
    //  printk("_pucDumpArfCBuf[u4InstID] = 0x%p\n", _pucDumpArfCBuf[u4InstID]);
   //   printk("_pucDumpGldCBuf[u4InstID] = 0x%p\n", _pucDumpGldCBuf[u4InstID]);
  //    printk("_pucDumpLstCBuf[u4InstID] = 0x%p\n", _pucDumpLstCBuf[u4InstID]);
      printk("_pucPpYSa[u4InstID] = 0x%p\n", _pucPpYSa[u4InstID]);
      printk("_pucPpCSa[u4InstID] = 0x%p\n", _pucPpCSa[u4InstID]);
      printk("_pucVLDWrapperWrok[u4InstID] = 0x%p\n", _pucVLDWrapperWrok[u4InstID]);
      printk("_pucPPWrapperWork[u4InstID] = 0x%p\n", _pucPPWrapperWork[u4InstID]);
#if VDEC_VP8_WRAPPER_OFF
      printk("_pucVLDWrapper[u4InstID] = 0x%p\n", _pucVLDWrapper[u4InstID]);
      printk("_pucPPWrapperY[u4InstID] = 0x%p\n", _pucPPWrapperY[u4InstID]);
      printk("_pucPPWrapperC[u4InstID] = 0x%p\n", _pucPPWrapperC[u4InstID]);
