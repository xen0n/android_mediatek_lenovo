...
static struct clkmux muxs[NR_MUXS] = {
    {
        ...
        .name = __stringify(MUX_AUD_22M),
        .base_addr = CLK_MISC_CFG_0,
        //.sel_mask = 0x00000007,
        .pdn_mask = 0x00000002,
        //.offset = 0,
        //.nr_inputs = 1,
        .ops = &hd_audio_clkmux_ops,
        .pll = &plls[UNIVPLL],
    }, {
        .name = __stringify(MUX_AUD_24M),
        .base_addr = CLK_MISC_CFG_0,
        //.sel_mask = 0x00000007,
        .pdn_mask = 0x00000004,
        //.offset = 0,
...
static void cg_all_force_on(void)
{
    //Top, don't care here
    //INFRA CG
    clk_writel(INFRA_PDN_CLR, 0x0091BFE3);
    //PERI CG
    clk_writel(PERI_PDN0_CLR, 0x07FFFFFF);
    //AUDIO
    clk_clrl(AUDIO_TOP_CON0, (0x1 << 6) | (0x1 << 2));
    //MFG
    clk_writel(MFG_CG_CLR, 0x1);
    //DISP
    clk_writel(DISP_CG_CLR0, 0x1FFFFF);
...
static void cg_bootup_pdn(void)
{
    ...
    clk_writel(PERI_PDN0_SET, 0x06FEF7FD); //turn off I2C0 by CLKMGR
...
//                clk_info("[%s]: clk=%d, clk->state=%d\n", __func__, j, clk->state);
    ...
    clks[MT_CG_DISP1_DPI_DIGITAL_LANE].mux = &muxs[MT_MUX_DPI0];
...
{
    clk_info("[%s]: Dump Top MUX register, CLK_CFG_0=0x%x, CLK_CFG_1=0x%x, CLK_CFG_2=0x%x, CLK_CFG_3=0x%x, CLK_CFG_4=0x%x, CLK_CFG_5=0x%x\n", \
    __func__, clk_readl(CLK_CFG_0), clk_readl(CLK_CFG_1), clk_readl(CLK_CFG_2), clk_readl(CLK_CFG_3), clk_readl(CLK_CFG_4), clk_readl(CLK_CFG_5));
...
static int __init mt_clkmgr_late_init(void)
{
    ...
    //mt_enable_clock(MT_CG_DISP1_DPI_DIGITAL_LANE, "clkmgr");
    //mt_disable_clock(MT_CG_DISP1_DPI_DIGITAL_LANE, "clkmgr");
