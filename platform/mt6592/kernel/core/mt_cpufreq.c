...
//#define ORIGINAL_VCORE_VOSEL_ON
...
static bool mt_cpufreq_init_first_call_already = false;
...
static unsigned int g_soc_freq_spare0 = 0;
...
static bool mt_socfreq_disabled_by_sdio_autoK = false;
static bool mt_socfreq_disabled_by_sdio_ot = false;
#endif
...
/* PMIC MT6323, VPROC */
static struct mt_cpu_freq_info mt6592_freqs_e1_3[] = {
    OP(DVFS_F1, DVFS_V1_ptp1),
    OP(DVFS_F2, DVFS_V2_ptp2),
    OP(DVFS_F3, DVFS_V3_ptp3),
    OP(DVFS_F4, DVFS_V4_ptp4),
...
/* PMIC external buck (fan53555), VPROC */
static struct mt_cpu_freq_info mt6592_freqs_fan53555_e1_3[] = {
    OP(DVFS_F1, DVFS_V1_fan53555),
    OP(DVFS_F2, DVFS_V2_fan53555),
    OP(DVFS_F3, DVFS_V3_fan53555),
    OP(DVFS_F4, DVFS_V4_fan53555),
...
extern struct mutex mt6333_i2c_access;

DEFINE_SPINLOCK(voltage_lock);
unsigned long voltage_lock_flag;

void DFS_phase_mt6333_config_interface(char dest_vol)
{
	char phase[PHASE_NUMBER];
	int i;
	int ret_val;
	char src_vol = 0;
	char src_vol_1 = 0;
	char src_debug_vol, src_debug_vol_1;
...
#if defined(DEBUG_DFS_phase_mt6333_config_interface)
	printk("[Phase check]dst_vol:0x%x\n", dest_vol);
#endif
	...
	if (PHASE_NUMBER == 3) {
		/* quick path */
		i = (dest_vol - src_vol) / PHASE_NUMBER;
		phase[0] = i;
		phase[1] = i;
		phase[2] = (dest_vol - src_vol) - i - i;
	} else {
		for (i = 0; i < PHASE_NUMBER; i++){
	    		phase[i] = (dest_vol - src_vol) / PHASE_NUMBER;
	    		if (i == (PHASE_NUMBER - 1)) {
				phase[i] = phase[i] + 
					(dest_vol - src_vol) % PHASE_NUMBER;
			}
		}
	}
	...
	for (i = 0 ; i < PHASE_NUMBER; i++) {
		src_vol = src_vol + phase[i];
#if defined(DEBUG_DFS_phase_mt6333_config_interface)
		printk("set voltage:0x%x \n", src_vol);
#endif

		mt6333_config_interface_polling(0x6B, src_vol, 0x7F, 0);
		mt6333_config_interface_polling(0x6C, src_vol, 0x7F, 0); /* 1.1 */

		spin_lock_irqsave(&voltage_lock,voltage_lock_flag);
		DFS_Phase_delay();
		spin_unlock_irqrestore(&voltage_lock,voltage_lock_flag);
	}

	disable_clock(MT_CG_PERI_AP_DMA, "DFS_GDMA_MODULE");

#if defined(DEBUG_DFS_phase_mt6333_config_interface)
	printk("%d phase voltage adjust done\n", PHASE_NUMBER);
#endif
...
/* socfreq disabled by sdio autoK */
...
void mt_vcore_dvfs_disable_by_sdio(unsigned int type, bool disabled)
{
	...
	if(type == 0)
	{
	    mt_socfreq_disabled_by_sdio_autoK = disabled;

	    if(mt_socfreq_disabled_by_sdio_autoK == true)
	    {
		    ...
		    xlog_printk(ANDROID_LOG_INFO, "Power/DVFS", "mt_socfreq_disable_by_sdio, limited freq. at \n");
	    }
		xlog_printk(ANDROID_LOG_INFO, "Power/DVFS", "mt_socfreq_disable_by_sdio, mt_socfreq_disabled_by_sdio_autoK = %d\n", mt_socfreq_disabled_by_sdio_autoK);
	}
	else if(type == 1)
	{
		mt_socfreq_disabled_by_sdio_ot = disabled;
		dprintk("mt_socfreq_disable_by_sdio: mt_socfreq_disabled_by_sdio_ot = %d\n", mt_socfreq_disabled_by_sdio_ot);
	}
...
EXPORT_SYMBOL(mt_vcore_dvfs_disable_by_sdio);

/* 1.00V = 1000000, 1.075V = 1075000, 1.125V = 1125000 */
void mt_vcore_dvfs_volt_set_by_sdio(unsigned int volt)
{
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/DVFS", "mt_socfreq_volt_set_by_sdio, reg = %d\n", reg);
	
	/* sdio autoK need to disable dvfs first, then set volt. */
	if(mt_socfreq_disabled_by_sdio_autoK == true)
	{
		#ifdef ORIGINAL_VCORE_VOSEL_ON
		mt6333_set_vcore_vosel_on(reg); 
		#else
		DFS_phase_mt6333_config_interface(reg); 
		#endif
	}
	else
	{
		xlog_printk(ANDROID_LOG_INFO, "Power/DVFS", "mt_socfreq_volt_set_by_sdio, could not set volt now,  mt_socfreq_disabled_by_sdio_autoK = %d\n", mt_socfreq_disabled_by_sdio_autoK);
	}
#endif
}
EXPORT_SYMBOL(mt_vcore_dvfs_volt_set_by_sdio);

/*************************************************************************************
* Check MMPLL Efuse
**************************************************************************************/
static unsigned int mt_soc_mmpll_freq_efuse(void)
{
    unsigned int soc_spare0 = 0, ret = 0;

	soc_spare0 = (get_devinfo_with_index(3) >> 30) & 0x3;

    if (soc_spare0 == 0x0)
    {
        ret = 0; // no limit
    }
    else if (soc_spare0 == 0x1)
    {
        ret = 1; // 700MHz
    }
    else if (soc_spare0 == 0x2)
    {
        ret = 2; // 600MHz
    }
    else if (soc_spare0 == 0x3)
    {
        ret = 3; // 500MHz, no product support
    }
...
static void mt_socfreq_return_default(unsigned int state)
{
	...
	else
	{
		...
		#endif
		{
			...
			#ifdef ORIGINAL_VCORE_VOSEL_ON
			mt6333_set_vcore_vosel_on(g_default_soc_volt);
			#else
			DFS_phase_mt6333_config_interface(g_default_soc_volt);
			#endif
			...
			else
			{
				...
				#else
				DFS_phase_mt6333_config_interface(0x3C); /* DVFS_VCORE_1p1 = 1.075V */
				#endif
			}
			#endif
			...
			#ifdef CPUFREQ_MEMPLL_OD
			/* Set MEMPLL */
			if((g_ddr_frequency_type == 3)||(g_ddr_frequency_type == 2))
			{
...
static void mt_socfreq_ddr_detection(void)
{
	...
	#endif
    {
		...
		if(g_soc_DRAM_Type == LPDDR3) /* 3 */
		{
			...
			/* set MMPLL freq */
			if((g_soc_freq_spare0 == 0) || (g_soc_freq_spare0 == 1))
			{
				mt_dfs_mmpll(2782000); /* MMPLL 700MHz*/
				g_cur_socfreq_gpu_freq = GPU_DVFS_F0;
			}
			else if(g_soc_freq_spare0 == 2)
			{
...
static unsigned int mt_soc_mempll_opp_mask(SOC_DVFS_TYPE_ENUM type, unsigned int sochp_enable)
{
		...
		if(((type >= SOC_DVFS_TYPE_VENC) && (type < SOC_DVFS_TYPE_NUM)) && (type != SOC_DVFS_TYPE_DISPLAY))
		#else
...
{	
	...
	#ifdef CPUFREQ_SDIO_TRANSFER
    /************************************************
    * SOC DVFS disabled when sdio online tuning
    *************************************************/
    if ((mt_socfreq_disabled_by_sdio_ot == TRUE) || (mt_socfreq_disabled_by_sdio_autoK == TRUE))
    {
    	dprintk("SDIO online tuning, disable soc dvfs, return!\n");
        return 0;
    }
	#endif
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					#else
					DFS_phase_mt6333_config_interface(0x3C); /* DVFS_VCORE_1p05 = 1.075V */
					#endif
...
unsigned int mt_cpufreq_voltage_set_by_ptpod(unsigned int pmic_volt[], unsigned int array_size)
{
	...
	if(is_ext_buck_exist() == 0)
	{
	    ...
	    if(g_cpufreq_get_ptp_level == 0)
	    {
	        ...
	        if((pmic_volt[4] <= 0x48) && (pmic_volt[4] >= 0x30)) /* VCORE VMAX <= 1.15V, VMIN >= 1.0V */
	        {
	            mt_cpufreq_spm_volt[6] = pmic_volt[4];
	        }
	    ...
	    else if(g_cpufreq_get_ptp_level == 2)  
	    {
	        ...
	        if((pmic_volt[2] <= 0x48) && (pmic_volt[2] >= 0x30)) /* VCORE VMAX <= 1.15V, VMIN >= 1.0V */
	        {
	            mt_cpufreq_spm_volt[6] = pmic_volt[2];
	        }
	    ...
	    else if(g_cpufreq_get_ptp_level == 3)  
	    {
	        ...
	        mt_cpufreq_spm_volt[6] = pmic_volt[3];
...
static void mt_cpufreq_set(unsigned int freq_old, unsigned int freq_new, unsigned int target_volt, unsigned int new_cpu_idx)
{
	...
	freq_old = mt_cpu_freqs[g_cur_cpufreq_OPPidx].cpufreq_khz;

	dprintk("mt_cpufreq_set: freq_old = %d, freq_new = %d\n", freq_old, freq_new);
	...
	{
		xlog_printk(ANDROID_LOG_INFO, "Power/DVFS", "Set max freq in first time!\n");
		//set_init_ret = mt_cpufreq_set_initial(g_max_freq_by_ptp, 0);
		/* Above 1GHz for FH, and match for ptpod needed 1.05GHz/ 1.0V */
		if(g_cpufreq_get_ptp_level == 0)
			set_init_ret = mt_cpufreq_set_initial(DVFS_F4, 4);
		else if(g_cpufreq_get_ptp_level == 1)
			set_init_ret = mt_cpufreq_set_initial(DVFS_F4, 4);
		else if(g_cpufreq_get_ptp_level == 2)
			set_init_ret = mt_cpufreq_set_initial(DVFS_F4, 2);
		else if(g_cpufreq_get_ptp_level == 3)
			set_init_ret = mt_cpufreq_set_initial(DVFS_F4, 3);
		else
			set_init_ret = mt_cpufreq_set_initial(DVFS_F4, 4);
		...
		if(set_init_ret == 1)
		{
			...
			if(g_cpufreq_get_ptp_level == 0)
				g_cur_cpufreq_OPPidx = 4;
			else if(g_cpufreq_get_ptp_level == 1)
				g_cur_cpufreq_OPPidx = 4;
			else if(g_cpufreq_get_ptp_level == 2)
				g_cur_cpufreq_OPPidx = 2;
			else if(g_cpufreq_get_ptp_level == 3)
				g_cur_cpufreq_OPPidx = 3;
			else
				g_cur_cpufreq_OPPidx = 4;
    ...
    *******************************/
	...
	if(is_ext_buck_exist() == 0)
	{
	    if(g_cpufreq_get_ptp_level == 0)
	        ...
	        next.cpufreq_khz = mt6592_freqs_e1_3[idx].cpufreq_khz;
	    else
	        next.cpufreq_khz = mt6592_freqs_e1[idx].cpufreq_khz;
	}
	else
	{
		if(is_fan53555_exist() == 1)
		{
			if(g_cpufreq_get_ptp_level == 0)
				...
				next.cpufreq_khz = mt6592_freqs_fan53555_e1_3[idx].cpufreq_khz;
			else
				next.cpufreq_khz = mt6592_freqs_fan53555_e1[idx].cpufreq_khz;
		}
		else if(is_ncp6335_exist() == 1)
...
static int mt_cpufreq_init(struct cpufreq_policy *policy)
{
    ...
    **********************************************/
	...
	if(mt_cpufreq_init_first_call_already == false)
	{
    	policy->cur = DVFS_F5;  /* preloader frequency */
		mt_cpufreq_init_first_call_already = true;
	}
	else
	{
    	policy->cur = g_max_freq_by_ptp;  /* frequency would be set to max when cpu hotplug */	
	}
	...
	if(is_ext_buck_exist() == 0)
	{
	    if(g_cpufreq_get_ptp_level == 0)
	        ...
	        ret = mt_setup_freqs_table(policy, ARRAY_AND_SIZE(mt6592_freqs_e1_3));
	    else
	        ret = mt_setup_freqs_table(policy, ARRAY_AND_SIZE(mt6592_freqs_e1));
	}
	else
	{
		if(is_fan53555_exist() == 1)
		{
		    if(g_cpufreq_get_ptp_level == 0)
		        ...
		        ret = mt_setup_freqs_table(policy, ARRAY_AND_SIZE(mt6592_freqs_fan53555_e1_3));
		    else
		        ret = mt_setup_freqs_table(policy, ARRAY_AND_SIZE(mt6592_freqs_fan53555_e1));		
		}
		else if(is_ncp6335_exist() == 1)
...
void mt_cpufreq_return_default_DVS_by_ptpod(void)
{
	...
	if(is_ext_buck_exist() == 0)
	{
	    ...
	    {
			#ifdef MT_DVFS_LOW_VOLTAGE_SUPPORT
			mt_cpufreq_reg_write(0x3A, PMIC_WRAP_DVFS_WDATA0); // 1.06V VPROC (1.0625)
			mt_cpufreq_reg_write(0x35, PMIC_WRAP_DVFS_WDATA1); // 1.03V VPROC (1.03125)
			mt_cpufreq_reg_write(0x30, PMIC_WRAP_DVFS_WDATA2); // 1.00V VPROC
			mt_cpufreq_reg_write(0x28, PMIC_WRAP_DVFS_WDATA3); // 0.95V VPROC
			...
			/* For PTP-OD */
			mt_cpufreq_pmic_volt[0] = 0x3A; // 1.06V VPROC (1.0625)
			mt_cpufreq_pmic_volt[1] = 0x35; // 1.03V VPROC (1.03125)
			mt_cpufreq_pmic_volt[2] = 0x30; // 1.00V VPROC
			mt_cpufreq_pmic_volt[3] = 0x28; // 0.95V VPROC
			...
			mt_cpufreq_reg_write(0x35, PMIC_WRAP_DVFS_WDATA1); // 1.03V VPROC (1.03125)
			mt_cpufreq_reg_write(0x30, PMIC_WRAP_DVFS_WDATA2); // 1.00V VPROC
			mt_cpufreq_reg_write(0x30, PMIC_WRAP_DVFS_WDATA3); // 1.00V VPROC
			mt_cpufreq_reg_write(0x30, PMIC_WRAP_DVFS_WDATA4); // 1.00V VPROC
			mt_cpufreq_reg_write(0x30, PMIC_WRAP_DVFS_WDATA5); // 1.00V VPROC
			mt_cpufreq_reg_write(0x30, PMIC_WRAP_DVFS_WDATA6); // 1.00V VPROC
			if(g_soc_avs_type == 0)
	    ...
	    {	
            #ifdef MT_DVFS_LOW_VOLTAGE_SUPPORT
            mt_cpufreq_reg_write(0x3F, PMIC_WRAP_DVFS_WDATA0); // 1.09V VPROC (1.09375)
            mt_cpufreq_reg_write(0x3A, PMIC_WRAP_DVFS_WDATA1); // 1.06V VPROC (1.0625)
            mt_cpufreq_reg_write(0x35, PMIC_WRAP_DVFS_WDATA2); // 1.03V VPROC (1.03125)
            mt_cpufreq_reg_write(0x30, PMIC_WRAP_DVFS_WDATA3); // 1.00V VPROC
            mt_cpufreq_reg_write(0x28, PMIC_WRAP_DVFS_WDATA4); // 0.95V VPROC
            mt_cpufreq_reg_write(0x20, PMIC_WRAP_DVFS_WDATA5); // 0.90V VPROC
            mt_cpufreq_reg_write(0x20, PMIC_WRAP_DVFS_WDATA6); // 0.90V VPROC
            if(g_soc_avs_type == 0)
            	mt_cpufreq_reg_write(0x44, PMIC_WRAP_DVFS_WDATA7); // 1.125V VPROC, for SOC DVFS high performance
            ...
            /* For PTP-OD */
            mt_cpufreq_pmic_volt[0] = 0x3F; // 1.09V VPROC (1.09375)
            mt_cpufreq_pmic_volt[1] = 0x3A; // 1.06V VPROC (1.0625)
            mt_cpufreq_pmic_volt[2] = 0x35; // 1.03V VPROC (1.03125)
            mt_cpufreq_pmic_volt[3] = 0x30; // 1.00V VPROC
            mt_cpufreq_pmic_volt[4] = 0x28; // 0.95V VPROC
            mt_cpufreq_pmic_volt[5] = 0x20; // 0.90V VPROC
            mt_cpufreq_pmic_volt[6] = 0x20; // 0.90V VPROC
            if(g_soc_avs_type == 0)
            	mt_cpufreq_pmic_volt[7] = 0x44; // 1.125V VPROC, for SOC DVFS high performance
	...
	else
	{
		...
		if(is_fan53555_exist() == 1)
		{
			...
			else if(g_cpufreq_get_ptp_level == 2)
	    	{
				#ifdef MT_DVFS_LOW_VOLTAGE_SUPPORT
		        /* For PTP-OD */
		        mt_cpufreq_pmic_volt[0] = 106000; // 1.06V VPROC
		        mt_cpufreq_pmic_volt[1] = 103000; // 1.03V VPROC
		        mt_cpufreq_pmic_volt[2] = 100000; // 1.00V VPROC
		        mt_cpufreq_pmic_volt[3] = 95000;  // 0.95V VPROC
		        mt_cpufreq_pmic_volt[4] = 90000;  // 0.90V VPROC
		        mt_cpufreq_pmic_volt[5] = 90000;  // 0.95V VPROC
		        mt_cpufreq_pmic_volt[6] = 90000;  // 0.90V VPROC
		        //mt_cpufreq_pmic_volt[7] = 0x44; // 1.125V VPROC, for SOC DVFS high performance
			...
			else if(g_cpufreq_get_ptp_level == 3)
	    	{
		        #ifdef MT_DVFS_LOW_VOLTAGE_SUPPORT
		        /* For PTP-OD */
		        mt_cpufreq_pmic_volt[0] = 109000; // 1.09V VPROC
		        mt_cpufreq_pmic_volt[1] = 106000; // 1.06V VPROC
		        mt_cpufreq_pmic_volt[2] = 103000; // 1.03V VPROC
		        mt_cpufreq_pmic_volt[3] = 100000; // 1.00V VPROC
		        mt_cpufreq_pmic_volt[4] = 95000;  // 0.95V VPROC
		...
		else if(is_ncp6335_exist() == 1)
		{
			...
			else if(g_cpufreq_get_ptp_level == 2)
			{
				#ifdef MT_DVFS_LOW_VOLTAGE_SUPPORT
				/* For PTP-OD */
				mt_cpufreq_pmic_volt[0] = 106250; // 1.0625V VPROC
				mt_cpufreq_pmic_volt[1] = 103125; // 1.03125V VPROC
				mt_cpufreq_pmic_volt[2] = 100000; // 1.00V VPROC
				mt_cpufreq_pmic_volt[3] = 95000;  // 0.95V VPROC
				mt_cpufreq_pmic_volt[4] = 90000;  // 0.90V VPROC
				mt_cpufreq_pmic_volt[5] = 90000;  // 0.90V VPROC
			...
			else if(g_cpufreq_get_ptp_level == 3)
			{
				#ifdef MT_DVFS_LOW_VOLTAGE_SUPPORT
				/* For PTP-OD */
				mt_cpufreq_pmic_volt[0] = 109375; // 1.09375V VPROC
				mt_cpufreq_pmic_volt[1] = 106250; // 1.0625V VPROC
				mt_cpufreq_pmic_volt[2] = 103125; // 1.03125V VPROC
				mt_cpufreq_pmic_volt[3] = 100000; // 1.00V VPROC
				mt_cpufreq_pmic_volt[4] = 95000;  // 0.95V VPROC
				mt_cpufreq_pmic_volt[5] = 90000;  // 0.90V VPROC
				mt_cpufreq_pmic_volt[6] = 90000;  // 0.90V VPROC
...
static int mt_socfreq_dump_info_read(char *buf, char **start, off_t off, int count, int *eof, void *data)
{
	...
	p += sprintf(p, "g_soc_freq_spare0 = %d\n", g_soc_freq_spare0);
	#ifdef CPUFREQ_MEMPLL_OD
...
static int mt_cpufreq_power_dump_read(char *buf, char **start, off_t off, int count, int *eof, void *data)
{
	...
	p += sprintf(p, "g_cpu_power_table_num = %d\n", g_cpu_power_table_num);
...
static int mt_cpufreq_total_limited_cpu_power_read(char *buf, char **start, off_t off, int count, int *eof, void *data)
{
    int len = 0;
    char *p = buf;

	p += sprintf(p, "mt_cpufreq_limited_cpu_power_array[0] = %d\n", mt_cpufreq_limited_cpu_power_array[0]);
	p += sprintf(p, "mt_cpufreq_limited_cpu_power_array[1] = %d\n", mt_cpufreq_limited_cpu_power_array[1]);
	p += sprintf(p, "mt_cpufreq_limited_cpu_power_array[2] = %d\n", mt_cpufreq_limited_cpu_power_array[2]);

    len = p - buf;
    return len;
}

static ssize_t mt_cpufreq_total_limited_cpu_power_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
    int limit_0 = 0, limit_1 = 0, limit_2 = 0;

    if (sscanf(buffer, "%d %d %d", &limit_0, &limit_1, &limit_2) == 3)
    {
        mt_cpufreq_limited_cpu_power_array[0] = limit_0;
        mt_cpufreq_limited_cpu_power_array[1] = limit_1;
        mt_cpufreq_limited_cpu_power_array[2] = limit_2;
    }
    else
    {
        xlog_printk(ANDROID_LOG_INFO, "Power/DVFS", "bad argument!! argument should be oC\n");
...
static int mt_cpufreq_total_limited_gpu_power_read(char *buf, char **start, off_t off, int count, int *eof, void *data)
{
    int len = 0;
    char *p = buf;

	p += sprintf(p, "mt_cpufreq_limited_gpu_power_array[0] = %d\n", mt_cpufreq_limited_gpu_power_array[0]);
	p += sprintf(p, "mt_cpufreq_limited_gpu_power_array[1] = %d\n", mt_cpufreq_limited_gpu_power_array[1]);
	p += sprintf(p, "mt_cpufreq_limited_gpu_power_array[2] = %d\n", mt_cpufreq_limited_gpu_power_array[2]);

    len = p - buf;
    return len;
}

static ssize_t mt_cpufreq_total_limited_gpu_power_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
    int limit_0 = 0, limit_1 = 0, limit_2 = 0;

    if (sscanf(buffer, "%d %d %d", &limit_0, &limit_1, &limit_2) == 3)
    {
        mt_cpufreq_limited_gpu_power_array[0] = limit_0;
        mt_cpufreq_limited_gpu_power_array[1] = limit_1;
        mt_cpufreq_limited_gpu_power_array[2] = limit_2;
    }
    else
    {
        xlog_printk(ANDROID_LOG_INFO, "Power/DVFS", "bad argument!! argument should be oC\n");
...
static int mt_cpufreq_pdrv_probe(struct platform_device *pdev)
{
    ...
    *************************************************/
	...
	g_soc_freq_spare0 = mt_soc_mmpll_freq_efuse();
	#ifdef CPUFREQ_HIGHEST_TURBO_MODE
	...
	#if 1
	g_cur_freq = 845000;  /* Default 845000KHz in pre-loader */ /* Default 0.95v in pmic init */

    if(g_cpufreq_get_ptp_level == 0)
        g_cur_cpufreq_OPPidx = 5;
    else if(g_cpufreq_get_ptp_level == 1)
        g_cur_cpufreq_OPPidx = 5;
    else if(g_cpufreq_get_ptp_level == 2)
        g_cur_cpufreq_OPPidx = 3;
	...
	*/
	...
	if(g_cpufreq_get_ptp_level == 0)
	{
		mt_cpufreq_ptpod_temperature_delta_counter_limit = MTH_DELTA_COUNTER_LIMIT_1P7GHz; // 300ms
		mt_cpufreq_downgrade_freq_step_idx_1 = MTH_FREQ_STEP_IDX_1_1P7GHz; // downgrade frequency 1 level, 45C < x < 65C
		mt_cpufreq_downgrade_freq_step_idx_2 = MTH_FREQ_STEP_IDX_2_1P7GHz; // downgrade frequency 1 level, 65C < x
		mt_cpufreq_ptpod_temperature_limit_1 = MTH_TEMP_LIMIT_1_1P7GHz; //45000; //45C
		mt_cpufreq_ptpod_temperature_limit_2 = MTH_TEMP_LIMIT_2_1P7GHz; //65000; //65C
		mt_cpufreq_ptpod_temperature_delta_1 = MTH_TEMP_DELTA_1_1P7GHz; // tempature delta 5C
		mt_cpufreq_ptpod_temperature_delta_1_ratio_1 = MTH_TEMP_DELTA_1_RATIO_1_1P7GHz; // 0:10 , 45C < x < 65C, delta < 5C
		mt_cpufreq_ptpod_temperature_delta_2_ratio_1 = MTH_TEMP_DELTA_2_RATIO_1_1P7GHz; // 5:5, 45C < x < 65C, delta > 5C
		mt_cpufreq_ptpod_temperature_delta_1_ratio_2 = MTH_TEMP_DELTA_1_RATIO_2_1P7GHz; // 0:10, 65C < x, delta < 5C
		mt_cpufreq_ptpod_temperature_delta_2_ratio_2 = MTH_TEMP_DELTA_2_RATIO_2_1P7GHz; // 8:2, 65C < x, delta > 5C
	}
	else if(g_cpufreq_get_ptp_level == 1)
	{
		mt_cpufreq_ptpod_temperature_delta_counter_limit = MTH_DELTA_COUNTER_LIMIT_2GHz; // 600ms
		mt_cpufreq_downgrade_freq_step_idx_1 = MTH_FREQ_STEP_IDX_1_2GHz; // downgrade frequency 1 level, 45C < x < 65C
		mt_cpufreq_downgrade_freq_step_idx_2 = MTH_FREQ_STEP_IDX_2_2GHz; // downgrade frequency 2 level, 65C < x
		mt_cpufreq_ptpod_temperature_limit_1 = MTH_TEMP_LIMIT_1_2GHz; //45000; //45C
		mt_cpufreq_ptpod_temperature_limit_2 = MTH_TEMP_LIMIT_2_2GHz; //70000; //70C
		mt_cpufreq_ptpod_temperature_delta_1 = MTH_TEMP_DELTA_1_2GHz; // tempature delta 5C
		mt_cpufreq_ptpod_temperature_delta_1_ratio_1 = MTH_TEMP_DELTA_1_RATIO_1_2GHz; // 12:8 , 45C < x < 65C, delta < 5C
		mt_cpufreq_ptpod_temperature_delta_2_ratio_1 = MTH_TEMP_DELTA_2_RATIO_1_2GHz; // 18:2, 45C < x < 65C, delta > 5C
		mt_cpufreq_ptpod_temperature_delta_1_ratio_2 = MTH_TEMP_DELTA_1_RATIO_2_2GHz; // 10:10, 65C < x, delta < 5C
		mt_cpufreq_ptpod_temperature_delta_2_ratio_2 = MTH_TEMP_DELTA_2_RATIO_2_2GHz; // 18:2, 65C < x, delta > 5C
	}
	else if(g_cpufreq_get_ptp_level == 2)
	{
		// not define
...
static int __init mt_cpufreq_pdrv_init(void)
{
    ...
    else
    {
        ...
        mt_entry = create_proc_entry("cpufreq_total_limited_cpu_power", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir);
		if (mt_entry)
		{
		    mt_entry->read_proc = mt_cpufreq_total_limited_cpu_power_read;
			mt_entry->write_proc = mt_cpufreq_total_limited_cpu_power_write;
		}

		mt_entry = create_proc_entry("cpufreq_total_limited_gpu_power", S_IRUGO | S_IWUSR | S_IWGRP, mt_cpufreq_dir);
		if (mt_entry)
		{
			mt_entry->read_proc = mt_cpufreq_total_limited_gpu_power_read;
			mt_entry->write_proc = mt_cpufreq_total_limited_gpu_power_write;
		}
