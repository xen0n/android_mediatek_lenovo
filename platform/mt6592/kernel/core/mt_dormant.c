...
#define CA7_BUS_CONFIG          (MCUSYS_CFGREG_BASE + 0x18)
#define CA7_BUS_CONFIG_PHY      (0x10200000 + 0x18)
#define CA7_CCI400_DVM_EN       (0xF0395000)
#define CCI400_STATUS           (0xF039000C )
...
extern void mcu_div(void);
 
/*********************************
* glabal variable
**********************************/
volatile static int dormant_ret_flag[2][4] ;
volatile int power_state[2][4];
...
static void platform_save_context(bool individual)
{
    unsigned cpu_id = read_cpuid(), cluster_id = read_clusterid();
    cpu_context *ns_cpu_ctx = &switcher_context.cluster.core[cpu_id+(cluster_id*4)].ns_cpu_ctx;    
    ...
    stop_generic_timer(cp15_timer_ctx);
	  ...
	  /*
    * Save CPUXGPT register when musys power down only for mt6592
	  */
    if((cpu_id==0)&&(individual==0)&&(cluster_id==0))
	      save_cpuxgpt();
    ...
    if((cpu_id==0)&&(cluster_id==0)&&(individual==0))
    {
        reg_write(CA7_CCI400_DVM_EN, reg_read(CA7_CCI400_DVM_EN) & ~0x3);
        while (reg_read(CCI400_STATUS) & 0x1);
    }

    if (power_state[cluster_id][cpu_id] == STATUS_DORMANT) {
        /* disable L2 invalidate when reset */
        reg_write(CA7_CACHE_CONFIG, reg_read(CA7_CACHE_CONFIG) | (1U << 4));
    }

    //CA7MCU BUS_CONFIG[4]	Disable CA7 snoop function (Snoop interface is inactive and no longer accepting requests.), rg_acinactm	Prevent L2 memory from accepting any new request
    if((cpu_id==0)&&(cluster_id==0)&&(individual==0))
    reg_write(CA7_BUS_CONFIG, reg_read(CA7_BUS_CONFIG) | 0x10);

    
        dormant_ret_flag[cluster_id][cpu_id] = 0;
...
static void platform_restore_context(void)
{
    ...
    cpu_context *ns_cpu_ctx = (cpu_context *) __pa(&switcher_context.cluster.core[cpu_id+(cluster_id*4)].ns_cpu_ctx);    
    ...
    cpu_context *ns_cpu_ctx_v = &switcher_context.cluster.core[cpu_id+(cluster_id*4)].ns_cpu_ctx;
    ...
    //CA7MCU BUS_CONFIG[4]  rg_acinactm Prevent L2 memory from accepting any new request
    if(gic_pvt_context->gic_saved)
        reg_write(CA7_BUS_CONFIG_PHY, reg_read(CA7_BUS_CONFIG_PHY) & ~0x10);
    ...
    if(switcher_context.cluster.core[cpu_id+(cluster_id*4)].ns_cpu_ctx.gic_cpu_ctx.gic_saved==0)
    ...
    restore_pmu_context(cluster_id, cpu_id);
	  ...
	  /*
   	* Restore CPUXGPT register when musys power on again
	  */
    if(switcher_context.cluster.core[cpu_id+(cluster_id*4)].ns_cpu_ctx.gic_cpu_ctx.gic_saved)
	      restore_cpuxgpt();
    ...
    if((cpu_id==0)&&(cluster_id==0)&&(switcher_context.cluster.core[cpu_id+(cluster_id*4)].ns_cpu_ctx.gic_cpu_ctx.gic_saved))
    {
        reg_write(CA7_CCI400_DVM_EN, reg_read(CA7_CCI400_DVM_EN) | 0x3);
        while (reg_read(CCI400_STATUS) & 0x1);
    }
    ...
    if (power_state[cluster_id][cpu_id] == STATUS_DORMANT) {
        dormant_ret_flag[cluster_id][cpu_id] = 1;
    } else if (power_state[cluster_id][cpu_id] == STATUS_SHUTDOWN) {
        dormant_ret_flag[cluster_id][cpu_id] = 2;
    }

    power_state[cluster_id][cpu_id] = STATUS_RUN;


    restore_banked_registers(gp_context_v);    
}

int cpu_power_down(int mode,bool individual)
{
    unsigned cpu_id,clusterid;
    ...
    cpu_id = read_cpuid();
    clusterid= read_clusterid();

#if !defined(CONFIG_TRUSTONIC_TEE_SUPPORT)
    reg_write(BOOTROM_BOOT_ADDR, __pa(restore_ptr));
#else //#if !defined(CONFIG_TRUSTONIC_TEE_SUPPORT)
    /* Koshi : temply disable the secure call for single core boot-up */
    /* Koshi modified { */
    
    //mt_secure_call (MC_FC_SLEEP, __pa(restore_ptr), cpu_id, 0);
    mt_secure_call (MC_FC_GOTO_SLEEP, __pa(restore_ptr), cpu_id, 0);
    
    /* Koshi modified } */
#endif //#if !defined(CONFIG_TRUSTONIC_TEE_SUPPORT)

    power_state[clusterid][cpu_id] = mode;
    ...
    if (power_state[clusterid][cpu_id] == STATUS_DORMANT)         
    {
    ...
    platform_save_context(individual);

    return dormant_ret_flag[clusterid][cpu_id];
}
...
void cpu_check_dormant_abort(void)
{
    ...
    //CA7MCU BUS_CONFIG[4]  rg_acinactm
    reg_write(CA7_BUS_CONFIG, reg_read(CA7_BUS_CONFIG) & ~0x10);
    ...
    // To sync MCUSYS and L2C div (mt_cci400.c)
    mcu_div();

    if (power_state[cluster_id][cpu_id] != STATUS_RUN)
    {
        //unsigned cpu_id;
        unsigned cluster_id;
        cpu_context *ns_cpu_ctx;
        ...
        __enable_cache();
#if defined(CONFIG_TRUSTONIC_TEE_SUPPORT)
        printk("[Power/cpu_dormant]: abort before SMC [cpu_id == 0x%x]\n", cpu_id);
        if (cpu_id == 0)
        {
            printk("[Power/cpu_dormant]: MC_FC_SLEEP_CANCELLED \n");
        ...
        //cpu_id = read_cpuid();
        cluster_id = read_clusterid();
        ns_cpu_ctx = &switcher_context.cluster.core[cpu_id+(cluster_id*4)].ns_cpu_ctx;
    ...
    {
        if ((cpu_id == 0) && (cluster_id == 0) && (dormant_ret_flag[cluster_id][cpu_id] == 2))
        {
            ...
            {
#if defined (CONFIG_TRUSTONIC_TEE_SUPPORT)
                mt_secure_call(MC_FC_SET_RESET_VECTOR, virt_to_phys(cpu_wake_up_errata_802022), 4, 0);
#endif            
                spm_mtcmos_ctrl_cpu4(STA_POWER_ON, 1);

#if defined (CONFIG_TRUSTONIC_TEE_SUPPORT)
                mt_secure_call(MC_FC_SET_RESET_VECTOR, virt_to_phys(cpu_wake_up_errata_802022), 5, 0);
#endif 
                spm_mtcmos_ctrl_cpu5(STA_POWER_ON, 1);

#if defined (CONFIG_TRUSTONIC_TEE_SUPPORT)
                mt_secure_call(MC_FC_SET_RESET_VECTOR, virt_to_phys(cpu_wake_up_errata_802022), 6, 0);
#endif 
                spm_mtcmos_ctrl_cpu6(STA_POWER_ON, 1);

#if defined (CONFIG_TRUSTONIC_TEE_SUPPORT)
                mt_secure_call(MC_FC_SET_RESET_VECTOR, virt_to_phys(cpu_wake_up_errata_802022), 7, 0);
#endif 
...
void cpu_check_mcdi_dormant_abort(void)
{
    unsigned cpu_id,cluster_id;
    cpu_id = read_cpuid();
    cluster_id = read_clusterid();
...
MODULE_DESCRIPTION("Dormant/Shutdown Mode Driver $Revision: #1 $");
