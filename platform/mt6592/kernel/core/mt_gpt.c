...
struct mt_clock mt6592_gpt =
{
    ...
    .irq =
    {
        ...
        .handler    = gpt_handler,
        .dev_id     = &mt6592_gpt.clockevent,
        .irq        = MT6592_APARM_GPTTIMER_IRQ_LINE,
    },
...
{
    unsigned int cmp;
    struct clock_event_device *evt = &mt6592_gpt.clockevent;
    struct gpt_device *dev = id_to_dev(GPT_CLKEVT_ID);
...
{
    struct clocksource *cs = &mt6592_gpt.clocksource;
    struct gpt_device *dev = id_to_dev(GPT_CLKSRC_ID);
...
MODULE_DESCRIPTION("MT6592 GPT Driver v0.1");
MODULE_LICENSE("GPL");
