...
//#define CONFIG_TRUSTONIC_TEE_SUPPORT
#if 1
...
int check_overclock_Freq(void)
{
  int DLE;
  DLE=DRAMC_READ_REG(0x624);
  if(DLE==(0x47153504<<1))
  {
    return 293;
  }
  else if (DLE==(0x58ea0916<<1))
  {
    return 1466;
  }
  else if (DLE==(0x40a76276<<1))
  {
    return 266;
  }
  else if (DLE==(0x50d8fe7c<<1))
  {
    return 334;
  }
}
static ssize_t dram_overclock_show(struct device_driver *driver, char *buf)
{
      ...
      if(get_ddr_type)
      {
        ...
        return snprintf(buf, PAGE_SIZE, "%d,%d",dram_type, check_overclock_Freq());
      }
      else
      {
...
static ssize_t dram_overclock_store(struct device_driver *driver, const char *buf, size_t count)
{
    ...
    printk("[System Stability tool]clk %d\n",clk);
    if(clk==293)
    {
      DRAMC_WRITE_REG(0x47153504<<1,0x624);
      printk("[System Stability tool]293Mhz,0x%x \n",DRAMC_READ_REG(0x624));
      ret=1;
    }
    else if(clk==367)
    {
      DRAMC_WRITE_REG(0x58ea0916<<1,0x624);
      printk("[System Stability tool]367Mhz,0x%x \n",DRAMC_READ_REG(0x624));
      ret=1;
    }
    else if(clk==266)
    {
     DRAMC_WRITE_REG(0x40a76276<<1,0x624);
     printk("[System Stability tool]266Mhz,0x%x \n",DRAMC_READ_REG(0x624));
    }
    else if(clk==334)
    {
      DRAMC_WRITE_REG(0x50d8fe7c<<1,0x624);
      printk("[System Stability tool]334Mhz,0x%x \n",DRAMC_READ_REG(0x624));
    }
    else
    {
      ret=-1;
    ...
    printk( "In %s pass, dram_clk:%d, clk:%d\n", __func__, dram_clk, clk);
    return count;
...
static ssize_t DVS_test_show(struct device_driver *driver, char *buf)
{
    int ret;
    int rank0_fine;
    int rank1_fine;
    if(dfs_con.Freq_Status==EMULATED_GPU_HIGH_FREQ)
    {
      
        dfs_con.Freq_Status=EMULATED_GPU_LOW_FREQ;        
        rank0_fine=DRAMC_READ_REG(0x374);
        rank1_fine=DRAMC_READ_REG(0x378);
       if(dfs_con.V_Status==HV)
       {
       ...
       else
       {
        ...
        return snprintf(buf, PAGE_SIZE, "hopping to %d, rank0_fine=0x%x,rank1_fine=0x%x DVS_DMA_Times=%d\n",dfs_con.ddr_LF*4,rank0_fine,rank1_fine,Voltage_DMA_counter);
    }
    else
    { 
       ...
       else
       {
        ...
        dfs_con.Freq_Status=EMULATED_GPU_HIGH_FREQ;
        rank0_fine=DRAMC_READ_REG(0x374);
        rank1_fine=DRAMC_READ_REG(0x378);
        return snprintf(buf, PAGE_SIZE, "hopping to %d, rank0_fine=0x%x,rank1_fine=0x%x DVS_DMA_Times %d\n",dfs_con.ddr_HF*4,rank0_fine,rank1_fine,Voltage_DMA_counter);
    }
}
...
/*
   ...
   5:HVc_NVm
*/
static ssize_t DVS_test_store(struct device_driver *driver, const char *buf, size_t count)
{
    int status;
    status = simple_strtol(buf, 0, 10);
...
DRIVER_ATTR(emi_DVS_test, 0664, DVS_test_show, DVS_test_store);
...
int __init dram_overclock_init(void)
{
    ...
    ret = driver_create_file(&dram_overclock_drv, &driver_attr_emi_DVS_test);
    if (ret) {
        printk(KERN_ERR "fail to create the DFS sysfs files\n");
        return ret;
    }	
...
void DFS_Voltage_Configuration(void)
{
  ...
  if(dfs_status==3){
    ...
    dfs_con.vcore_HF_HV=Vcore_HV_1066+0x4; //Emulated GPU High Voltage
    dfs_con.vcore_HF_NV=Vcore_NV_1066+0x4; //Emulated GPU High Voltage
    dfs_con.vcore_HF_LV=Vcore_LV_1066+0x4; //Emulated GPU High Voltage
