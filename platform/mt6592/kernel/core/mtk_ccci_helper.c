...
static void cal_md_mem_usage(void)
{
	...
	if(meta_md_support[MD_SYS1]){
		...
		printk("[ccci/ctl] (0)set md%d type with meta value:%d\n",MD_SYS1, meta_md_support[MD_SYS1]);
...
int parse_meta_md_setting(unsigned char args[])
{
	...
	switch(active_id){
			...
			if(md_setting_flag&MD_WG_FLAG) {
				meta_md_support[MD_SYS1] = modem_wg;
			} else if(md_setting_flag&MD_TG_FLAG) {
				meta_md_support[MD_SYS1] = modem_tg;
			} else if(md_setting_flag&MD_2G_FLAG) {
				meta_md_support[MD_SYS1] = modem_2g;
			}
			if(meta_md_support[MD_SYS1])
			{
				md_support[MD_SYS1]=meta_md_support[MD_SYS1];
			}
			printk("[ccci/ctl] (0)Meta active id:1, md type:%d\n", meta_md_support[MD_SYS1]);
			break;
...
extern unsigned int get_devinfo_with_index(unsigned int  index);
	unsigned int  md1_spare = get_devinfo_with_index(3);
	unsigned int md1_disable=(md1_spare &(1<<22));
	...
	{
		printk("[ccci/ctl]No need power down\n");
	}
	return 0;
}
