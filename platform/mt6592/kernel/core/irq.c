...
#endif
        spin_lock_irqsave(&irq_lock, flags);
        mask->mask[0] = readl(IOMEM(GIC_ICDISER0));
...
int mt_PPI_mask_restore(struct mtk_irq_mask *mask)
{
    ...
    mt65xx_reg_sync_writel(mask->mask[0], GIC_ICDISER0);
...
int mt_SPI_mask_all(struct mtk_irq_mask *mask)
{
    ...
    if (mask) {
        ...
        for (i = 1; i < (NR_MT_IRQ_LINE/32); i++)
        {
            mask->mask[i] = readl(IOMEM(GIC_ICDISER0 + 4*i)); 
            writel(0xFFFFFFFF, IOMEM(GIC_ICDICER0 + 4*i));
        }
        spin_unlock_irqrestore(&irq_lock, flags);
#if defined(CONFIG_FIQ_GLUE)
        local_fiq_enable();
...
int mt_SPI_mask_restore(struct mtk_irq_mask *mask)
{
    ...
    for (i = 1; i < (NR_MT_IRQ_LINE/32); i++)
    {
        writel(mask->mask[i], IOMEM(GIC_ICDISER0 + 4*i));
    }
    spin_unlock_irqrestore(&irq_lock, flags);
#if defined(CONFIG_FIQ_GLUE)
    local_fiq_enable();
...
//for debug
    char str_buf[256];
    if ((irq == MT_WDT_IRQ_ID) || (irq == FIQ_SMP_CALL_SGI)) 
    {
