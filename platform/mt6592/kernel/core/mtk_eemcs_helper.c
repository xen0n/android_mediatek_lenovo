...
static unsigned int meta_ext_md_support[MAX_EXT_MD_NUM];
...
void apply_pre_md_run_setting(void)
{
	#if defined(GPIO_UART_UTXD3_PIN) && defined(GPIO_UART_URXD3_PIN) && defined(GPIO_UART_UTXD3_PIN_M_UTXD) && defined(GPIO_UART_URXD3_PIN_M_URXD)
	printk("[EEMCS/BOOT] Enable UART pin\n");
	...
	#else 
	printk("[EEMCS/BOOT] Enable UART pin dummy\n");
	#endif
}
EXPORT_SYMBOL(apply_pre_md_run_setting);
...
void apply_post_md_run_setting(void)
{
	...
	printk("[EEMCS/BOOT] Disable UART pin\n");
	...
	#else
	printk("[EEMCS/BOOT] Disable UART pin dummy\n");
	#endif
  ...
  switch(active_id) {
    case MD_SYS5:
		if (md_setting_flag == MD_LWG_FLAG){
			meta_ext_md_support[active_index] = modem_lwg;
			//ext_md_support[active_index] = modem_lwg;
		} else if (md_setting_flag == MD_LTG_FLAG){
			meta_ext_md_support[active_index] = modem_ltg;
			//ext_md_support[active_index] = modem_ltg;
		} else if (md_setting_flag == MD_SGLTE_FLAG){
			meta_ext_md_support[active_index] = modem_sglte;
		}
		if(meta_ext_md_support[active_index]) {
			ext_md_support[active_index] = meta_ext_md_support[active_index];
		}
		printk("[EEMCS/PLAT] META EXT MD type:%d\n", meta_ext_md_support[active_index]);
		break;
	}
	return 0;	
}
...
void get_ext_md_post_fix(int md_id, char buf[], char buf_ex[])
{
  ...
  if (md_id < MD_SYS5) {
	...
	switch(feature_val) {
  	case modem_lwg:
  		snprintf(YYY, 16, "_lwg_n");
  		break;
  	case modem_ltg:
  		snprintf(YYY, 16, "_ltg_n");
  		break;
	case modem_tg:
  		snprintf(YYY, 16, "_tg_n");
  		break;
  	case modem_sglte:
  		snprintf(YYY, 16, "_sglte_n");
  		break;
  	default:
...
unsigned int set_ext_modem_support(int md_id, int md_type){
    ...
    switch(md_id) {
  	case MD_SYS5:
        if (md_type >= modem_lwg &&  md_type <= modem_sglte){
...
extern void eemcs_set_reload_image(int need_reload);
