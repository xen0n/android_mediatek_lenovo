    ...
    if (CheckFormat(format) && CheckSampleRate(devices, sampleRate) && CheckChannel(devices, channels))
    {
        ...
        *status = NO_ERROR;
        ...
        else
        {
...
#ifdef EXTCODEC_ECHO_REFERENCE_SUPPORT  //for echo ref 
            //DLSA.mChannels = 2;
            //DLSA.mSampleRate = 44100;
            //DLSA.mBufferSize = 0xDC8;
            ...
            //DLSA.mBufferSize = mStreamInManager->mEchoRefAWBBufferSize;
            DLSA.mBufferSize = ((DLSA.mSampleRate * DLSA.mChannels * 2 * 20) / 1000);
            mStreamInManager->mEchoRefAWBBufferSize = DLSA.mBufferSize;            
#else
            DLSA.mChannels = mAudioSpeechEnhanceInfoInstance->GetOutputChannelInfo();
            DLSA.mSampleRate = mAudioSpeechEnhanceInfoInstance->GetOutputSampleRateInfo();
    ...
    if (mStarting == false)
    {
        ...
        switch (mAudioResourceManager->GetAudioMode())
        {
            ...
            case AUDIO_MODE_IN_COMMUNICATION:
            {
                ...
                if ((mAttribute.mSource == AUDIO_SOURCE_VOICE_RECOGNITION) || mUsingMagiASR || mUsingAECRec || mVoIPRunning || mHDRecTunning16K)
                {
                    ...
                    if (process_sample_rate != num_sample_rate)
                    {
                        ...
                        // Need SRC 48k->16k
                        mBliHandler1 = new MtkAudioSrc(num_sample_rate, num_channel,
                                                       process_sample_rate, num_channel, SRC_IN_Q1P15_OUT_Q1P15);
                        mBliHandler1->Open();
                        ...
                        ALOGD("mAttribute.mBufferSize=%d", mAttribute.mBufferSize);
                        // Need SRC 16k->48k
                        mBliHandler2 = new MtkAudioSrc(process_sample_rate, num_channel,
                                                       num_sample_rate, num_channel, SRC_IN_Q1P15_OUT_Q1P15);
                        mBliHandler2->Open();
            ...
            case AUDIO_MODE_IN_CALL_EXTERNAL:
            {
                ...
                {
                    mStreamInClient->mBliSrc  = new MtkAudioSrc(
                        modem_num_sample_rate, modem_num_channel,
                        target_num_sample_rate, target_num_channel,
                        SRC_IN_Q1P15_OUT_Q1P15);
                    mStreamInClient->mBliSrc->Open();
                }
    ...
    if (mStarting == true)
    {
        ...
        switch (mAudioResourceManager->GetAudioMode())
        {
            ...
            case AUDIO_MODE_IN_CALL_EXTERNAL:
            {
                ...
                {
                    mStreamInClient->mBliSrc->Close();
                    delete mStreamInClient->mBliSrc;
                    mStreamInClient->mBliSrc = NULL;
                }
                mStreamInManager->ReleaseClient(mStreamInClient);
                mStreamInClient = NULL;//assign null value;
                break;
...
status_t  AudioMTKStreamIn::BliSrc::init(uint32 inSamplerate, uint32 inChannel, uint32 OutSamplerate, uint32 OutChannel)
{
    if (mHandle == NULL)
    {
        ALOGD("BliSrc::init InputSampleRate=%u, inChannel=%u, OutputSampleRate=%u, OutChannel=%u",
              inSamplerate, inChannel, OutSamplerate, OutChannel);
...
size_t AudioMTKStreamIn::BliSrc::process(const void *inbuffer, size_t *inBytes, void *outbuffer, size_t *outBytes)
{
    if (mHandle)
    {
        size_t consume = *inBytes;
        mHandle->Process((short *)inbuffer, inBytes, (short *)outbuffer, outBytes);
        consume -= *inBytes;
...
uint32_t AudioMTKStreamIn::HDRecordPreprocess(void *buffer , uint32_t bytes, AdditionalInfo_STRUCT AddInfo)
{
    if (mAttribute.mSource != AUDIO_SOURCE_FM && mAttribute.mSource != AUDIO_SOURCE_MATV)
    {
        ...
        if (mBliHandler1 != 0 && mBliHandler2 != 0)
        {
            ...
            consume = inputLength;
            mBliHandler1->Process((int16_t *)buffer, &inputLength, (int16_t *)mBliHandler1Buffer, &outputLength);
            consume -= inputLength;
            ...
            consume = outputLength;
            mBliHandler2->Process((int16_t *)mBliHandler1Buffer, &outputLength, (int16_t *)buffer, &bytes);
            consume -= outputLength;
            return bytes;
        }
        else
...
void AudioMTKStreamIn::NeedVoIPResync(void)
{
    //Mutex::Autolock _l(mLock);
    ALOGD("NeedVoIPResync %d",mVoIPRunning);
    if (mVoIPRunning)
    {
        if (IsHDRecordRunning())
        {
