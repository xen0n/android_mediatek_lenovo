...
status_t AudioFMResourceManager::SetFmSourceModuleEnable(const bool enable)
{
    ...
    else // Enable 2nd I2S
    {
        if (enable == true)
        {
            ...
            m2ndI2SInAttribute.mI2S_IN_PAD_SEL = audio_i2s_pad_sel;
            ...
            mAudioDigitalInstance->Set2ndI2SIn(&m2ndI2SInAttribute);
...
#ifdef USING_TFA9887_EXTAMP // add for fm issue alps01373308
    if (enable == true)
    {
        // open dac i2s
        ...
        // open 2nd i2s out
        mAudioDigitalInstance->Set2ndI2SOutAttribute(GetFmDownlinkSamplingRate());
...
status_t AudioFMResourceManager::SetFmDirectConnection(const bool enable)
{
    ...
    if (enable == true)
    {
        ...
        // Set DAC I2S Out
#ifndef USING_TFA9887_EXTAMP

#ifdef HP_USING_EXTDAC || MTK_AUDIO_EXTCODEC_SUPPORT
        mAudioDigitalInstance->SetI2SDacOutAttribute(GetFmDownlinkSamplingRate());
        mAudioDigitalInstance->SetI2SDacEnable(true);        
#else
        if (AudioMTKStreamManager::getInstance()->IsOutPutStreamActive() == false)
    ...
    else
    {
...
#ifdef USING_TFA9887_EXTAMP // add for fm issue alps01373308
        mAudioDigitalInstance->SetinputConnection(AudioDigitalType::Connection, AudioDigitalType::I05, AudioDigitalType::O03);
        mAudioDigitalInstance->SetinputConnection(AudioDigitalType::Connection, AudioDigitalType::I06, AudioDigitalType::O04);
#endif
        ...
        // Set DAC I2S Out
#ifndef USING_TFA9887_EXTAMP // add for fm issue alps01373308
        if (AudioMTKStreamManager::getInstance()->IsOutPutStreamActive() == false)
        {
            mAudioDigitalInstance->SetI2SDacEnable(false);
        }
