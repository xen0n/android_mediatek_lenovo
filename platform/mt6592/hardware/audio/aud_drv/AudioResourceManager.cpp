...
bool AudioResourceManager::mExtDACIsOpen = false;
...
AudioAnalogControlExt *AudioResourceManager::mAudioAnalogExtInstance = NULL;
...
status_t AudioResourceManager::TurnonAudioDevice(unsigned int mDlOutputDevice)
{
    ...
    if (DeviceCount == 1)
    {
        ...
        {
#if defined(NXP_SMARTPA_SUPPORT)
            //           ::ioctl(mFd, SET_GPIO_CURRENT, 3);
            //           ALOGD("SET_GPIO_CURRENT, outputdevice= %d, devicecout= %d",mDlOutputDevice,DeviceCount);
            if (::ioctl(mFd, AUDDRV_GET_IPO_EVER) == true)
            {
                ::ioctl(mFd, AUDDRV_CLEAR_IPO_EVER);
                MTK_Tfa98xx_Reset();
            }
            MTK_Tfa98xx_SetSampleRate(44100);
            MTK_Tfa98xx_SpeakerOn();
#elif defined(USING_TFA9887_EXTAMP)
            tfa9887_setSamplerate(44100);
            tfa9887_SpeakerOn();
#else
        ...
        {
#ifdef MTK_AUDIO_EXTCODEC_SUPPORT
#ifdef HIFIDAC_SWITCH
            if (AudioMTKStreamManager::getInstance()->GetHiFiDACStatus() == false) //HP switch
            {
            ...
            {
#ifdef HP_USING_EXTDAC
                mAudioAnalogExtInstance->AnalogOpen(AudioAnalogType::DEVICE_OUT_HEADSETR, AudioAnalogType::DEVICE_PLATFORM_MACHINE);
                mExtDACIsOpen = true;
#ifdef EXTDAC_PMIC_MUTE //HP switch
                mAudioAnalogInstance->AnalogOpen(AudioAnalogType::DEVICE_OUT_HEADSETR, AudioAnalogType::DEVICE_PLATFORM_MACHINE);
#endif
#endif
            }
...
#ifdef MTK_AUDIO_EXTCODEC_SUPPORT
#ifdef RINGTONE_USE_PMIC //HP switch
...
status_t AudioResourceManager::TurnonAudioDeviceIncall(unsigned int mDlOutputDevice)
{
    ...
    SetMicInvserse(mMicInverseSetting);
    uint32 DeviceCount = PopCount(mDlOutputDevice);
    ALOGD("TurnonAudioDeviceIncall = 0x%x DeviceCount = %d", mDlOutputDevice, DeviceCount);
    if (DeviceCount == 1)
    {
        if (mDlOutputDevice & AUDIO_DEVICE_OUT_EARPIECE)
        {
        ...
        if (mDlOutputDevice & AUDIO_DEVICE_OUT_SPEAKER)
        {
            ...
            MTK_Tfa98xx_SetSampleRate(16000);
            MTK_Tfa98xx_SpeakerOn();
            MTK_Tfa98xx_EchoReferenceConfigure(1);
#elif defined(USING_TFA9887_EXTAMP)
            tfa9887_setSamplerate(16000);
            tfa9887_SpeakerOn();
#else
...
#ifdef MTK_AUDIO_EXTCODEC_SUPPORT
#ifdef HIFIDAC_SWITCH
            if (1) //HP switch, phone call always use internal DAC
            {
                mAudioAnalogInstance->AnalogOpen(AudioAnalogType::DEVICE_OUT_HEADSETR, AudioAnalogType::DEVICE_PLATFORM_MACHINE);
            }
            else
    ...
    else
    {
        if (mDlOutputDevice & AUDIO_DEVICE_OUT_SPEAKER)
        {
#if defined(NXP_SMARTPA_SUPPORT)//            ::ioctl(mFd, SET_GPIO_CURRENT, 3);
            //            ALOGD("SET_GPIO_CURRENT, outputdevice= %d, devicecout= %d",mDlOutputDevice,DeviceCount);
            if (::ioctl(mFd, AUDDRV_GET_IPO_EVER) == true)
            {
                ::ioctl(mFd, AUDDRV_CLEAR_IPO_EVER);
            ...
            MTK_Tfa98xx_EchoReferenceConfigure(1);
#elif defined(USING_TFA9887_EXTAMP)
            tfa9887_setSamplerate(16000);  //44100
            tfa9887_SpeakerOn();
#endif
...
status_t AudioResourceManager::TurnoffAudioDevice(unsigned int mDlOutputDevice)
{
    ...
    if (DeviceCount == 1)
    {
        ...
        if ((mDlOutputDevice & AUDIO_DEVICE_OUT_WIRED_HEADSET) || (mDlOutputDevice & AUDIO_DEVICE_OUT_WIRED_HEADPHONE))
        {
            ...
            {
#ifdef HP_USING_EXTDAC
                mAudioAnalogExtInstance->AnalogClose(AudioAnalogType::DEVICE_OUT_HEADSETR, AudioAnalogType::DEVICE_PLATFORM_MACHINE);
                mExtDACIsOpen = false;
#ifdef EXTDAC_PMIC_MUTE //HP switch
                mAudioAnalogInstance->AnalogClose(AudioAnalogType::DEVICE_OUT_HEADSETR, AudioAnalogType::DEVICE_PLATFORM_MACHINE);
#endif
...
status_t AudioResourceManager::AddSubSPKToOutputDevice()
{
    if (mDlOutputDevice & AUDIO_DEVICE_OUT_SPEAKER)
    {
        ...
        if (AudioMTKStreamManager::getInstance()->GetHiFiDACStatus() == false) //HP switch
        {
            mAudioAnalogInstance->AnalogOpenForAddSPK(AudioAnalogType::DEVICE_OUT_SPEAKERR, AudioAnalogType::DEVICE_PLATFORM_MACHINE);    //Temp remove for build error
        }
        else
#endif
        {
    ...
    else
    {
        ...
        if (mExtDACIsOpen == false)
        {
            mAudioAnalogInstance->AnalogCloseForSubSPK(AudioAnalogType::DEVICE_OUT_SPEAKERR, AudioAnalogType::DEVICE_PLATFORM_MACHINE);//Temp remove for build error
        }
        else
        {
#ifdef HP_USING_EXTDAC
