...
AudioMTKStreamInManager *AudioMTKStreamInManager::getInstance()
{
    ...
    ALOGV("AudioMTKStreamInManager getInstance()");
    ASSERT(UniqueStreamInManagerInstance != NULL);
...
AudioMTKStreamInManager::AudioMTKStreamInManager()
{
    ...
    mAWBforEcho = false;
    mShortIntrforEchoRef = false;
    mAudioSpeechEnhanceInfoInstance = AudioSpeechEnhanceInfo::getInstance();
    ASSERT(mAudioSpeechEnhanceInfoInstance != NULL);
    mBliHandlerEchoRef = NULL;
    mBliOutputBufferEchoRef = NULL;
    mFd = 0;
    mEchoRefAWBBufferSize = 0xDC8; //44100K, 20ms data size
#endif
...
#ifdef EXTCODEC_ECHO_REFERENCE_SUPPORT
void AudioMTKStreamInManager::SetULIntrCountSwitch(bool state)
{
    if (mFd == 0)
    {
        ALOGD("AudioMTKStreamInManager open kAudioDeviceName");
    ...
    if (mFd != 0)
    {
        ALOGD("AudioMTKStreamInManager SetULIntrCountSwitch AUDDRV_ECHOREF_MODE %d", state);
        ::ioctl(mFd, AUDDRV_ECHOREF_MODE, state);
    }
}

uint32_t AudioMTKStreamInManager::WriteToReference(void *buffer , uint32 copy_size, AdditionalInfo_STRUCT AddInfo)
{
    struct InBufferInfo ReferenceBufinfo;
    ReferenceBufinfo.pBufBase = (short *)buffer;
    ReferenceBufinfo.BufLen = copy_size;
    ReferenceBufinfo.time_stamp_queued = GetSystemTime(false);
    ReferenceBufinfo.bHasRemainInfo = AddInfo.bHasAdditionalInfo;
    ReferenceBufinfo.time_stamp_predict = AddInfo.timestamp_info;
    if (mAudioSpeechEnhanceInfoInstance == NULL)
    {
        mAudioSpeechEnhanceInfoInstance = AudioSpeechEnhanceInfo::getInstance();
        ASSERT(mAudioSpeechEnhanceInfoInstance != NULL);
    }
    mAudioSpeechEnhanceInfoInstance->WriteReferenceBuffer(&ReferenceBufinfo);
    return copy_size;
}

uint32_t AudioMTKStreamInManager::CopyBufferToReference(uint32 mMemDataType, void *buffer , uint32 copy_size, AdditionalInfo_STRUCT AddInfo)
{
    /*
        ALOGD("CopyBufferToReference mMemDataType = %d buffer = %p copy_size = %d mAudioInput.size() = %d buffer = 0x%x",
             mMemDataType, buffer, copy_size, mAudioInput.size(), *(unsigned int *)buffer);
    */
#if 1   //need to use new audiolock to prevent the uplink queue block with the reference queue
    int ret = 0;
    ret = mAudioResourceManager->EnableAudioLock(AudioResourceManagerInterface::AUDIO_ECHOREFERENCECLIENT_LOCK, 1000);
    if (ret)
    {
        ALOGW("EnableAudioLock AUDIO_ECHOREFERENCECLIENT_LOCK fail ret = %d", ret);
        return 0;
    }
#endif
    ...
    mAudioSpeechEnhanceInfoInstance->WriteReferenceBuffer(&ReferenceBufinfo);   //copy to all SPELayer
    ...
    {
        ALOGW("CopyBufferToReference, process too long? sec %ld nsec %ld, sec %ld nsec %ld", EnterTime.tv_sec, EnterTime.tv_nsec, Finishtime.tv_sec, Finishtime.tv_nsec);
    }
    //---
...
uint32_t AudioMTKStreamInManager::CopyBufferToClient(uint32 mMemDataType, void *buffer , uint32 copy_size, AdditionalInfo_STRUCT AddInfo)
{
    ...
    for (int i = 0 ; i < mAudioInput.size() ; i ++)
    {
        ...
        {
#ifdef EXTCODEC_ECHO_REFERENCE_SUPPORT
            if ((temp->mMemDataType == AudioDigitalType::MEM_AWB) && (temp->mAttributeClient->mEchoRefUse == true))
            {
                //echo reference data, no need to do uplink process and copy to client
                ALOGD("CopyBufferToClient no need to do uplink process and copy to client");
            }
            else
#endif
            {
...
uint32_t AudioMTKStreamInManager::CopyBufferToClientIncall(RingBuf ul_ring_buf)
{
    ...
    for (size_t i = 0; i < mAudioInput.size(); ++i) // copy data to all clients
    {
        ...
        if (pClient->mEnable)
        {
            ...
            if (pClient->mBliSrc == NULL) // No need SRC
            {
            ...
            else // Need SRC
            {
                ...
                uint32_t consumed=num_modem_left_data;
                pClient->mBliSrc->Process((int16_t *)p_read, &num_modem_left_data,
                                      (int16_t *)pClient->mBliOutputLinearBuffer, &num_converted_data);
                consumed-=num_modem_left_data;
...
#ifdef EXTCODEC_ECHO_REFERENCE_SUPPORT
int AudioMTKStreamInManager::checkAWBEchoInUse(AudioMTKStreamInClient *Client)
{
    ALOGD("checkAWBEchoInUse");
    int count = 0;
    for (int i = 0; i < mAudioInput.size(); i++)
    {
        AudioMTKStreamInClient *mTemp = mAudioInput.valueAt(i);
        // if mem is the same  and other client is enable , measn this mem is alreay on
        if ((mTemp->mMemDataType == AudioDigitalType::MEM_AWB) && (mTemp->mAttributeClient->mEchoRefUse == true))
        {
            ALOGD("checkAWBEchoInUse in use Client mTemp = %p", mTemp);
            count++;
            //mAWBforEcho = true;
            //return true;
        }
...
#ifdef EXTCODEC_ECHO_REFERENCE_SUPPORT 
    uint32 RecordAWBBufferSize = mEchoRefAWBBufferSize;
#endif
    switch (mMemDataType)
    {
...
#ifdef EXTCODEC_ECHO_REFERENCE_SUPPORT
            ALOGD("StartStreamInThread RecordAWBBufferSize = %d", RecordAWBBufferSize);
            mAWBThread = new AudioMTkRecordThread(this, mMemDataType , mAWBbuffer, RecordAWBBufferSize);
#else
...
{
    ALOGD("+Do_input_standby Client = %p, mAudioInput.size()=%d", Client, mAudioInput.size());
        ...
        case AUDIO_MODE_IN_COMMUNICATION:   // fix me, is that mode in communication needs to be care more??
        {
            ...
            switch (Client->mMemDataType)
            {
                ...
                case AudioDigitalType::MEM_AWB:
                    ...
                    ALOGD("Do_input_standby mAWBThread exit");
                    break;
                default:
...
#ifdef EXTCODEC_ECHO_REFERENCE_SUPPORT
            else    //if there still have  other record, but not echo ref
            {
                if ((Client->mAttributeClient->mEchoRefUse) && (checkAWBEchoInUse() <= 1))
                {
                    ALOGD("Do_input_standby still have  other record, but not echo ref");
                    //set back the intr
                    if (mShortIntrforEchoRef == true)
                    {
                        mShortIntrforEchoRef = false;
                        ALOGD("Do_input_standby SetIrqMcuSampleRate , set back to 50ms for normal case");
                        mAudioDigital->SetIrqMcuSampleRate(AudioDigitalType::IRQ2_MCU_MODE, 16000);
                        mAudioDigital->SetIrqMcuCounter(AudioDigitalType::IRQ2_MCU_MODE, 800); // 50ms 800
                        //::ioctl(mFd, AUDDRV_ECHOREF_MODE, mShortIntrforEchoRef);
                        SetULIntrCountSwitch(mShortIntrforEchoRef);
                    }
                }
            }
#endif
            ...
            if (checkSourceInUse(Client) == false)
            {
                switch (Client->mSourceType)
                {
                    ...
                    case AudioDigitalType::MEM_DL1: //echo reference disconnect
                        AudioIn1 = AudioDigitalType::I05;
                        AudioIn2 = AudioDigitalType::I06;
                        break;
#endif
                    default:
                        break;
    ...
    if ((Client->mAttributeClient->mEchoRefUse) && (checkAWBEchoInUse() <= 1))
    {
        if (mBliHandlerEchoRef)
        {
            mBliHandlerEchoRef->Close();
            delete mBliHandlerEchoRef;
            mBliHandlerEchoRef = NULL;
            ALOGD("-Do_input_standby delete mBliHandlerEchoRef");
        }
        if (mBliOutputBufferEchoRef)
        {
            delete[] mBliOutputBufferEchoRef;
            mBliOutputBufferEchoRef = NULL;
            ALOGD("-Do_input_standby delete mBliOutputBufferEchoRef");
        }
    }
#endif
        ...
        case AUDIO_MODE_IN_COMMUNICATION:
        {
                ...
                case AudioDigitalType::I2S_IN_ADC:
                    ...
                    if (Client->mAttributeClient->mIsDigitalMIC) //for digital MIC 32K constrain
                    {
                        if (mBliHandlerDMIC == NULL)
                        {
                            ...
                            mBliHandlerDMIC = new MtkAudioSrc(getMemVULSamplerate(), 2, 48000, 2, SRC_IN_Q1P15_OUT_Q1P15);
                            mBliHandlerDMIC->Open();
                    ...
                    break;
#ifdef EXTCODEC_ECHO_REFERENCE_SUPPORT
                case AudioDigitalType::MEM_DL1: //use for echo reference from DL1 to awb
                    AudioIn1 = AudioDigitalType::I05;
                    AudioIn2 = AudioDigitalType::I06;
                    break;
#endif
...
#ifdef EXTCODEC_ECHO_REFERENCE_SUPPORT
                    if (Client->mAttributeClient->mEchoRefUse)
                    {
                        ALOGD("Do_input_start need record for echo reference");
                        //need config SRC for 44.1K -> 16K for echo reference
                        
                        #if defined(NXP_SMARTPA_SUPPORT)
                        ALOGD("Do_input_start MTK_Tfa98xx_EchoReferenceConfigure");
                        ...
                        ALOGD("Do_input_start I2S_LOW_JITTER_MODE");
                        m2ndI2SInAttribute.mI2S_WLEN = AudioDigtalI2S::WLEN_32BITS;
                        m2ndI2SInAttribute.mI2S_HD_EN = AudioDigtalI2S::LOW_JITTER_CLOCK;
                        #else
                        m2ndI2SInAttribute.mI2S_WLEN = AudioDigtalI2S::WLEN_16BITS;
                        m2ndI2SInAttribute.mI2S_HD_EN = AudioDigtalI2S::NORMAL_CLOCK;
                        #endif

                        ALOGD("Do_input_start SetI2SSoftReset");
                        mAudioDigital->SetI2SSoftReset(true);   //set 4-wire I2S mode
                        ...
                        mAudioDigital->SetI2SSoftReset(false);  //finish 4-wire I2S mode
                    }
                    else
#endif
                    {
            ...
            switch (Client->mMemDataType)
            {
                ...
                case AudioDigitalType::MEM_AWB:
                    ...
                    if (Client->mAttributeClient->mEchoRefUse)
                    {
                        ...
                        if (mBliHandlerEchoRef == NULL)
                        {
                            if (Client->mSourceType ==  AudioDigitalType::MEM_DL1)
                            {
                                //source sample rate from streamout config
                                mBliHandlerEchoRef = new MtkAudioSrc(Client->mAttributeClient->mSampleRate , 2, 16000, 1, SRC_IN_Q1P15_OUT_Q1P15);
                            }
                            else if (Client->mSourceType ==  AudioDigitalType::I2S_IN_2)
                            {
                                //source sample rate from I0/I1
                                mBliHandlerEchoRef = new MtkAudioSrc(Client->mAttributeClient->mSampleRate , 2, 16000, 1, SRC_IN_Q1P15_OUT_Q1P15);
                            }
                            else
                            {
                                mBliHandlerEchoRef = new MtkAudioSrc(getMemVULSamplerate(), 2, 16000, 1, SRC_IN_Q1P15_OUT_Q1P15);
                                ALOGD("Do_input_start not expect behavior, something wrong?");
                            }

                            mBliHandlerEchoRef->Open();
                            mBliOutputBufferEchoRef = new char[BliOutBufferSizeEchoRef]; // tmp buffer for blisrc out
                            ASSERT(mBliOutputBufferEchoRef != NULL);
                            ALOGD("Do_input_start create mBliHandlerEchoRef");

                        }
                        else
                        {
                            ALOGD("Do_input_start mBliHandlerEchoRef already exist!!");
                        }
                    }
#endif
                    break;
            ...
            if (IrqStatus.mStatus == false)
            {
                ...
                if (WCNChipController::GetInstance()->BTUseCVSDRemoval() != true)
                {
                    ...
                    {
                        ALOGD("Do_input_start SetIrqMcuSampleRate = 16000, SetIrqMcuCounter=800 ");
                        mAudioDigital->SetIrqMcuSampleRate(AudioDigitalType::IRQ2_MCU_MODE, 16000); //48000
                        mAudioDigital->SetIrqMcuCounter(AudioDigitalType::IRQ2_MCU_MODE, 800); // 50ms  //2400, 960 = 20ms
#ifdef EXTCODEC_ECHO_REFERENCE_SUPPORT
                        //::ioctl(mFd, AUDDRV_ECHOREF_MODE, mShortIntrforEchoRef);
                        SetULIntrCountSwitch(mShortIntrforEchoRef);
#endif
                ...
                else
                {
                    ...
                    if (Client->mMemDataType != AudioDigitalType::MEM_DAI)
                    {
                        ...
                        if (Client->mAttributeClient->mEchoRefUse)
                        {
                            ALOGD("Do_input_start SetIrqMcuSampleRate , set 20ms for echoref");
                            mShortIntrforEchoRef = true;
                            mAudioDigital->SetIrqMcuSampleRate(AudioDigitalType::IRQ2_MCU_MODE, 16000);
                            mAudioDigital->SetIrqMcuCounter(AudioDigitalType::IRQ2_MCU_MODE, 320); // 20ms 320
                            //::ioctl(mFd, AUDDRV_ECHOREF_MODE, mShortIntrforEchoRef);
                            SetULIntrCountSwitch(mShortIntrforEchoRef);
                            mAudioDigital->SetIrqMcuEnable(AudioDigitalType::IRQ2_MCU_MODE, true);
                        }
...
#endif
                        {
                            ALOGD("Do_input_start SetIrqMcuSampleRate , set 50ms for normal case");
                            mAudioDigital->SetIrqMcuSampleRate(AudioDigitalType::IRQ2_MCU_MODE, 16000);
            ...
            {
#ifdef EXTCODEC_ECHO_REFERENCE_SUPPORT
                ALOGD("IRQ2_MCU_MODE is enabled, check if need reconfig");
                if ((mShortIntrforEchoRef == false) && (Client->mAttributeClient->mEchoRefUse)) //need to shorten the interrupt
                {
                    ALOGD("Do_input_start SetIrqMcuSampleRate reconfig for echoref");
                    mShortIntrforEchoRef = true;
                    mAudioDigital->SetIrqMcuSampleRate(AudioDigitalType::IRQ2_MCU_MODE, 16000);
                    mAudioDigital->SetIrqMcuCounter(AudioDigitalType::IRQ2_MCU_MODE, 320); // 20ms 320
                    //::ioctl(mFd, AUDDRV_ECHOREF_MODE, mShortIntrforEchoRef);
            ...
            {
                if (Client->mMemDataType == AudioDigitalType::MEM_AWB)
                {
                    ALOGD("AWB case do the inter connection modification");
                    mAudioDigital->SetinputConnection(AudioDigitalType::Connection, AudioIn1, AudioOut1);
                    mAudioDigital->SetinputConnection(AudioDigitalType::Connection, AudioIn2, AudioOut2);
                }
            }
...
#ifdef EXTCODEC_ECHO_REFERENCE_SUPPORT
    if (Mem_type == AudioDigitalType::MEM_AWB)
    {
        sched_p.sched_priority = RTPM_PRIO_AUDIO_RECORD + 5;
        ALOGD("priority for echo ref");
    }
#endif
...
bool AudioMTKStreamInManager::AudioMTkRecordThread::threadLoop()
{
    ...
    while (!(exitPending() == true))
    {
        ...
        else
        {
            ...
            if ((IsAudioSupportFeature(AUDIO_SUPPORT_DMIC)) && (mMemType == AudioDigitalType::MEM_VUL))
            {
                uint32 consume = 0;
                ...
                if (mManager->mBliHandlerDMIC)
                {
                    ...
                    consume = inputLength;
                    mManager->mBliHandlerDMIC->Process((int16_t *)mRingBuffer, &inputLength, (int16_t *)mManager->mBliOutputBufferDMIC, &outputLength);
                    consume-=inputLength;
            ...
            {
#ifdef EXTCODEC_ECHO_REFERENCE_SUPPORT
                //this thread loop is AWB with echo reference, no need to copy buffer to client
                //ALOGD("AudioMTkRecordThread threadloop active,mMemType=%d",mMemType);
                if ((mMemType == AudioDigitalType::MEM_AWB) && (mManager->mAWBforEcho == true))
                {
                    uint32 consume = 0;
                    size_t inputLength = mBufferSize;
                    size_t outputLength = BliOutBufferSizeEchoRef;

                    if (mManager->mBliHandlerEchoRef)   //do blisrc from 44.1K to 16K for echo reference using
                    {
                        consume = inputLength;
                        mManager->mBliHandlerEchoRef->Process((int16_t *)mRingBuffer, &inputLength, (int16_t *)mManager->mBliOutputBufferEchoRef, &outputLength);
                        consume -= inputLength;

                        mManager->CopyBufferToReference(mMemType, (void *)mManager->mBliOutputBufferEchoRef, outputLength, tempAddInfo);
                    }
                    else
                    {
                        mManager->CopyBufferToReference(mMemType, (void *)mRingBuffer, mBufferSize, tempAddInfo);
                    }
                }
                else
                {
...
void AudioMTKStreamInManager::AudioMTkRecordThread::btsco_cvsd_RX_main(void)
{
    ...
    do
    {
        ...
        if (mManager->mBliHandlerDAIBT)
        {
            ...
            consume = inputLength;
            mManager->mBliHandlerDAIBT->Process((int16_t *)outbuf, &inputLength, (int16_t *)mManager->mBliOutputBufferDAIBT, &outputLength);
            consume-=inputLength;
