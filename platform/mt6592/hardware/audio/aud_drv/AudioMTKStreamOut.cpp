...
AudioMTKStreamOut::AudioMTKStreamOut(uint32_t devices, int *format, uint32_t *channels, uint32_t *sampleRate, status_t *status)
{
    ...
    *status = NO_ERROR;
    ...
    setLowLatencyMode(mLowLatencyMode,true);
...
uint32_t AudioMTKStreamOut::calInterrupttime()
{
    ...
    mHwBufferSize = mAudioDigitalControl->GetMemBufferSize(AudioDigitalType::MEM_DL1);
    buffersize = mLowLatencyMode ? mDL1Attribute->mBufferSize : (mHwBufferSize * 3) / 4 ;
...
#if (defined(MTK_AUDIO_EXTCODEC_SUPPORT) || defined(I2S_LOW_JITTER_MODE))
...
status_t AudioMTKStreamOut::Set2ndI2SOutAttribute()
{
    ...
    m2ndI2SOut->mI2S_WLEN = AudioDigtalI2S::WLEN_32BITS;
    m2ndI2SOut->mI2S_HD_EN = AudioDigtalI2S::LOW_JITTER_CLOCK;
#else
    m2ndI2SOut->mI2S_WLEN = AudioDigtalI2S::WLEN_16BITS;
    m2ndI2SOut->mI2S_HD_EN = AudioDigtalI2S::NORMAL_CLOCK;
#endif
...
/*lenovo-sw fuxm1 add for new feature leaveway mode 20140522*/
	     if (!isLeavewayMode) 
                pBGSPlayer->Open(pSpeechDriver, 0x0, 0xFF);
           else
                pBGSPlayer->Open(pSpeechDriver, 0xFF, 0xFF);
/*lenovo-sw fuxm1  add end 20140522*/		 
        ...
        {            
            ...
            WrittenBytes *= 2;  //restore to 32bits unit
    ...
    else    //other modes, write data to AudioHW/BT
    {
        ...
        if (needTurnOnDevice)   //first write, bt/audiohw common part
        {
            ...
            //Todo: the echo reference start time need to provide by new echo reference record thread
#else
    ...
    ALOGD("AudioMTKStreamOut::write (-) %d latency_in_s,%1.6lf,%1.6lf,%1.6lf", WrittenBytes, time[0], time[1], time[2]);
    if (isDebugMode)
    {
        if (time[0] > 0.0045)
        {
...
status_t AudioMTKStreamOut::TurnOnAfeDigital(uint32 AfeDigital)
{
    ...
    switch (AfeDigital)
    {
        ...
        case (AudioDigitalType::I2S_OUT_2):
        {
            ...
            SetI2SOutDACAttribute();
            SetI2SDACOut(true);
            ALOGD("Turn on default I2S out DAC part for NXP");
#endif
            // turn on 2nd_I2S out
...
status_t AudioMTKStreamOut::TurnOffAfeDigital(uint32 AfeDigital, bool keepDacOpen)
{
    ...
    switch (AfeDigital)
    {
        ...
        case (AudioDigitalType::I2S_OUT_2):
        {
            ...
            SetI2SDACOut(false);
            ALOGD("Turn off default I2S out DAC part for NXP");
#endif
            break;
        }
        case (AudioDigitalType::I2S_OUT_DAC):
...
void AudioMTKStreamOut::dokeyRouting(uint32_t new_device)
{
    ...
    const uint32_t current_device = mDL1Attribute->mdevices = mAudioResourceManager->getDlOutputDevice();
...
/*lenovo-sw fuxm1 add new feature leaveway mode 20140522*/
    if (param.getInt(String8("SET_LEAVEAWAY_MODE_ENABLE"), value) == NO_ERROR)
    {
	  ALOGD("%s(), SET_LEAVEAWAY_MODE_ENABLE = %d.", __FUNCTION__,value);
	   
        if(value) {
	      ALOGD("chris check:: leaveway mode is on !!!!!!");
             isLeavewayMode = true;
	  } else {
	      ALOGD("chris check:: leaveway mode is off  !!!!!!");	 
	 	isLeavewayMode = false;
	  }
    }
/*lenovo-sw fuxm1 add end 20140522*/
...
int AudioMTKStreamOut::get_playback_delay(size_t frames, struct echo_reference_buffer *buffer, bool bIsAccuracyTime, struct timespec TimeInfo)
{
    ...
    else
    {
        ...
        buffer->delay_ns = (long)(((int64_t)(frames) * 1000000000) / mDL1Attribute->mSampleRate);
...
size_t AudioMTKStreamOut::writeDataToEchoReference(const void *buffer, size_t bytes, int remain_ms, struct timespec TimeInfo)
{
    ...
    else
    {
        ...
        tempTimeInfo = BInfo.time_stamp_predict;
    ...
    if (mEcho_reference != NULL)
    {
        ...
        get_playback_delay(b.frame_count, &b, BInfo.bHasRemainInfo, tempTimeInfo);
        mEcho_reference->write(mEcho_reference, &b);
    }
    EffectMutexUnlock();
#endif

#ifdef EXTCODEC_ECHO_REFERENCE_SUPPORT  //echo reference data queue from the record thread
    if (mAudioSpeechEnhanceInfoInstance->IsInputStreamAlive())
    {
#if 1 //not here but in queue reference data?
        //do not queue to echo reference if FM/mATV enalbed
        if (AudioFMController::GetInstance()->GetFmEnable() == true ||
            AudioMATVController::GetInstance()->GetMatvEnable() == true)
        {
            mAudioSpeechEnhanceInfoInstance->CloseEchoReferenceThread();
            return bytes;
        }
#endif
...
#endif
        //need echo reference data, going to create a record thread for this.
        mAudioSpeechEnhanceInfoInstance->StartEchoReferenceThread(mDL1Attribute->mdevices);

    }

#else   //echo reference data queue from the downlink data thread
    Mutex::Autolock _l(mSPEVoIPLock);
...
#endif  //EXTCODEC_ECHO_REFERENCE_SUPPORT
...
#ifdef EXTCODEC_ECHO_REFERENCE_SUPPORT  //close echo reference thread
    mAudioSpeechEnhanceInfoInstance->CloseEchoReferenceThread();
#endif
}
...
void AudioMTKStreamOut::setLowLatencyMode(bool mode,bool bforce)
{
    ALOGD("%s(%s)", __FUNCTION__, mode ? "true" : "false");
    static Mutex mLowLatencyModeLock;
    Mutex::Autolock _l(mLowLatencyModeLock);

    if ((mode == mLowLatencyMode && !bforce)||(mAudioBTCVSDControl->BT_SCO_ExtMDGetBTSCORunning()==true)) { return; }
