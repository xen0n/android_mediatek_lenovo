...
status_t AudioDigitalControl::SetMemIfFetchFormatPerSample(uint32 InterfaceType, AudioMEMIFAttribute::FETCHFORMATPERSAMPLE eFetchFormat)
{
    ...
    ALOGD("+%s(), InterfaceType = %d, eFetchFormat = %d, mAudioMEMIF[InterfaceType].mFetchFormatPerSample = %d\n"
          , __FUNCTION__, InterfaceType, eFetchFormat, mAudioMEMIF[InterfaceType].mFetchFormatPerSample);
    switch (InterfaceType)
    {
        case AudioDigitalType::MEM_DL1:
        {
            mAfeReg->SetAfeReg(AFE_MEMIF_PBUF_SIZE, mAudioMEMIF[InterfaceType].mFetchFormatPerSample << 16 , 0x00030000);
            break;
        }
        case AudioDigitalType::MEM_DL2:
        {
            mAfeReg->SetAfeReg(AFE_MEMIF_PBUF_SIZE, mAudioMEMIF[InterfaceType].mFetchFormatPerSample << 18 , 0x000C0000);
            break;
        }
        case AudioDigitalType::MEM_I2S:
        {
            //mAfeReg->SetAfeReg(AFE_DAC_CON1, mAudioMEMIF[InterfaceType].mSampleRate << 8 , 0x00000f00);
            ALOGD("Unsupport MEM_I2S");
            break;
        }
        case AudioDigitalType::MEM_AWB:
        {
            mAfeReg->SetAfeReg(AFE_MEMIF_PBUF_SIZE, mAudioMEMIF[InterfaceType].mFetchFormatPerSample << 20, 0x00300000);
            break;
        }
        case AudioDigitalType::MEM_VUL:
        {
            mAfeReg->SetAfeReg(AFE_MEMIF_PBUF_SIZE, mAudioMEMIF[InterfaceType].mFetchFormatPerSample << 22, 0x00C00000);
            break;
        }
        case AudioDigitalType::MEM_DAI:
        {
            mAfeReg->SetAfeReg(AFE_MEMIF_PBUF_SIZE, mAudioMEMIF[InterfaceType].mFetchFormatPerSample << 24, 0x03000000);
            break;
        }
        case AudioDigitalType::MEM_MOD_DAI:
        {
            mAfeReg->SetAfeReg(AFE_MEMIF_PBUF_SIZE, mAudioMEMIF[InterfaceType].mFetchFormatPerSample << 26, 0x0C000000);
            break;
        }
        default:
            return INVALID_OPERATION;
...
{
    ALOGD("+%s(), Data Format = %d, Output = %d\n", __FUNCTION__, ConnectionFormat, Output);
    mAfeReg->SetAfeReg(AFE_CONN_24BIT, (ConnectionFormat << Output), (1 << Output));
    return NO_ERROR;
}
...
{
    return ((mAfeReg->GetAfeReg(AFE_CONN_24BIT) & (1 << Output)) ? AudioDigitalType::OUTPUT_DATA_FORMAT_24BIT : AudioDigitalType::OUTPUT_DATA_FORMAT_16BIT);
}
...
status_t AudioDigitalControl::SetIrqMcuEnable(AudioDigitalType::IRQ_MCU_MODE Irqmode, bool bEnable)
{
    ...
    switch (Irqmode)
    {
        ...
        case AudioDigitalType::IRQ3_MCU_MODE:
        {
            ...
            if ((mAfeReg->GetAfeReg(AFE_IRQ_MCU_CON)&(1 << Irqmode)) != (bEnable << Irqmode))
            {
                ALOGD("+%s(), Irqmode = %d, Reg = 0x%x ,bEnable = %d\n", __FUNCTION__, Irqmode,mAfeReg->GetAfeReg(AFE_IRQ_MCU_CON), bEnable);
                ASSERT((mAfeReg->GetAfeReg(AFE_IRQ_MCU_CON)&(1 << Irqmode)) == (bEnable << Irqmode));
            }
#endif
            break;
        }
...
status_t AudioDigitalControl::GetIrqStatus(AudioDigitalType::IRQ_MCU_MODE Irqmode, AudioIrqMcuMode *Mcumode)
{
    switch (Irqmode)
    {
        ...
        case AudioDigitalType::IRQ3_MCU_MODE:
            ...
            if((mAfeReg->GetAfeReg(AFE_IRQ_MCU_CON)&(1 << Irqmode)) != ((Mcumode->mStatus)<<Irqmode))
            {
                ALOGD("+%s(), Irqmode = %d, Reg = 0x%x ,bEnable = %d\n", __FUNCTION__, Irqmode,mAfeReg->GetAfeReg(AFE_IRQ_MCU_CON), Mcumode->mStatus);
                ASSERT((mAfeReg->GetAfeReg(AFE_IRQ_MCU_CON)&(1 << Irqmode)) == ((Mcumode->mStatus)<<Irqmode));
            }
#endif
            break;
        default:
...
#if (defined(I2S_LOW_JITTER_MODE) || defined(MTK_AUDIO_EXTCODEC_SUPPORT))
    mDacI2SOut.mI2S_WLEN = AudioDigtalI2S::WLEN_32BITS;
    mDacI2SOut.mI2S_HD_EN = AudioDigtalI2S::LOW_JITTER_CLOCK;
#else
    mDacI2SOut.mI2S_WLEN = AudioDigtalI2S::WLEN_16BITS;
    mDacI2SOut.mI2S_HD_EN = AudioDigtalI2S::NORMAL_CLOCK;
#endif
...
status_t AudioDigitalControl::SetI2SDacOut(AudioDigtalI2S *mDigitalI2S)
{
    ...
    if (mAudioDigitalBlock[AudioDigitalType::I2S_OUT_DAC] == true)
    {
        ALOGD("I2S_OUT_DAC already enabled! ");
        return NO_ERROR;
    }
    ...
    Audio_I2S_Dac |= (mDacI2SOut.mI2S_HD_EN << 12);//Low jitter using external clock
...
status_t AudioDigitalControl::SetI2SDacEnable(bool bEnable)
{
    ...
    bool prev_I2SDacStatus;

    prev_I2SDacStatus = mAudioDigitalBlock[AudioDigitalType::I2S_OUT_DAC];
    if (prev_I2SDacStatus != bEnable)
    {
#if (defined(I2S_LOW_JITTER_MODE) || defined(MTK_AUDIO_EXTCODEC_SUPPORT))
        uint8_t low_jitter_clk_grp;
        switch (mDacI2SOut.mI2S_SAMPLERATE)
        {
            case 8000:
            case 16000:
            case 32000:
            ...
            case 24000:
            case 48000:
                low_jitter_clk_grp = 1;
                break;
            case 11025:
            case 22050:
            case 44100:
                low_jitter_clk_grp = 0;
                break;
            default:
                low_jitter_clk_grp = 0;
                break;
        }
        ALOGD("+%s(), low_jitter_clk_grp = %d\n", __FUNCTION__, low_jitter_clk_grp);

        // enable low-jitter clock
        if (bEnable == true && prev_I2SDacStatus == false)
        {
            ALOGD("+%s(), SET_LOWJITTER_CLK_ON \n", __FUNCTION__);
            if (low_jitter_clk_grp == 1)
            {
                ::ioctl(mFd, SET_LOWJITTER_CLK_ON, 1); //0: 22M, 1: 24M
            }
            else
            {
                ::ioctl(mFd, SET_LOWJITTER_CLK_ON, 0);
            }
        }
#endif
...
#if (defined(I2S_LOW_JITTER_MODE) || defined(MTK_AUDIO_EXTCODEC_SUPPORT))
        // disable low-jitter clock
        if (bEnable == false && prev_I2SDacStatus == true)
        {
            ALOGD("+%s(), SET_LOWJITTER_CLK_OFF \n", __FUNCTION__);
            if (low_jitter_clk_grp == 1)
            {
                ::ioctl(mFd, SET_LOWJITTER_CLK_OFF, 1); //0: 22M, 1: 24M
            }
            else
            {
                ::ioctl(mFd, SET_LOWJITTER_CLK_OFF, 0);
            }
        }
#endif
    }
...
status_t AudioDigitalControl::Set2ndI2SOut(AudioDigtalI2S *mDigitalI2S)
{
    ...
    u32AudioI2S |= (m2ndI2S.mI2S_HD_EN << 12);//Low jitter using external clock
...
status_t AudioDigitalControl::Set2ndI2SInConfig(unsigned int sampleRate, bool bIsSlaveMode)
{
    ...
    ALOGD("+%s(), sampleRate = %d, bIsSlaveMode = %d\n", __FUNCTION__, sampleRate, bIsSlaveMode);
...
#if defined(I2S_LOW_JITTER_MODE)
    _2ndI2SIn_attribute.mI2S_WLEN = AudioDigtalI2S::WLEN_32BITS;
    _2ndI2SIn_attribute.mI2S_HD_EN = AudioDigtalI2S::LOW_JITTER_CLOCK;
#else
    _2ndI2SIn_attribute.mI2S_WLEN = AudioDigtalI2S::WLEN_16BITS;
    _2ndI2SIn_attribute.mI2S_HD_EN = AudioDigtalI2S::NORMAL_CLOCK;
#endif
...
status_t AudioDigitalControl::Set2ndI2SIn(AudioDigtalI2S *mDigitalI2S)
{
    ...
    Audio_I2S_Adc |= (m2ndI2S.mI2S_HD_EN << 12);//Low jitter using external clock
...
status_t AudioDigitalControl::Set2ndI2SOutEnable(bool bEnable)
{
    ...
    uint8_t low_jitter_clk_grp;
    {
        switch (m2ndI2S.mI2S_SAMPLERATE)
        {
            case 8000:
            case 16000:
            case 32000:
