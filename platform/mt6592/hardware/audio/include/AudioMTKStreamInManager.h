...
class AudioMTKStreamInManager
{
    public:
        ...
        * @return status_t*/
        ...
        uint32_t CopyBufferToReference(uint32 mMemDataType, void *buffer , uint32 copy_size, AdditionalInfo_STRUCT AddInfo);
        // check if AWB is using for echo reference
        int checkAWBEchoInUse(AudioMTKStreamInClient *Client = 0);
        bool mAWBforEcho;

        // BLI_SRC
        MtkAudioSrc *mBliHandlerEchoRef;
        char       *mBliOutputBufferEchoRef;
        static const uint32 BliOutBufferSizeEchoRef  = MemAWBBufferSize; //44.1k->16k
        uint32 mEchoRefAWBBufferSize; //44100K, 20ms data size
#endif
    ...
    private:
        ...
        uint32_t WriteToReference(void *buffer , uint32 copy_size, AdditionalInfo_STRUCT AddInfo);
        bool mShortIntrforEchoRef;
        int mFd;
        void SetULIntrCountSwitch(bool state);
#endif

        MtkAudioSrc *mBliHandlerDAIBT;
