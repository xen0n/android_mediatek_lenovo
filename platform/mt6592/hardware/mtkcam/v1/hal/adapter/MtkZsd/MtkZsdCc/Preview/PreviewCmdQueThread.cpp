...
#if 1//chenglong1 add for ae-stable polling
	{
		//chenglong1 add for ae stable state
		char property_value[PROPERTY_VALUE_MAX] = { 0 };
	    property_set("persist.af.aestate", "false");
		MY_LOGD("ae-stable PreviewCmdQueThread init pid: %d, uid: %d", getpid(), getuid());
		property_get("persist.af.aestate", property_value, "y");
		MY_LOGD("persist.af.aestate getback value: %s", property_value);
	}
#endif
...
updateOne()
{
	...
	{
		//add by wjh
	    int isAEStable = mp3AHal->isAEStable();
		char property_value[PROPERTY_VALUE_MAX] = { 0 };

	    MY_LOGD("ae-stable ae-stable:%d, pid: %d, uid: %d", isAEStable, getpid(), getuid());
	    if (isAEStable) {
	        //getParamsMgr()->set("ae-stable", 1);
	        //MY_LOGD("ae-stable param %d", getParamsMgr()->getInt("ae-stable"));
			property_set("ae_stable_state", "1");
			property_set("persist.af.aestate", "true");
	    } else{
	        //getParamsMgr()->set("ae-stable", 0);
	        //MY_LOGD("ae-stable param %d xx", getParamsMgr()->getInt("ae-stable"));
			property_set("ae_stable_state", "0");
			property_set("persist.af.aestate", "false");
	    }
		
		//property_get("ae_stable_state", property_value, "x");
		//MY_LOGD("ae_stable_state getback value: %s", property_value);
