...
init()
{
    ...
    // init: for allocating memory
    if ( ! mpPreviewCmdQueThread->postCommand(PrvCmdCookie::eInit, PrvCmdCookie::eSemBefore) )
    {
        MY_LOGE("PrvCmdQThread Init fail");
        goto lbExit;
    }
    //
...
uninit()
{
    ...
    if(mbTakePicPrvNotStop)
    {
        ...
        mbTakePicPrvNotStop = false;
        stopPreview();
        MY_LOGD("Force stop preview done");
    }
    //
    //
    if ( ! mpPreviewCmdQueThread->postCommand(PrvCmdCookie::eUnInit, PrvCmdCookie::eSemAfter) )
    {
        MY_LOGE("UnInit fail");
        return false;
    }
