...
startPreview()
{
    if(mbTakePicPrvNotStop)
    {
        ...
        {
            if(!mpPreviewCmdQueThread->postCommand(PrvCmdCookie::eEnablePass2, PrvCmdCookie::eSemBefore))
            {
...
onHandleStartPreview()
{
    ...
    //
#if 0
    if ( ! mpPreviewCmdQueThread->postCommand(PrvCmdCookie::eDelay, PrvCmdCookie::eSemBefore) )
    {
...
onHandlePreCapture()
{
    ...
    //
    if ( OK != mpCaptureCmdQueThread->onInitMem() )
    {
        MY_LOGE("StartPreview initMem (start): fail");
        goto lbExit;
    }
    //
