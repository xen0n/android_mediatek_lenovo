...
onHandleStartRecording()
{
    ...
    //
    status_t ret = OK;
    ResourceLock::ECamAdapter resLockAdapter;
    ...
    if(mbIsHsVr)
    {
        resLockAdapter = ResourceLock::eMTKVIDEO_REC_HS;
    }
    else
    {
        resLockAdapter = ResourceLock::eMTKVIDEO_REC;
    }
    //
    if ( ! mpResourceLock->SetMode(resLockAdapter) )
    {
    ...
    //
    if ( ! mpResourceLock->Lock(resLockAdapter) )
    {
...
onHandleStopRecording()
{
    ...
    //
    if ( ! mpResourceLock->Unlock(resLockAdapter) )
    {
        CAM_LOGE("Resource Lock fail");
        return INVALID_OPERATION;
    }
