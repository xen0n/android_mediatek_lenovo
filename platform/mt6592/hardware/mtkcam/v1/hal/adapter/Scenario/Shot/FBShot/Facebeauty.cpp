...
MBOOL
Mhal_facebeauty::
doFaceDetection(IMEM_BUF_INFO Srcbufinfo, int u4SrcWidth, int u4SrcHeight)
{
    halFDBase*   mpFDHalObj = halFDBase::createInstance(HAL_FD_OBJ_FDFT_SW); 
    if (mpFDHalObj == NULL)
    {
        MY_LOGE("[doFaceDetection] can't get halFDBase instance.");
        return MFALSE;
    }
    mpFDHalObj->halFDInit(u4SrcWidth, u4SrcHeight, (MUINT32) mpFDWorkingBuffer, FD_WORKING_BUF_SIZE, 1, 0); //1:Enale SW resizer
    ...
    do {
        ...
        mpFDHalObj->halFDDo(0, (MINT32)qVGABuffer, (MINT32)Srcbufinfo.virtAddr, false, mShotParam.mi4Rotation);
    ...
    {
        MY_LOGI("[doFaceDetection] %d faces are detected on captured image", FBmetadata.number_of_faces);
...
#warning [FIXME] workaround for Alta phone capture mode can not work
    ...
    ret = doFaceDetection(Srcbufinfo, mu4W_yuv, mu4H_yuv);
    if  ( ! ret )
    {
        goto lbExit;
    }
