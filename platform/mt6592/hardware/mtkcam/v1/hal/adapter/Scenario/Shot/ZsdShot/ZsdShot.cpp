...
onYuv_capture(CapBufQueNode & rNode)
{
    ...
    {
        // DDP image transform downscale only support 32X. Max src may be 1920*1080?
        ...
        {
            rMainImgBufInfo.u4ImgWidth = (rMainImgBufInfo.u4ImgWidth < rSubImgBufInfo.u4ImgWidth/16)
                                        ? rMainImgBufInfo.u4ImgWidth
                                        : rSubImgBufInfo.u4ImgWidth/16;
            rMainImgBufInfo.u4ImgHeight = (rMainImgBufInfo.u4ImgHeight < rSubImgBufInfo.u4ImgHeight/16)
                                        ? rMainImgBufInfo.u4ImgHeight
                                        : rSubImgBufInfo.u4ImgHeight/16;
...
handleJpegData(MUINT8* const puJpegBuf, MUINT32 const u4JpegSize, MUINT8* const puThumbBuf, MUINT32 const u4ThumbSize)
{
    ...
    makeExifHeader(eAppMode_ZsdMode, puThumbBuf, u4ThumbSize, puExifHeaderBuf, u4ExifHeaderSize);
...
checkIfImgTransformSupport(ImgBufInfo const & rSrcBufInfo, ImgBufInfo const & rDstBufInfo, int rot)
{
    if (rot == 90 || rot == 270)
    {
        if (rDstBufInfo.u4ImgWidth*32 < rSrcBufInfo.u4ImgHeight ||
             rDstBufInfo.u4ImgHeight*32 < rSrcBufInfo.u4ImgWidth)
        {
            MY_LOGD("Not support Resize >32x src =(%d,%d), dst=(%d,%d), rot=(%d)",
                     rSrcBufInfo.u4ImgWidth, rSrcBufInfo.u4ImgHeight,
    ...
    {
        if (rDstBufInfo.u4ImgWidth*32 < rSrcBufInfo.u4ImgWidth ||
             rDstBufInfo.u4ImgHeight*32 < rSrcBufInfo.u4ImgHeight)
        {
