...
sendCommand(int32_t cmd, int32_t arg1, int32_t arg2)
{
    ...
    case CAMERA_CMD_STOP_FACE_DETECTION:
        ...
        if (p3AHal)
        {
            ...
            {
                MY_LOGD("[SetFDInfo] CAMERA_CMD_STOP_FACE_DETECTION In");
                mpDetectedFaces->number_of_faces = 0;
                p3AHal->setFDInfo(mpDetectedFaces);
                MY_LOGD("[SetFDInfo] CAMERA_CMD_STOP_FACE_DETECTION Out");
            }
...
stopPreview()
{
    ...
    if (p3AHal)
    {
        ...
        if(mpDetectedFaces != 0)
        {
            MY_LOGD("[SetFDInfo] stopPreview In");
            mpDetectedFaces->number_of_faces = 0;
            p3AHal->setFDInfo(mpDetectedFaces);
            MY_LOGD("[SetFDInfo] stopPreview Out");
        }
        p3AHal->destroyInstance();
    }
    #endif
