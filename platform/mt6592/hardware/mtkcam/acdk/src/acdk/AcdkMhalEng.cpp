...
MINT32 AcdkMhalEng::acdkMhal3ASetParam(MINT32 devID, MUINT8 IsFactory)
{
    ...
    //2013/10/3 AF mode sync  :1.factory set to continue mode  2.Meda mode set to manual mode
    // kd_camera_feature_enum.h (custom/common/kernel/imgsensor/inc) 
    // AF mode enum. 0: AF-Single Shot Mode, 1: AF-Continuous Mode, 2: AF-Continuous Mode (Video)
    //               3: AF Macro Mode, 4:Focus is set at infinity, 5:Manual Focus Mode, 6:AF Full Scan Mode
    if(IsFactory)
        cam3aParam.u4AfMode = 1; 
    else
        cam3aParam.u4AfMode = 5;
...
MINT32 AcdkMhalEng::acdkMhalPreviewStart(MVOID *a_pBuffIn)
{
    ...
    //====== factory mode ======
    if(mAcdkMhalPrvParam.IsFactoryMode == 1)
        mIsFactory = 1;
    else
        mIsFactory = 0;
    //====== Member Variable ======
...
MINT32 AcdkMhalEng::acdkMhalCaptureStart(MVOID *a_pBuffIn)
{
    ...
    else // meta mode : af function control by CCT tool
    {
        mFocusDone = MTRUE;
        mFocusSucceed = MTRUE;
    }
