...
MBOOL
ZSDScenario::
deque(MUINT32 port, vector<PortQTBufInfo> *pBufIn)
{
    if ( ! pBufIn )
    {
        MY_LOGE("pBufIn==NULL");
    ...
    if (port & eID_Pass1DispOut || port & eID_Pass1RawOut)
    {
        ...
        if (port & eID_Pass1DispOut) {
            ...
            if(mpExtImgProcHw != NULL)
            {
                if(mpExtImgProcHw->getImgMask() & ExtImgProcHw::BufType_ISP_ZSD_P1_Out)
                {
                    IExtImgProc::ImgInfo img;
                    //
                    img.bufType     = ExtImgProcHw::BufType_ISP_ZSD_P1_Out;
