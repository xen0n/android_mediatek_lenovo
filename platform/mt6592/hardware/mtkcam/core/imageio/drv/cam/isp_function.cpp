...
//TEST 0418            ISP_WRITE_REG(m_pPhyIspReg ,CAM_CTL_TILE, 0x7A03001E);  // TEST_MDP // hardware do not read both tpipe width and height
        ...
        if(m_pIspDrv->IsReadOnlyMode())
        {
            ...
            ISP_IOCTL_WRITE_REG(m_pIspDrv, m_pIspDrv->getRegAddrMap(),  CAM_CTL_SRAM_MUX_CFG_CLR, ( CQ_CAM_CTL_CAN_BE_CLR_BITS(SRAM_MUX_CFG,this->isIspOn)&(~0x01000000) )&(~ctl_sram_mux_cfg.reg_val));
        ...
        else
        {
            ...
            ISP_WRITE_REG(m_pIspReg,  CAM_CTL_MUX_SEL2_CLR, (this->isIspOn?0xBF47E083:0) & (~ctl_mux_sel2.reg_val));
            //DONOT change SGG_HRZ_SEL(no EN bit)
            ISP_WRITE_REG(m_pIspReg,  CAM_CTL_SRAM_MUX_CFG_CLR, ( CQ_CAM_CTL_CAN_BE_CLR_BITS(SRAM_MUX_CFG,this->isIspOn)&(~0x01000000) )&(~ctl_sram_mux_cfg.reg_val));
...
int ISP_TOP_CTRL::_enable( void* pParam  )
{
    ...
    if( CAM_ISP_PASS1_START == (*start) ) 
    {
        ...
        //SENINF1_INT_EN Note :Move to Senif_drv.cpp
