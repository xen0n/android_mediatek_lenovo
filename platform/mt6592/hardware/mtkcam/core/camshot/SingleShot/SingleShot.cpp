...
startOne(SensorParam const & rSensorParam)
{
    ...
    // The raw type, 0: pure raw, ZSD+IP
    //               1: pre-process raw, VSS
    //               2: pattern
    ...
    if (NULL == mpCamIOPipe) 
    {
        ...
        // pure raw: ZSD+IP, pre-process raw: VSS
...
createSensorRawImg(SensorParam const & rSensorParam, ImgBufInfo const & rRawImgBufInfo)
{
    ...
    if( mbCCmode )
    {
        if (!lock(RES_MGR_DRV_SCEN_HW_VSS, PIPE_MGR_DRV_PIPE_MASK_CAM_IO, 3000))
        {
...
createYuvRawImg(ImgBufInfo const & rSrcImgBufInfo, Rect const rSrcCropRect, MUINT32 const u4Img1Rot, MUINT32 const u4Img1Flip, ImgBufInfo const & rDstImgBufInfo1, ImgBufInfo const &rDstImgBufInfo2 )
{
    ...
    // The raw type, 0: pure raw, 1: pre-process raw 
    // pure raw: ZSD+IP, pre-process: VSS
    IPostProcPipe *pPostProcPipe = IPostProcPipe::createInstance(
            mbCCmode ? eSWScenarioID_VSS : eSWScenarioID_CAPTURE_NORMAL, 
    ...
    {
        CPTLogStr(Event_SShot_createYuvRawImg, CPTFlagSeparator, "3A Create");
        p3AObj = Hal3ABase::createInstance(mSensorParam.u4DeviceID);           
        //
        CPTLogStr(Event_SShot_createYuvRawImg, CPTFlagSeparator, "SetIspProfCap");
        p3AObj->setIspProfile(EIspProfile_NormalCapture);
    }
#endif 
