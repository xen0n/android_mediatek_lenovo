...
EisHal::EisHal() : EisHalBase()
{
    ...
    //Accumulation Mode
    mAccumulationModeOn = MFALSE;
    mConfidenceThreshold = 0;
    mAccumulatedGMV_X = 0;
    mAccumulatedGMV_Y = 0;
    mIIR_DS = 1;
}
...
MINT32 EisHal::doEIS()
{
    if(mConfigPass == MTRUE)
    {
        ...
        //====== Accumation Mode ======
        updateAccumulatedGMV();
        //@TEST
        MFLOAT AGMV_X, AGMV_Y;
        getAccumulatedGMV(AGMV_X, AGMV_Y);
...
MVOID EisHal::startLog()
{
    mLogOffset = 0;
    mLogBuffer = (char*)malloc(4<<20);
    mLogOn = MTRUE;
}


MVOID EisHal::writeLog(char *string, MUINT32 length)
{
    if(!mLogOn) {
        return;
    }

    if(mLogOffset > (4<<20)-1000) {
        EIS_LOG("log buffer is full");
        return;
    }

#if 1
    memcpy(mLogBuffer + mLogOffset, string, length);
    mLogOffset += length;
#endif
}

MVOID EisHal::stopLog()
{
#if 1
    if(!mLogOn) {
        return;
    ...
    char logname[128];
    sprintf(logname, "/sdcard/eis_log_%d.txt", count);

    EIS_LOG("open file %s", logname);
    FILE *fp = fopen(logname, "wb");
    if(fp == NULL)
    {
        EIS_LOG("fopen %s is NULL in EisHal::stopLog", logname);
        return;
    }

    EIS_LOG("write file to %s", logname);
    fwrite( mLogBuffer, mLogOffset , 1, fp);
    EIS_LOG("close file %s", logname);
    fclose(fp);

    EIS_LOG("release mLogBuffer %p", mLogBuffer);
    free(mLogBuffer);
    mLogBuffer = NULL;
...
// Accumulation Mode
MVOID   EisHal::startAccumulationMode(MUINT32 confidenceThreshold)
{
    EIS_LOG("+");

    char value[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("debug.eis.log", value, "1");
    MUINT32 enableLog = atoi(value);
    if(enableLog)
        startLog();

    mLock.lock();
    mAccumulationModeOn = MTRUE;
...
MVOID   EisHal::stopAccumulationMode()
{
    EIS_LOG("+");
...
MVOID   EisHal::resetAccumulatedGMV()
{
    EIS_LOG("+");
...
MVOID   EisHal::getAccumulatedGMV(MFLOAT &AGMV_X, MFLOAT &AGMV_Y)
{
    EIS_LOG("+");

    MBOOL   on;
    mLock.lock();
    on = mAccumulationModeOn;
    mLock.unlock();

    if(!on) {
        EIS_LOG("mAccumulationModeOn is 0");
        AGMV_X = 0;
        AGMV_Y = 0;
        goto lbExit;
    }

    EIS_LOG("get from mAccumulatedGMV");
    mLock.lock();
    AGMV_X = mAccumulatedGMV_X;
    AGMV_Y = mAccumulatedGMV_Y;
    mLock.unlock();

lbExit:
    EIS_LOG("AGMV_X=%f AGMV_Y=%f", AGMV_X, AGMV_Y);
    EIS_LOG("-");
}


MVOID   EisHal::updateAccumulatedGMV()
{
    char value[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("debug.eis.method", value, "2");


    MUINT32 method = atoi(value);
    EIS_LOG("method = %d", method);
    switch(method) {
        case 1: updateAccumulatedGMV1(); break;
        case 2: updateAccumulatedGMV2(); break;
        default: updateAccumulatedGMV2();
    }
}


MVOID   EisHal::updateAccumulatedGMV1()
{
    MBOOL on;
    MUINT32 confidenceThreshold;
    mLock.lock();
    on = mAccumulationModeOn;
    confidenceThreshold = mConfidenceThreshold;
    mLock.unlock();
    ...
    //  ()  get hw statistic
    EIS_STATISTIC_STRUCT eisHWStatistic;
    getEISStatistic(&eisHWStatistic);

    //  ()  calulate GMV
    MINT32  sum_x_tmp=0, cnt_x_tmp=0;
    MINT32  sum_y_tmp=0, cnt_y_tmp=0;

    for( int window=0; window<EIS_MAX_WIN_NUM; window++) {
        //x
        if( eisHWStatistic.NewTrust_X[window] > confidenceThreshold )
        {
            sum_x_tmp += eisHWStatistic.i4LMV_X[window];
            cnt_x_tmp++;
        }

        //y
        if( eisHWStatistic.NewTrust_Y[window] > confidenceThreshold )
        {
            sum_y_tmp += eisHWStatistic.i4LMV_Y[window];
            cnt_y_tmp++;
        }
    }

    if( cnt_x_tmp*8 < EIS_MAX_WIN_NUM ) {
        sum_x_tmp = 25600;  // (100*256) if number of reliable EIS windows is low, set a large MV to prevent error propagation
    } else {
        sum_x_tmp /= cnt_x_tmp;
    }

    if( cnt_y_tmp*8 < EIS_MAX_WIN_NUM ) {
        sum_y_tmp = 25600; // if number of reliable EIS windows is low, set a large MV to prevent error propagation
    } else {
        sum_y_tmp /= cnt_y_tmp;
    }

    mLock.lock();
    mAccumulatedGMV_X += (MFLOAT)sum_x_tmp*mIIR_DS/256;
    mAccumulatedGMV_Y += (MFLOAT)sum_y_tmp*mIIR_DS/256;
    mLock.unlock();

    //if(g_debugDump >= 1) {
    if(1) {
        EIS_LOG("mAccumulatedGMV_X=%f, mAccumulatedGMV_Y=%f, mIIR_DS=%d"
                , mAccumulatedGMV_X
                , mAccumulatedGMV_Y
                , mIIR_DS
                );
    ...
    if(mLogOn) {
        ...
        static MUINT32 frame = 0;
        length = sprintf(buffer, "#%d\n", frame);
        frame++;
        writeLog(buffer, length);
        memset(buffer, 0, 256);

        for( int window=0; window<EIS_MAX_WIN_NUM; window++) {
            length = sprintf(buffer
                    , "MB%d%d\tLMV\t%d\t%d\tTrust\t%d\t%d\n"
                    , window /4
                    , window % 4
                    , eisHWStatistic.i4LMV_X[window]
                    , eisHWStatistic.i4LMV_Y[window]
                    , eisHWStatistic.NewTrust_X[window]
                    , eisHWStatistic.NewTrust_Y[window]
                    );
            writeLog(buffer, length);
            memset(buffer, 0, 256);
        }
        length = sprintf(buffer, "AGMV %f %f\n", mAccumulatedGMV_X, mAccumulatedGMV_Y);
        writeLog(buffer, length);
        memset(buffer, 0, 256);
    }
...
MVOID   EisHal::updateAccumulatedGMV2()
{
    MINT32 sum_x_tmp;
    MINT32 sum_y_tmp;
    MINT32 cnt_x_tmp;
    ...
    // initialize hist
    memset(hist, 0, 1024 * sizeof(MUINT8));
    ...
    for( int window=0; window<EIS_MAX_WIN_NUM; window++ )
    {
        if( eisHWStatistic.NewTrust_X[window] > confidenceThreshold && eisHWStatistic.NewTrust_Y[window] > confidenceThreshold )
        {
            //X
            if( eisHWStatistic.i4LMV_X[window] >= 0 )
            {
                mvx = (eisHWStatistic.i4LMV_X[window] + 128) >> 8;
            }
            else
            {
                mvx = -((-eisHWStatistic.i4LMV_X[window] + 128) >> 8);
            ...
            //Y
            if( eisHWStatistic.i4LMV_Y[window] >= 0 )
            {
                mvy = (eisHWStatistic.i4LMV_Y[window] + 128) >> 8;
            }
            else
            {
                mvy = -((-eisHWStatistic.i4LMV_Y[window] + 128) >> 8);
            ...
            hist[ ((mvy + 16) << 5) + (mvx + 16) ]++;

            if( hist[ ((mvy + 16) << 5) + (mvx + 16) ] > max_vote_cnt )
            {
                max_vote_cnt = hist[ ((mvy + 16) << 5) + (mvx + 16) ];
                max_vote_idx = ((mvy + 16) << 5) + (mvx + 16);
            }
        }
    }
    ...
    for( int window=0; window<EIS_MAX_WIN_NUM; window++ )
    {
        if( eisHWStatistic.NewTrust_X[window] > confidenceThreshold && eisHWStatistic.NewTrust_Y[window] > confidenceThreshold )
        {
            ...
            if( (((mvy + 16) << 5) + (mvx + 16)) == max_vote_idx )
            {
                sum_x_tmp += eisHWStatistic.i4LMV_X[window];
                sum_y_tmp += eisHWStatistic.i4LMV_Y[window];
                cnt_x_tmp++;
    ...
    mvy = (max_vote_idx >> 5) - 16;
    mvx = max_vote_idx - ((max_vote_idx >> 5) << 5) - 16;

    if( cnt_x_tmp == 0 || abs(mvx) > 10 || abs(mvy) > 10 )
    {
        sum_x_tmp += 25600;
        sum_y_tmp += 25600;
    }
    ...
    {
        sum_x_tmp = sum_x_tmp * mIIR_DS / cnt_x_tmp;
        sum_y_tmp = sum_y_tmp * mIIR_DS / cnt_x_tmp;
    }

    mAccumulatedGMV_X += (MFLOAT)sum_x_tmp/256;
    mAccumulatedGMV_Y += (MFLOAT)sum_y_tmp/256;
