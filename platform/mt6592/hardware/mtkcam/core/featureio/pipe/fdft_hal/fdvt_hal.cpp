...
#if(FAR_V1)
static MUINT32 image_width_array[FD_SCALES] = {360, 280, 220, 172, 134, 106, 84, 68, 54, 44, 34};
static MUINT32 image_height_array[FD_SCALES] = {270, 210, 164, 128, 100, 80, 64, 50, 40, 32, 26};
#endif

#if(FAR_V2)
static MUINT32 image_width_array[FD_SCALES] = {400, 304, 240, 192, 152, 120, 96, 80, 64, 48, 34};
static MUINT32 image_height_array[FD_SCALES] = {300, 228, 180, 144, 114, 90, 72, 60, 48, 36, 26};
#endif

static MUINT32 image_width_array_v1[FD_SCALES] = {320, 256, 204, 160, 128, 102, 80, 64, 50, 40, 32};
static MUINT32 image_height_array_v1[FD_SCALES] = {240, 192, 152, 120, 96, 76, 60, 48, 38, 30, 24};
static MUINT32 image_width_array_v2[FD_SCALES] = {360, 280, 220, 172, 134, 106, 84, 68, 54, 44, 34};
static MUINT32 image_height_array_v2[FD_SCALES] = {270, 210, 164, 128, 100, 80, 64, 50, 40, 32, 26};
static MUINT32 image_width_array_v3[FD_SCALES] = {400, 304, 240, 192, 152, 120, 96, 80, 64, 48, 34};
static MUINT32 image_height_array_v3[FD_SCALES] = {300, 228, 180, 144, 114, 90, 72, 60, 48, 36, 26};
...
getInstance()
{
    ...
    MY_LOGD("[Create] count:%d->%d pHalFD:%p", oldCount, mFDCount, pHalFD);
    if  ( 0 == oldCount )
    {
        if  ( ! pHalFD )
        {
...
//**********************************************************************//    
    g_SDStatus = SD_mode;
    MY_LOGD("[mHalFDInit] Start, SD:%d, ",SD_mode);
    if(SD_mode!=1)
    {
...
)
{
    switch(ucBufferGroup)
    {
        case 0:
                  ...
                  g_uwDst_height = ASD_IMAGE_HEIGHT;
                  doRGB565Buffer_SW(dstAddr, srcAddr);
                  break;
        case 1:
                  g_ucPlane = 2;
        ...
        case 3:
                  g_ucPlane = 2;
                  g_uwDst_width = ASD_IMAGE_WIDTH;
                  g_uwDst_height = ASD_IMAGE_WIDTH*9/16;
                  doRGB565Buffer_DDP(dstAddr, srcAddr);
                  break;
        case 4:
                  g_ucPlane = 3;
        ...
        case 5:
                  g_ucPlane = 2;
                  g_uwDst_width = ASD_IMAGE_WIDTH;
                  g_uwDst_height = ASD_IMAGE_WIDTH*3/5;
                  doRGB565Buffer_DDP(dstAddr, srcAddr);
                  break;
        case 6:
                  g_ucPlane = 3;
    ...
    sprintf(szFileName1, "/sdcard/srcOK_%04d_%04d_YCC420_%d.raw", g_FDW,g_FDH,g_count);
    pRawFp1 = fopen(szFileName1, "wb");               
    if (NULL == pRawFp1 )
    {
        MY_LOGD("Can't open file to save RAW Image\n"); 
    ...
    sprintf(szFileName, "/sdcard/dstOK_320_240_RGB565_%d.raw", g_count);
    pRawFp = fopen(szFileName, "wb");               
    if (NULL == pRawFp )
    {
        MY_LOGD("Can't open file to save RAW Image\n"); 
    ...
    i4WriteCnt = fwrite((void *)dstAddr,1, (320 * 240 * 2),pRawFp);
    fflush(pRawFp); 
    fclose(pRawFp);
#endif     
...
{
    g_SDStatus = SDEnable;
    if( (g_SDStatus == 1) && (IMAGE_MAX_WIDTH != 320))
    {
        MY_LOGW("Incorrect halFDDo (SD): SDEnable:%d, img_w:%d, img_h:%d,", g_SDStatus, IMAGE_MAX_WIDTH, IMAGE_MAX_HEIGHT);
        return MHAL_INPUT_SIZE_ERROR;
    }
    MY_LOGD("SD Status: SDEnable:%d, img_w:%d, img_h:%d,", g_SDStatus, IMAGE_MAX_WIDTH, IMAGE_MAX_HEIGHT);
...
)
{
    if( (g_SDStatus == 1) && (IMAGE_MAX_WIDTH != 320))
    {
        MY_LOGW("Incorrect GetFaceResult(SD): SDEnable:%d, img_w:%d, img_h:%d,", g_SDStatus, IMAGE_MAX_WIDTH, IMAGE_MAX_HEIGHT);
        return 0;
    }
...
halFDVT::halSDGetSmileResult()
{
    if( (g_SDStatus == 1) && (IMAGE_MAX_WIDTH != 320))
    {
        MY_LOGW("Incorrect GetSmileResult(SD): SDEnable:%d, img_w:%d, img_h:%d,", g_SDStatus, IMAGE_MAX_WIDTH, IMAGE_MAX_HEIGHT);
        return 0;
    }
