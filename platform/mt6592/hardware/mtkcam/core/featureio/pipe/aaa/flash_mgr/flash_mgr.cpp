...
int connectSocket()
{
	//LogInfo("connectSocket %d",__LINE__);

	return 0;
}

int dec2GPower(int bDec)
{
	LogInfo("dec2GPower+ %d %d",bDec, __LINE__);
	///////////////////////
	//connect socket
	int g_sock;
	g_sock = socket_local_client("rild-oem", ANDROID_SOCKET_NAMESPACE_RESERVED, SOCK_STREAM);
	...
	char s[strLen];
        int count =1 ;
	if(bDec==0)
		sprintf(s, "AT+ERFTX=1,0,0");
	else
		sprintf(s, "AT+ERFTX=1,64,0");

	ret = send(g_sock, &count, sizeof(int), 0);
	ret = send(g_sock, &strLen, sizeof(strLen), 0);
	ret = send(g_sock, s, strLen, 0);

	//recv
	int lenReply;
	char sReply[20];
	ret = recv(g_sock, (void*)&lenReply, sizeof(lenReply), 0);
	ret = recv(g_sock, (void*)sReply, 10, 0);
...
int closeSocket()
{
	//LogInfo("closeSocket% d", __LINE__);
...
FlashMgr::FlashMgr()
{
	...
	//g_bCapturing=0;
       ...
       if(isFlashAWBv2Enabled())
       {
            ...
            FlashAwbInitParam = getFlashAWBParam();

            FlashAlg::getInstance()->Flash_Awb_Init(FlashAwbInitParam);
        }
        //end
...
void FlashMgr::hw_setFlashProfile(FlashAlg* pStrobeAlg, FLASH_PROJECT_PARA* pPrjPara, NVRAM_CAMERA_STROBE_STRUCT* pNvram)
{
	...
	{
		LogInfo("setProfile-IChangeByVBatEn vbat=%d, th=%d",vbat,pPrjPara->engLevel.vBatL);
	...
	{
	    LogInfo("setProfile-IChangeByVBatEn=NO vbat=%d err=%d",vbat,err);
    }
...
int FlashMgr::init(int sensorType)
{
    ...
    FLASH_PROJECT_PARA prjParaA;
    prjParaA = getAutoProjectPara();
	g_pStrobe->init(sensorType, prjParaA.engLevel.mfDutyMaxL);
	NVRAM_CAMERA_STROBE_STRUCT* pNv;
    nvGetBuf(pNv);
	//forceLoadNvram();
	return 0;
...
MUINT32 u4FlashResultWeight;
	...
	{
		int isLow;
		g_pStrobe->lowPowerDetectEnd(&isLow);
		if(isLow==1)
		{
			FLASH_PROJECT_PARA prjPara;
			int aeMode;
			aeMode = AeMgr::getInstance().getAEMode();
			prjPara = getFlashProjectPara(aeMode);
			pStrobeAlg->setStrobeMaxDutyStep(prjPara.engLevel.pfDutyL, prjPara.engLevel.pmfStepL, prjPara.engLevel.mfDutyMaxL, prjPara.engLevel.pmfStepL);
			pStrobeAlg->setStrobeMinDutyStep(prjPara.engLevel.mfDutyMinL, prjPara.engLevel.pmfStepL);
		}
		...
		LogInfo("Estimate-");
              ...
              if(isFlashAWBv2Enabled())
              {
                    pStrobeAlg->Flash_Awb_Algo(&u4FlashResultWeight);
              }
		int afe;
		int isp;
...
int FlashMgr::setFlashMode(int mode)
{
	...
	/*lenovo-sw sunliang modify 2014_5_16 begin*/
	/*
	if(m_isAfState==1)
	{
		m_flashMode = mode;
	...
	/*lenovo-sw sunliang modify 2014_5_16 end*/
    int fmode;
    int fstyle;
    fmode = mode;
    ...
    else if(g_previewMode==e_NonePreview)
    {
        ...
        if(mode==LIB3A_FLASH_MODE_FORCE_TORCH)
        {
            LogInfo("torch");
            g_pStrobe = StrobeDrv::createInstance();
            g_pStrobe->initTemp(m_sensorType, prjParaA.engLevel.mfDutyMaxL);
            turnOnTorch();
        }
        else
        {
...
int FlashMgr::capCheckAndFireFlash_Start()
{
	FLASH_PROJECT_PARA prjParaA;
	prjParaA = getAutoProjectPara();
	LogInfo("capCheckAndFireFlash_Start %d",__LINE__);
	LogInfo("capCheckAndFireFlash_Start %d e %d",__LINE__,prjParaA.bDec2GPower);
	if(prjParaA.bDec2GPower==1)
		connectSocket();
    ...
    if(g_eg_bUserMf==1)
{
	...
	if(m_flashOnPrecapture==1)
	{
		...
		g_pStrobe->lowPowerProtectStart();
		hwSetFlashOn();

		if(prjPara.bDec2GPower==1)
		{
			int engTest;
			int engCur;
			engTest = FlashAlg::getInstance()->calFlashEng(prjPara.dutyDec2G , prjPara.stepDec2G);
			engCur = FlashAlg::getInstance()->calFlashEng(g_expPara.duty, g_expPara.step);
