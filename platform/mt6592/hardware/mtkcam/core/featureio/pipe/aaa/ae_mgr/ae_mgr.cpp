...
MINT32 u4AEPlineInfoSize, u4AEPlineMappingSize, u4AFTableSize, u4GainTableSize;
    ...
    if(m_pIAeAlgo != NULL) {
        ...
        u4AEPlineInfoSize = sizeof(g_rAEPlineTable->AEPlineInfo);
        u4AEPlineMappingSize = 31*sizeof(strAEPLineMapping);
        u4AFTableSize = sizeof(g_rAEInitInput.rAEPARAM.strAFPLine);
        u4GainTableSize = sizeof(g_rAEPlineTable->AEGainList);
        ...
        memcpy(&rAEPlineDebugInfo, &(g_rAEPlineTable->AEPlineInfo), u4AEPlineInfoSize);
        memcpy(&(rAEPlineDebugInfo.PlineInfo[u4AEPlineInfoSize]), &(g_rAEPlineTable->pAEModePLineMapping), u4AEPlineMappingSize);
        memcpy(&(rAEPlineDebugInfo.PlineInfo[u4AEPlineInfoSize + u4AEPlineMappingSize]), &(g_rAEInitInput.rAEPARAM.strAFPLine), u4AFTableSize);
        memcpy(&(rAEPlineDebugInfo.PlineInfo[u4AEPlineInfoSize + u4AEPlineMappingSize + u4AFTableSize]), &(g_rAEInitInput.rAEPARAM.strAFZSDPLine), u4AFTableSize);
        memcpy(&(rAEPlineDebugInfo.PlineInfo[u4AEPlineInfoSize + u4AEPlineMappingSize + 2*u4AFTableSize]), &(g_rAEPlineTable->AEGainList), u4GainTableSize);
    } else {
