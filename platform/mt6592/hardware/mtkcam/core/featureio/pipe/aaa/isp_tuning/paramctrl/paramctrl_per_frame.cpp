...
prepareHw_PerFrame_BPC()
{
    if (m_rIspCamInfo.eIspProfile == EIspProfile_NormalCapture_16M)
        ISP_MGR_BNR_T::getInstance(getSensorDev()).setBPCEnable(MFALSE);
    else
        ISP_MGR_BNR_T::getInstance(getSensorDev()).setBPCEnable(MTRUE);
...
{
    if (m_rIspCamInfo.eIspProfile == EIspProfile_NormalCapture_16M)
        ISP_MGR_BNR_T::getInstance(getSensorDev()).setCTEnable(MFALSE);
    else
        ISP_MGR_BNR_T::getInstance(getSensorDev()).setCTEnable(MTRUE);
