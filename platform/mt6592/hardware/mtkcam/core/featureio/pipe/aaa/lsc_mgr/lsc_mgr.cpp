...
static ACDK_SCENARIO_ID_ENUM SensorScenarioIspProfileMapping[] =
{
        ...
        ACDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG,   //EIspProfile_NormalCapture_CC
        ACDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG,   //EIspProfile_NormalCapture_16M
...
setMetaIspProfile(EIspProfile_T const eIspProfile, MUINT32 sensor_mode)
{
    ...
    if (eIspProfile == EIspProfile_NormalCapture ||
        eIspProfile == EIspProfile_NormalCapture_CC ||
        eIspProfile == EIspProfile_NormalCapture_16M) 
    {
        switch (m_SensorMode) 
        {
...
#if ENABLE_TSF
,
m_bTSF(MFALSE), m_bTsfForceAWB(MFALSE), m_fgTsfSetTbl(MFALSE), m_fgPreflash(MFALSE), m_fgCtIdxExcd(MFALSE), m_bIsVdo(MFALSE)
#endif
{
                    ...
                    if (lsc->checkAspectRatioChange() == MTRUE) 
                    {
                        ...
                        if (ret != S_TSF_OK) {
                            ...
                            if (ret != S_TSF_OK) {
                             ...
                             EIspProfile_NormalCapture_CC == lsc->getIspProfile() ||
                             EIspProfile_NormalCapture_16M == lsc->getIspProfile())
                    {
...
MVOID
LscMgr::onRecordingStart()
{
    MINT32 val;
    GET_PROP("debug.lsc_mgr.vdoTsfOff", "-1", val);

    ::pthread_mutex_lock(&mTSFMutex);
    
    if (val != -1)
    {
        m_bIsVdo = val ? MTRUE : MFALSE;
    }
    else
    { 
        m_bIsVdo = (isTSFVdoStop(m_SensorDev)) ? MTRUE : MFALSE;
    }

    ::pthread_mutex_unlock(&mTSFMutex);

    MY_LOG("[%s] m_bIsVdo(%d)", __FUNCTION__, m_bIsVdo);
}

MVOID
LscMgr::onRecordingEnd()
{
    MY_LOG("[%s]", __FUNCTION__);
    ::pthread_mutex_lock(&mTSFMutex);
    m_bIsVdo = MFALSE;
