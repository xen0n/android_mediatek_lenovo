...
#define STATE_END2 5
	...
	else if(state==STATE_END)
	{
		...
		fclose(fp);
              ...
              {
                    FILE *fpFlashWB;
                    fpFlashWB = fopen("/sdcard/flashWB.txt","wt");
                    ...
                    Rgain = 0;
                    Bgain = 0;
                    for(j=0; j<testNum-1; j++)
                    {
                        if( (rm[j*10+5] !=0) && (bm[j*10+5] !=0))
                        {
                            Rgain = (gm[j*10+5]*512)/rm[j*10+5];
                            Bgain = (gm[j*10+5]*512)/bm[j*10+5];
                        }
                        else
                        {
                            Rgain = 0;
                        ...
                        if(j==0)
                        {
                             fprintf(fpFlashWB,"//{%d, 512, %d}, //duty=%d, step=%d \n", Rgain, Bgain, dutyArr[j], stepArr[j]);
                        }
                        else
                        {
                            fprintf(fpFlashWB,"{%d, 512, %d}, //duty=%d, step=%d \n", Rgain, Bgain, dutyArr[j], stepArr[j]);
                    ...
                    /*
                        ...
                        {
                            fprintf(fpFlashWB,"Error calibration data!!duty=%d, step=%d\n", dutyArr[j], stepArr[j]);
                        }
                        else
                        {
                            avg_Rgain = (G_Sum*512)/R_Sum;
                            avg_Bgain = (G_Sum*512)/B_Sum;

                            if(j==0)
                                fprintf(fpFlashWB,"//{%d, 512, %d}, //duty=%d, step=%d \n", avg_Rgain, avg_Bgain, dutyArr[j], stepArr[j]);
                            else
                                fprintf(fpFlashWB,"{%d, 512, %d}, //duty=%d, step=%d \n", avg_Rgain, avg_Bgain, dutyArr[j], stepArr[j]);
                        }
                        R_Sum = 0;
                    ...
                    */
                    ...
                    if(testNum < FLASH_DUTY_NUM)
                    {
                        for(i=testNum-1; i<=FLASH_DUTY_NUM; i++)
                            fprintf(fpFlashWB,"{%d, 512, %d}, //duty=%d, step=%d \n", Rgain, Bgain, dutyArr[testNum-2], stepArr[testNum-2]);
                    }
