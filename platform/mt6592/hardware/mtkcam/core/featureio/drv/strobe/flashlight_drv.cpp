...
*
********************************************************************************/
int FlashlightDrv::init(unsigned long sensorDev, int lowBatDuty)
{
	DRV_DBG("init line=%d lowBatDuty=%d",__LINE__,lowBatDuty);
    int err = 0;
    ...
    if (mUsers == 0 && bOpenTest==1)
    {
        if (m_fdSTROBE == -1)
        {
            ...
            if(lowBatDuty!=-1)
            {
                mLowBatVDuty=lowBatDuty;
                err = ioctl(m_fdSTROBE,FLASH_IOC_LOW_POWER_DUTY,lowBatDuty);
                if (err < 0)
                {
                    DRV_ERR("FLASH_IOC_LOW_POWER_DUTY error\n");
                    return err ;
                }
            }
            m_isOn=0;
...
int FlashlightDrv::initTemp(unsigned long sensorDev, int lowBatDuty)
{
	DRV_DBG("initTemp line=%d lowBatDuty=%d",__LINE__,lowBatDuty);
    int err = 0;
    unsigned long flashlightIdx = 0; //default dummy driver
...
int FlashlightDrv::lowPowerProtectStart()
{
	int hasLPD;
	hasLowPowerDetect(&hasLPD);
	if(hasLPD==0)
	...
	DRV_DBG("lowPowerProtectStart()");
	int err = 0;
	if (m_fdSTROBE < 0)
	{
	    DRV_DBG("lowPowerProtectStart() m_fdSTROBE < 0\n");
	    return StrobeDrv::STROBE_UNKNOWN_ERROR;
	}
	err = ioctl(m_fdSTROBE,FLASH_IOC_LOW_POWER_PROTECT_START,0);
	if (err < 0)
	{
	    DRV_ERR("lowPowerProtectStart() err=%d\n", err);
	    return err;
	}
	return 0;
}

int FlashlightDrv::lowPowerProtectEnd()
{
	int hasLPD;
	hasLowPowerDetect(&hasLPD);
	if(hasLPD==0)
	...
	DRV_DBG("FLASH_IOC_LOW_POWER_PROTECT_END()");
	int err = 0;
	if (m_fdSTROBE < 0)
	{
	    DRV_DBG("FLASH_IOC_LOW_POWER_PROTECT_END() m_fdSTROBE < 0\n");
	    return StrobeDrv::STROBE_UNKNOWN_ERROR;
	}
	err = ioctl(m_fdSTROBE,FLASH_IOC_LOW_POWER_PROTECT_END,0);
	if (err < 0)
	{
	    DRV_ERR("FLASH_IOC_LOW_POWER_PROTECT_END() err=%d\n", err);
	    return err;
	}
	return 0;
}
