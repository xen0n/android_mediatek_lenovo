...
* 
********************************************************************************/
MBOOL  IspDrvImp::ClockOverDriveEnable(MUINT32 EnableLevel)  //For ISP CLOCK Overdrive enable
{
    MINT32 Ret;
    ISP_REG_IO_STRUCT IspRegIo;
    
    LOG_INF("EnableLevel(%d)\n",EnableLevel);
    
    Ret = ioctl(mFd, ISP_CLK_OD_EN, &EnableLevel);
    if(Ret < 0)
    {
        LOG_ERR("ISP_CLK_OD_EN fail(%d)",Ret);
        return MFALSE;
    }
