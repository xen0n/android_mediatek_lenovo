...
MBOOL ResMgrDrvImp::SetMode(RES_MGR_DRV_MODE_STRUCT* pMode)
{
    ...
    //
    if(pMode->ScenSw == RES_MGR_DRV_SCEN_SW_VIDEO_REC_HS)
    {
        mIsHsVr = MTRUE;
    }
    //
    LOG_MSG("ScenSw(%d),ScenHw(%d),Dev(%d),HSVR(%d)",
            pMode->ScenSw,
            pMode->ScenHw,
            pMode->Dev,
            mIsHsVr);
    ...
    if(pMode->ScenSw == RES_MGR_DRV_SCEN_SW_VIDEO_REC_HS)
    {
        pMode->ScenSw = RES_MGR_DRV_SCEN_SW_VIDEO_REC;
    }
    //
    GetMode(&CurrMode);
    //
    ...
    if(Result)
    {
        ...
        else
        {
            switch(CurrMode.ScenSw)
            {
                ...
                {
                    if( mIsHsVr &&
                        CurrMode.ScenSw == RES_MGR_DRV_SCEN_SW_VIDEO_REC)
                    {
