...
MINT32 SensorHalImp::setConf(halSensorIFParam_t halSensorIFParam[2])
{
    ...
    if(mSensorDev & SENSOR_DEV_MAIN || mSensorDev & SENSOR_DEV_ATV) {
        ...
        if(sensorCropInfo[0].u4CropW*sensorCropInfo[0].u4CropH > DVFS_SIZE) {//DVFS from 296MHz to 400MHz ~4/3
            sensorInfo[0].MIPIDataLowPwr2HighSpeedSettleDelayCount = (sensorInfo[0].MIPIDataLowPwr2HighSpeedSettleDelayCount<<2)/3;
        }
    ...
    if(mSensorDev & SENSOR_DEV_SUB ) {
        ...
        if(sensorCropInfo[1].u4CropW*sensorCropInfo[1].u4CropH > DVFS_SIZE) {//DVFS from 296MHz to 400MHz ~4/3
            sensorInfo[1].MIPIDataLowPwr2HighSpeedSettleDelayCount = (sensorInfo[1].MIPIDataLowPwr2HighSpeedSettleDelayCount<<2)/3;
        }
...
MINT32 SensorHalImp::initCSI2Peripheral(MINT32 initCSI2)
{
    ...
    if((mSensorDev & SENSOR_DEV_MAIN ) || (mSensorDev & SENSOR_DEV_ATV )){
        if(sensorInfo[0].SensroInterfaceType == SENSOR_INTERFACE_TYPE_MIPI) {   // enable peripheral
            ...
            if(initCSI2) {
                ret = pSeninfDrv->initTg1CSI2(SENSOR_INTERFACE_TYPE_MIPI,1);
                if (ret < 0) {
                    LOG_ERR("init Tg1 CSI2 peripheral fail\n");
                }
            } else {
                ret = pSeninfDrv->initTg1CSI2(SENSOR_INTERFACE_TYPE_MIPI,0);
                if (ret < 0) {
            ...
            ret = pSeninfDrv->initTg1CSI2(SENSOR_INTERFACE_TYPE_PARALLEL,0);
            if (ret < 0) {
                LOG_ERR("uninit Tg1 CSI2 peripheral fail\n");
            }
        }
...
MINT32 SensorHalImp::querySensorInfo()
{
    ...
    LOG_MSG("[querySensorInfo] mSensorDev = SENSOR_NONE\n");
