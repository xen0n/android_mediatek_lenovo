...
//        *pCMMCLKReg = ( (*pCMMCLKReg)&(~0x1C0) ) | 0x40;
   ...
   //set CMMCLK mode 1
    ...
    temp = *(mpSeninfHwRegAddr + (0x8010/4) );
    mt65xx_reg_writel((((temp&0xFFFF0FFF)|0x80000000)), mpSeninfHwRegAddr + (0x8010/4));       

      //turn on nCSI2 first
    //temp = *(mpSeninfHwRegAddr + (0x8010/4) );
    //mt65xx_reg_writel(((temp&0xFFFF0FFF)|0x80008000), mpSeninfHwRegAddr + (0x8010/4));       

    //mt65xx_reg_writel(0x10, mpSeninfHwRegAddr + (0x8634/4));      //ncsi2 force clock
  
    //temp = *(mpSeninfHwRegAddr + (0x8118/4) );//csi2 force clock
    //mt65xx_reg_writel(temp|0x8000000, mpSeninfHwRegAddr + (0x8118/4));  

    //efuse setting
    /* ----------------------------------- 
     Read Efuse data                   
    If offset 0x0184[31:28] !=0 
    RG_CSI0_LNRC_RT_CODE[3:0] = 0x0184[31:28]
    If offset 0x0184[27:24] !=0 
    RG_CSI0_LNRD0_RT_CODE[3:0] = 0x0184[27:24]
    If offset 0x0184[23:20] !=0 
    RG_CSI0_LNRD1_RT_CODE[3:0] = 0x0184[23:20]
    If offset 0x0184[19:16] !=0 
    RG_CSI0_LNRD2_RT_CODE[3:0] = 0x0184[19:16]
    If offset 0x0184[15:12] !=0 
    RG_CSI0_LNRD3_RT_CODE[3:0] = 0x0184[15:12]
  
     ----------------------------------- */     
    temp = *(mpEfuseRegAddr + (0x184/4) );
    if ((temp & 0xF0000000) != 0x0) {
        temp1 = ((temp & 0xF0000000)>>28)<<12;
        temp2 = *(mpCSI2RxAnalogRegAddr);
        mt65xx_reg_writel(((temp2&0xFFFF0FFF)|temp1), mpCSI2RxAnalogRegAddr );
    }

    if ((temp & 0xF000000) != 0x0) {
        temp1 = ((temp & 0xF000000)>>24)<<12;
        temp2 = *(mpCSI2RxAnalogRegAddr + (0x4/4));
        mt65xx_reg_writel(((temp2&0xFFFF0FFF)|temp1), mpCSI2RxAnalogRegAddr + (0x4/4));
    }
    
    if ((temp & 0xF00000) != 0x0) {
        temp1 = ((temp & 0xF00000)>>20)<<12;
        temp2 = *(mpCSI2RxAnalogRegAddr + (0x8/4));
        mt65xx_reg_writel(((temp2&0xFFFF0FFF)|temp1), mpCSI2RxAnalogRegAddr + (0x8/4));
    }

    if ((temp & 0xF0000) != 0x0) {
        temp1 = ((temp & 0xF0000)>>16)<<12;
        temp2 = *(mpCSI2RxAnalogRegAddr + (0xC/4));
        mt65xx_reg_writel(((temp2&0xFFFF0FFF)|temp1), mpCSI2RxAnalogRegAddr + (0xC/4));
    }
    
    if ((temp & 0xF000) != 0x0) {
        temp1 = (temp & 0xF000);
        temp2 = *(mpCSI2RxAnalogRegAddr + (0x10/4));
        mt65xx_reg_writel(((temp2&0xFFFF0FFF)|temp1), mpCSI2RxAnalogRegAddr + (0x10/4));
    }        
    android_atomic_inc(&mUsers);
...
int SeninfDrvImp::uninit()
{
    ...
    if (mUsers == 0) {
        ...
        *(mpGpioHwRegAddr + (0x780/4)) &= 0xFE3F;   
        ...
        result = m_pIspDrv ->ClockOverDriveEnable(0);//disable DVFS
        //
...
)
{
    ...
    SENINF_WRITE_REG(pSeninf, SENINF_TOP_CTRL, temp|0xC00);
...
)
{
    ...
    if((pixelEnd - pixelStart) * (lineEnd - lineStart)  > DVFS_SIZE) {
        m_pIspDrv ->ClockOverDriveEnable(1);
        LOG_MSG("[setTg1GrabRange] enable DVFS \n");
    }
    else {
        m_pIspDrv ->ClockOverDriveEnable(0);  
        LOG_MSG("[setTg1GrabRange] disable DVFS \n");      
    }
...
*
********************************************************************************/
int SeninfDrvImp::initTg1CSI2(int interfacetype, int csi2_en)
{
	seninf_reg_t *pSeninf = (seninf_reg_t *)mpSeninfHwRegAddr;
    int ret = 0;
    unsigned int temp = 0;
    ACDK_SENSOR_GPIO_STRUCT mpgpio;


    LOG_MSG("[initCSI2]:type =%d, enable = %d\n",(int)interfacetype, (int)csi2_en);
    
    mpgpio.mSensorDev = mDevice;
    mpgpio.GpioEnable = csi2_en;
    mpgpio.SensroInterfaceType = (ACDK_SENSOR_INTERFACE_TYPE_ENUM)interfacetype;
    ...
    // Sub cam support MIPI & parallel interface 
	if(csi2_en == 0) {
        //disable mipi BG
        temp = *(mpCSI2RxAnalogRegAddr + (0x24/4));//RG_CSI_BG_CORE_EN       
       ...
       mt65xx_reg_writel(temp&0xFFFFFFFE, mpCSI2RxConfigRegAddr + (0x38/4)); 
        ...
        if(mMipiType == 0) { //MIPI_OPHY_NCSI2     
            ...
            mt65xx_reg_writel((temp&0xFFFFFFFC)|0x43, mpCSI2RxAnalogRegAddr + (0x20/4));                 
        ...
        temp = *(mpCSI2RxAnalogRegAddr + (0x20/4));//RG_CSI0_4XCLK_DISABLE = 1,RG_CSI0_LNRD_HSRX_BCLK_INVERT = 1        
            mt65xx_reg_writel(temp|0x00000002, mpCSI2RxAnalogRegAddr + (0x20/4)); 
...
)
{
    ...
    if(csi2_en == 1) {  // enable NCSI2
        ...
        SENINF_WRITE_REG(pSeninf, SENINF1_CSI2_INTEN, temp&(~0xF));
        //CSI2 IRQ Read Clear
        ...
        if(csi2_en == 1) {  // enable CSI2
            ...
            //SENINF_WRITE_REG(pSeninf, SENINF1_CSI2_CTRL, temp&0xFFFFFFF0);    // disable CSI2 first
            ...
            temp = SENINF_READ_REG(pSeninf, SENINF1_CSI2_INTSTA); 
            SENINF_WRITE_REG(pSeninf, SENINF1_CSI2_INTSTA, temp|0x20);            
            //NCIS2 Disable IRQ
            SENINF_WRITE_REG(pSeninf, SENINF1_NCSI2_INT_EN,0x00000000);
    
            temp = SENINF_READ_REG(pSeninf, SENINF1_CSI2_CTRL);
            temp &= 0xFFFFFFF0;
            SENINF_WRITE_REG(pSeninf, SENINF1_CSI2_CTRL,(temp |((dataFlow<<17)| (dataheaderOrder<<5) | (1<<4) | (((1<<dlane_num)-1)<<1) | (csi2_en<<0))));
            ...
            //temp = SENINF_READ_REG(pSeninf, SENINF1_CSI2_CTRL);
            //SENINF_WRITE_REG(pSeninf, SENINF1_CSI2_CTRL, temp&0xFFFFFFF0);
            temp = SENINF_READ_REG(pSeninf, SENINF1_CSI2_CTRL);        
...
#endif
    //Enable Sensor IF IRQ
    SENINF_WRITE_REG(pSeninf, SENINF1_INTEN, 0x00000001);
...
int SeninfDrvImp::autoDeskewCalibration_csi2()
{
    int ret = 0;
    unsigned int temp = 0;
    MUINT32 lane_num=0,min_lane_code=0;
    ...
    int k =0;
    LOG_MSG("autoDeskewCalibration start \n");
    temp = SENINF_READ_REG(pSeninf,SENINF1_CSI2_DBG); // force enable clk lane  
    SENINF_WRITE_REG(pSeninf,SENINF1_CSI2_DBG, temp|0x08000000); // bit 27 : enable clk lane
    ...
    // CSI2 IRQ Enable
    SENINF_WRITE_REG(pSeninf,SENINF1_CSI2_INTEN,0x00000007);
    SENINF_WRITE_REG(pSeninf,SENINF1_CSI2_INTSTA,0x0000007f);
 
    //@add check default if any interrup error exist, if yes, debug and fix it first. if no, continue calibration 
    //@add print ecc & crc error status
    if((SENINF_READ_REG(pSeninf,SENINF1_CSI2_INTSTA)&0x07)!= 0){
        LOG_ERR("autoDeskewCalibration Line %d, default input has error, please check it \n",__LINE__);
    }
    LOG_MSG("autoDeskewCalibration start interupt status = 0x%x\n",SENINF_READ_REG(pSeninf,SENINF1_CSI2_INTSTA));
    SENINF_WRITE_REG(pSeninf,SENINF1_CSI2_INTSTA,0x7F);
    //Fix Clock lane
    //set lane 0
    temp = *(mpCSI2RxAnalogRegAddr + (0x04/4));//enable data lane 0 delay    
    mt65xx_reg_writel(temp|0x1000000, mpCSI2RxAnalogRegAddr + (0x04/4));     
    ...
    for(i=0; i<=0xF; i++) {
        lane0_code = i;
        currPacket = SENINF_READ_REG(pSeninf,SENINF1_CSI2_INTEN)& 0xffffff00;
        //LOG_MSG("currPacket = 0x%x\n",currPacket);
        //@add read interrupt status to clear
        temp = *(mpCSI2RxAnalogRegAddr + (0x04/4));//set to 0 first    
        mt65xx_reg_writel(temp&0xE1FFFFFF, mpCSI2RxAnalogRegAddr + (0x04/4));           
        temp = *(mpCSI2RxAnalogRegAddr + (0x04/4));    
        ...
        temp = SENINF_READ_REG(pSeninf,SENINF1_CSI2_CTRL);   
        SENINF_WRITE_REG(pSeninf,SENINF1_CSI2_CTRL, temp|0x00001000); // bit 12 : sw rst  
        usleep(5);
        SENINF_WRITE_REG(pSeninf,SENINF1_CSI2_CTRL, temp&(~0x00001000)); // bit 12 : sw rst 
        usleep(5);
        //LOG_MSG("SENINF1_CSI2_INTEN = 0x%x\n",SENINF_READ_REG(pSeninf,SENINF1_CSI2_INTEN));
        while(((SENINF_READ_REG(pSeninf,SENINF1_CSI2_INTEN))&0xffffff00)==0){};
         
         if((SENINF_READ_REG(pSeninf,SENINF1_CSI2_INTSTA)&0x07)!= 0) {
            SENINF_WRITE_REG(pSeninf,SENINF1_CSI2_INTSTA,0x1f);
            usleep(5);
            if((SENINF_READ_REG(pSeninf,SENINF1_CSI2_INTSTA)&0x07)!= 0) { //double confirm error happen
                break;
            }
         }
    }
