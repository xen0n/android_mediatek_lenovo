...
#if 0
    /*lenovo sw linrq1 20140320 add vibrate on boot begin*/
    upmu_set_rg_vibr_vosel(0x7);
    upmu_set_rg_vibr_en(0x1);
    udelay(150000); //0.15secs
    upmu_set_rg_vibr_vosel(0x4);
    upmu_set_rg_vibr_en(0x0);
    /*lenovo sw linrq1 20140320 add vibrate on boot end*/
#endif
...
DISP_STATUS DISP_PowerEnable(BOOL enable)
{
  ...
  if (!enable && lcm_drv && lcm_drv->suspend_power)
  {
