...
/*lenovo-sw chuyq 2014.4.22 add begin*/
    /*    ///show_logo(0);
        init_fb_screen();
        if(g_boot_mode == FACTORY_BOOT)
        fill_animation_logo(39, mt_get_fb_addr(), mt_get_tempfb_addr(), logo_addr, phical_screen);
        else if(g_boot_mode == META_BOOT)
        fill_animation_logo(40, mt_get_fb_addr(), mt_get_tempfb_addr(), logo_addr, phical_screen);
        else
        ...
        show_logo(0);
        mt_disp_update(0, 0, CFG_DISPLAY_WIDTH, CFG_DISPLAY_HEIGHT);
/*lenovo-sw chuyq 2014.3.22 add end*/
    }
