...
kal_bool is_low_battery(kal_uint32 val)
{
    ...
    kal_uint32 chrg_status;

    if(0 == val)
        val = get_i_sense_volt(5);
    #else
    if(0 == val)
        val = get_bat_sense_volt(5);
    #endif
    ...
    {
        #ifdef MTK_BQ24196_SUPPORT
        chrg_status = bq24196_get_chrg_stat();
        dprintf(INFO, "bq24196_chrg_status = %d\n", chrg_status);
    
        if(chrg_status == 0x1) //Pre-charge
        {
        ...
        #ifdef MTK_BQ24296_SUPPORT
        chrg_status = bq24296_get_chrg_stat();
...
#ifdef MTK_BQ24196_SUPPORT
	bq24196_hw_init();
	bq24196_charging_enable(KAL_FALSE);  //disable charging with power path
	bq24196_dump_register();
#endif

#ifdef MTK_BQ24296_SUPPORT
	bq24296_hw_init();
	bq24296_charging_enable(KAL_FALSE);  //disable charging with power path
