...
//        *((V_UINT32P)(EMI_BASE   +0x0078))=0x23110003;// ???????????0x00030F4d;
  ...
  if((*((volatile unsigned int *)(0x10206174))& 0x00000800)!=0)
      ...
      *((volatile unsigned int *)(DRAMC0_BASE + 0x01C0)) |= 0x00000000;
        ...
        *((V_UINT32P)(DRAMC0_BASE + 0x0088)) = emi_setting->LPDDR3_MODE_REG_3;    //Dram Driving : 34.3-ohm  //Edward: modify default driving to 40ohm (1->2)?
        *((V_UINT32P)(DDRPHY_BASE + 0x0088)) = emi_setting->LPDDR3_MODE_REG_3;    //Dram Driving : 34.3-ohm
...
return index;
    ...
    if (!(found))
    {
        ...
        if ((0 == found) && (1 == enable_combo_dis))
        {
            ...
            print("[EMI] Discrete dram first init timing\n");
            print_DBG_info();
#endif
            if (dramc_calib() < 0) {
...
int DFS_ability_detection(void)
{
  ...
  if (DDR_TYPE == TYPE_LPDDR3){
    ...
    if((rank0_coarse <=20 && rank1_coarse <=20)&&((*((volatile unsigned int *)(0x10206174))& 0x00000800)==0))
     {
...
else if ((emi_set->type & 0xF) == TYPE_LPDDR3)
{
    ...
    if((*((volatile unsigned int *)(0x10206174))& 0x00000800)!=0)
    {
        mt_mempll_init(1466,1);//0x10206174[1]=1 means Argo-BI
    }
    else
    {
...
#ifdef DRAMC_DEBUG
//print Vcore Vmem Driving DQS Setting
//print Vcore Vmem Setting
int ret_val=0;
unsigned int OldVcore1=0;
unsigned int OldVcore2=0;
unsigned int OldVmem1=0;
...
print("[Vcore]0x6B=0x%x,\r\n[Vmem]0x81=0x%x \r\n",OldVcore1,OldVmem1);

ret_val=mt6333_read_interface(0x6C,&OldVcore2,0x7F,0);
ret_val=mt6333_read_interface(0x82,&OldVmem2,0x7F,0);
print("[Vcore]0x6C=0x%x,\r\n[Vmem]0x82=0x%x \r\n",OldVcore2,OldVmem2);

//print Driving
print("[Driving]0x%x \r\n",emi_set->LPDDR2_MODE_REG_3);

//print DQS Setting
unsigned int Rank0_DQS;
unsigned int Rank1_DQS;
Rank0_DQS=DRAMC_READ_REG(DRAMC_R0DELDLY);
Rank1_DQS=DRAMC_READ_REG(DRAMC_R1DELDLY);
print("[MTK DEBUG]Original DRAMC_R0DELELY:0x%x\n",Rank0_DQS);
print("[MTK DEBUG]Original DRAMC_R1DELELY:0x%x\n",Rank1_DQS);
print_DQS_DQ_tx_delay();
//print Vcore Vmem Driving DQS Setting
#endif
...
void print_DQS_DQ_tx_delay(void)
{
  ...
  for(DQSbit=0;DQSbit<4;DQSbit++)
  {
    ...
    for(DQbit=0;DQbit<8;DQbit++)
    {
      ...
      print(" DQ%d_OUTDLY:0x%x",(DQSbit<<3)+DQbit,(DRAMC_READ_REG(DRAMC_DQODLY1+(DQSbit<<2))& (0xf<<(DQbit<<2))) >> (DQbit<<2));
      if(DQbit%4==3)
      {
        print("\n");
      }
