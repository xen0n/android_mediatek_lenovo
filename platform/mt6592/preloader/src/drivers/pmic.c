...
{    
    ...
    pmic_read_interface( (U32)(STRUP_CON8), &just_rst, PMIC_JUST_PWRKEY_RST_MASK, PMIC_JUST_PWRKEY_RST_SHIFT);              
    printf("INT_MISC_CON: %d  TOP_RST_MISC: %d just_rst = %d\n", val1, val2, just_rst);
    if(val1 && !val2 && !just_rst)
...
int hw_check_battery(void)
{
    ...
    {                     
		...
		print("[0x%x]=0x%x\n",CHR_CON7,reg_val);
//lenovo_sw liaohj modify for temp no ntc dectec 2014-03-10           
        //return 0; 
        return 1;       
    }
    else
