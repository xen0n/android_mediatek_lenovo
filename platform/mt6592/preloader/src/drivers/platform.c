...
static void get_dram_rank_info(u32 *rank_num, u32 rank_size[])
{
    u32 rank_nr = (u32)get_dram_rank_nr();
    ...
    *rank_num = rank_nr;
...
#if CFG_TRUSTONIC_TEE_SUPPORT
    {
        ...
        tee_get_secmem_size(&secmem_size);

        for (i = rank_nr - 1; i >= 0; i--) {
            if (secmem_size >= rank_size[i]) {
                secmem_size -= rank_size[i];
                rank_size[i] = 0;
            } else {
                rank_size[i] -= secmem_size;
                break;
            }
        }
    }
...
void platform_set_boot_args()
{
    ...
    get_dram_rank_info(&bootarg.dram_rank_num, bootarg.dram_rank_size);
...
static boot_reason_t platform_boot_status(void)
{
    ...
    if (rtc_2sec_reboot_check()) {
     ...
     /*special case when WDT boot fail three time*/

    if( *(u32*)RAM_CONSOLE_ADDR == (u32)WDT_RESET_MAGIC_NUM  ){
        print("%s WDT trigger WDT+PMIC Reset!\n", MOD);
        /* Because we use ram console area to pass parameter 
           We need fill back ram console signature*/
        *(volatile u32*)RAM_CONSOLE_ADDR = (u32)RAM_CONSOLE_SIG;
        rtc_mark_bypass_pwrkey();
        return BR_POWER_KEY;
    }
...
bool platform_com_wait_forever_check(void)
{
    ...
    if (TRUE == mtk_detect_key(COM_WAIT_KEY)) {
        print("%s COM handshake timeout force disable: %s\n", MOD, "Key");
        return TRUE;
    }
#endif

#ifdef USBDL_DETECT_VIA_AT_COMMAND
    print("%s COM wait forever check\n", MOD);
