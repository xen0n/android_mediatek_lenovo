...
int part_load(blkdev_t *bdev, part_t *part, u32 *addr, u32 offset, u32 *size)
{
    ...
    u32 dsize = 0, maddr = 0, mode = 0, mem_size, mmap_size;
    u32 ms;
    ...
    if (maddr == PART_HEADER_MEMADDR) {
        ...
        mem_size  = platform_memory_size();
        mmap_size = (u32)0UL - (u32)CFG_DRAM_ADDR;

        /* load address > memory mapping address, then adjust load address 
         * into memory mapping region. note that header's load address should
         * consider if in the memory mapping region and specified into proper 
         * address is recommendded.
         */
        if ((mem_size - maddr) > mmap_size) {
            maddr = mmap_size - maddr + CFG_DRAM_ADDR;
        } else {
        	maddr = mem_size - maddr + CFG_DRAM_ADDR;
    	}
    }
