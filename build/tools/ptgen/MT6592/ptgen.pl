...
sub InitGlobalValue
{
    ...
    $ArgList{TRUSTONIC_TEE_SUPPORT}      = $ENV{TRUSTONIC_TEE_SUPPORT};
    ...
    $ArgList{PERSIST_PARTITION_SUPPORT}	 = "yes";#$ENV{MTK_PERSIST_PARTITION_SUPPORT};
...
sub ProcessRawPartitionLayoutData
{
    ...
    for ($partition_idx = 0 ; $partition_idx < @partition_layout_process ; $partition_idx++)
    {
        ...
        {
            if ($ArgList{TRUSTONIC_TEE_SUPPORT} ne "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
