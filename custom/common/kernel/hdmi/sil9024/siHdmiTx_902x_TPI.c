...
void ReadBlockTPI (byte TPI_Offset, word NBytes, byte * pData)
{
	...
	//printk("[9024]RegOffset=i=0x%x,pData[i-TPI_Offset]=0x%x \n", i, pData[i-TPI_Offset]);
		}
	return ;
}
...
void WriteBlockTPI (byte TPI_Offset, word NBytes, byte * pData)
{
	...
	//printk("[9024]RegOffset=0x%x,pData[i-TPI_Offset]=0x%x \n", i, pData[i-TPI_Offset]);
		}
...
void HDCP_Init (void)
{
	...
	ReadModifyWriteTPI(0x3E_EN, KSV_FIFO_READY_EN_MASK, KSV_FIFO_READY_ENABLE);
#endif
...
void HDCP_CheckStatus (byte InterruptStatusImage)
{
	...
	if ((g_hdcp.HDCP_TxSupports == TRUE) && (g_hdcp.HDCP_AksvValid == TRUE))
	{
			...
			if ((QueryData & PROTECTION_TYPE_MASK)&&(!g_hdcp.HDCP_Started))   // Is HDCP avaialable
			{
				HDCP_On();
			}
				...
				switch (g_hdcp.HDCP_LinkProtectionLevel)
				{
					...
					case LOCAL_LINK_PROTECTION_SECURE:	
						...
						if (IsHDMI_Sink())
							{
								DelayMS(500);
								ReadModifyWriteTPI(0x26, AUDIO_MUTE_MASK, AUDIO_MUTE_NORMAL);
							}
						break;
...
byte InitVideo (byte TclkSel)
{
	...
	{
		case 0:  temp = 0x00; ReadModifyWriteTPI(0x40, BIT_2, 0x00); break;
		case 1:  temp = 0x80; ReadModifyWriteTPI(0x40, BIT_2, BIT_2); break;
		case 2:  temp = 0xC0; ReadModifyWriteTPI(0x40, BIT_2, BIT_2); break;
		case 3:  temp = 0x40; ReadModifyWriteTPI(0x40, BIT_2, BIT_2); break;
		default: temp = 0x00; ReadModifyWriteTPI(0x40, BIT_2, 0x00); break;
...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	if ((g_hdcp.HDCP_TxSupports == TRUE) && (g_hdcp.HDCPAuthenticated == VMD_HDCP_AUTHENTICATED)&&(Sii9024A_HDCP_supported))
	{
		if (g_hdcp.HDCP_AksvValid == TRUE)
		{
			...
			ReadModifyWriteTPI(0x1A, LINK_INTEGRITY_MODE_MASK | TMDS_OUTPUT_CONTROL_MASK | AV_MUTE_MASK,
...
// Globals: none
//------------------------------------------------------------------------------
BOOL HPD_Status_Check(void)
{
	if ((g_sys.hdmiCableConnected == TRUE) && (g_sys.dsRxPoweredUp == TRUE))
	{
		return TRUE;
	}
	else
