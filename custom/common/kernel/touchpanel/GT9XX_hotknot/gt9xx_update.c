...
static u8 gup_enter_update_judge(st_fw_head *fw_head)
{
    ...
    {
        update_msg.fw_total_len = 42*1024;
        //TODO flashless equals another length
    }
    else
    {
        u32 len = fw_head->hw_info[3];
        len += (((u32)fw_head->hw_info[2])<<8);
        len += (((u32)fw_head->hw_info[1])<<16);
        len += (((u32)fw_head->hw_info[0])<<24);
        update_msg.fw_total_len = len;
    }
    GTP_INFO("Firmware total length:%d", update_msg.fw_total_len);
    if(update_msg.fw_total_len < 36*1024 || update_msg.fw_total_len > 128*1024)
    {
        GTP_ERROR("Invalid firmware length,exit update.");
        return FAIL;
    }
...
s32 gup_load_main_system(char *filepath)
{
    ...
    {
        firmware = gtp_default_FW_fl;
		GTP_INFO("[load_main_system]Use default firmware.");
    }
	else
	{
	    is_load_from_file = 1;
