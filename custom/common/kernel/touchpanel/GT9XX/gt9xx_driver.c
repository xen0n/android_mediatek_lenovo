...
//static u8 slide_doze_enabled = 0;
#endif
...
static int get_area_limit(void);
#endif
...
s32 gtp_i2c_read(struct i2c_client *client, u8 *buf, s32 len)
{
    ...
    {
    #if GTP_SLIDE_WAKEUP
       if (DOZE_ENABLED == doze_status)// if (slide_doze_enabled)
        {
            return ret;
        }
    #endif
...
{
unsigned int tp_version =0x00;
 tp_version = tpd_info.mid;
 printk("[TSP-xw]:tp_fw_version is 0x%08x\n",tp_version);
 return(tp_version);
}
/*
...
static int get_area_limit(void)
{
  tpd_limit ->min_limit = 30;
  tpd_limit ->max_limit = 300;
...
static s32 gtp_init_panel(struct i2c_client *client)
{
    ...
    u8 opr_buf[16];
    u8 sensor_id = 0;
    u8 cfg_info_group[] = GT9157_CTP_CFG_GROUP1;
    u8 cfg_info_group1[] = GT9158_CTP_CFG_GROUP1;
    if (tpd_info.pid == 0x9157)
    memcpy(cfg_info_group1,cfg_info_group,sizeof(cfg_info_group1));
...
static s8 gtp_i2c_test(struct i2c_client *client)
{
    ...
    while (retry++ < 5)
    {
        ...
        if ((!ret) && (hw_info == 0x00900600))              //20121212 //9157 need to match a new hw_info by xuwen1
        {
            return ret;
        }
...
static u8 gtp_bak_ref_proc(struct i2c_client *client, u8 mode)
{
   ...
   if (RECOVERY_BOOT !=  g_boot_mode)
	...
	if((RECOVERY_BOOT == g_boot_mode) || (gtp_ref_retries >= GTP_CHK_FS_MNT_MAX))
    {
...
/*lenovo-sw xuwen1 add 20140109 for area touch end*/
   ...
   if (FACTORY_BOOT == g_boot_mode || RECOVERY_BOOT == g_boot_mode)
       ...
       if (DOZE_ENABLED == doze_status)// if (slide_doze_enabled)
        {
            ...
            if (ret > 0)
            {    
                printk("[xw]%s, lpwg_int_flag = %d.\n", __func__,lpwg_int_flag);   
                if ((doze_buf[2] == 0xAA)&& (lpwg_int_flag == 1))
                {
                    GTP_INFO("Slide To Light up the screen!");
			doze_status = DOZE_WAKEUP;
/*begin, lenovo.sw zengzm 2014.02.28 added: for bug STEED-939 */
/*
                    ...
                    input_sync(tpd->dev);
*/
/*end, lenovo.sw zengzm 2014.02.28 added: for bug STEED-939 */
			/*lenovo-sw xuwen1 add 20131129 for slide-wakeup begin*/
			 //input_report_key(tpd->dev, KEY_SLIDE, 1);
					//
                    input_sync(tpd->dev);
                    //input_report_key(tpd->dev, KEY_SLIDE, 0);
                    //input_sync(tpd->dev);	
			/*lenovo-sw xuwen1 add 20131129 for slide-wakeup end*/					
                   
			//slide_doze_enabled = 0;
                    // clear 0x814B
                    doze_buf[2] = 0x00;
			lpwg_int_flag = 0;
                    gtp_i2c_write(i2c_client_point, doze_buf, 3);
                }
              else  if ((doze_buf[2] == 0xBB)&& (lpwg_int_flag == 1))
                {
                    GTP_INFO("Slide To Light up the screen!");
		      doze_status = DOZE_WAKEUP;
/*begin, lenovo.sw zengzm 2014.02.28 added: for bug STEED-939 */
...
static s8 gtp_enter_doze(struct i2c_client *client)
{
    ...
    while(retry++ < 5)
    {
       if(gtp_chip_type == CHIP_TYPE_GT9F)
        {
          /*  ret = gup_load_gesture_system();
            if(FAIL == ret)
            {
                GTP_ERROR("Load gesture 4k subsystem failed,retry:%d", retry);
                continue;
            }
...
{
#if GTP_COMPATIBLE_MODE
    if ((CHIP_TYPE_GT9F == gtp_chip_type)&& (RECOVERY_BOOT != g_boot_mode))
    {
...
static s8 gtp_wakeup_sleep(struct i2c_client *client)
{
    ...
    if (CHIP_TYPE_GT9F == gtp_chip_type)
    {
        ...
        while (retry++ < 3)
        {
            ...
            {
                GTP_DEBUG("Power key wakeup slide sleep.");
                recovery_type = 1;
            }
            ret = gup_recovery_gesture_system(recovery_type);
            if(FAIL == ret)
            {
                GTP_ERROR("recovery gesture system failed,retry:%d", retry);
                continue;
            }
