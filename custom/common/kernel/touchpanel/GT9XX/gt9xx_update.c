...
s32 gup_load_gesture_system(void)
{
    s32 ret = -1;
    char *firmware = NULL;
    u32 length = 0;
    ...
    GTP_INFO("[load_gesture_system] Load gesture system."); 

    firmware = &gtp_default_FW_fl[0x9000 + FW_HEAD_LENGTH ];
    ...
    if (FAIL == ret)
    {
        goto load_gesture_exit;
    }
    
    ret = gup_set_ic_msg(i2c_client_point, _bRW_MISCTL__SRAM_BANK, 1);    
    if (ret <= 0)
    {
        GTP_ERROR("[load_gesture_system]select bank 1 fail.");
        goto load_gesture_exit;
    }
        
    ret = i2c_write_bytes(i2c_client_point, 0xE000, firmware, length);
    if (ret == -1)
    {
        GTP_ERROR("[load_gesture_system]download 4k gesture code fail.");
        goto load_gesture_exit;
    }    
    
    ret = gup_check_and_repair(i2c_client_point, 0xE000, firmware, length);
    if(FAIL == ret)
    {
        GTP_ERROR("[load_gesture_system]checked 4k gesture code fail.");
        goto load_gesture_exit;
    }
    
    GTP_DEBUG("load gesture code success,begin send cmd.");
    
    gup_set_ic_msg(i2c_client_point, 0x8046, 8);
    gup_set_ic_msg(i2c_client_point, 0x8040, 8);
    
    //init sw WDT
    gup_set_ic_msg(i2c_client_point, 0x8041, 0xAA);
    
    //release SS51 & DSP
    gup_set_ic_msg(i2c_client_point, 0x4180, 0x00);
    
    //check fw run status
    gup_get_ic_msg(i2c_client_point, 0x8041, rd_buf, 1);
    if(0xAA == rd_buf[0])
    {
...
s32 gup_recovery_gesture_system(u8 type)
{
    s32 ret = -1;
    char *firmware = NULL;
    u32 length = 0;
    ...
    GTP_INFO("[recovery_gesture_system] Recovery gesture system."); 
   ...
   // length = 4*1024;
    ...
    if (FAIL == ret)
    {
        GTP_ERROR("[recovery_gesture_system]Enter update mode failed.");
        goto recovery_gesture_exit;
    }
    
  /*  firmware = &gtp_default_FW_fl[0x6000 + FW_HEAD_LENGTH ];  //gavin
    
    ret = gup_set_ic_msg(i2c_client_point, _bRW_MISCTL__SRAM_BANK, 1);    
    if (ret <= 0)
    {
        GTP_ERROR("[recovery_gesture_system]select bank 1 fail.");
        goto recovery_gesture_exit;
    }
    ...
    if (ret == -1)
    {
        GTP_ERROR("[recovery_gesture_system]download 4k code fail.");
        goto recovery_gesture_exit;
    }    */
   ...
   /* ret = gup_check_and_repair(i2c_client_point, 0xE000, firmware, length);
    if(FAIL == ret)
    {
        GTP_ERROR("[recovery_gesture_system]checked FW bank fail.");
        goto recovery_gesture_exit;
    }*/
    
    if(type == 1)
    {
        firmware = &gtp_default_FW_fl[0x8000 + FW_HEAD_LENGTH ];
        
        ret = gup_set_ic_msg(i2c_client_point, _bRW_MISCTL__SRAM_BANK, 2);    
        if (ret <= 0)
        {
            GTP_ERROR("[recovery_gesture_system]select bank 2 fail.");
            goto recovery_gesture_exit;
        }
        
        ret = i2c_write_bytes(i2c_client_point, 0xC000, firmware, length);
        if (ret == -1)
        {
            GTP_ERROR("[recovery_gesture_system]download 4k dsp code fail.");
            goto recovery_gesture_exit;
        }
        
      /*  ret = gup_check_and_repair(i2c_client_point, 0xC000, firmware, length);
        if(FAIL == ret)
        {
            GTP_ERROR("[recovery_gesture_system]checked FW bank fail.");
            goto recovery_gesture_exit;
    ...
    ret = i2c_write_bytes(i2c_client_point, 0x801F, &flag, 1);
    ret = i2c_write_bytes(i2c_client_point, 0xBA25, &flag, 1);   //gavin
    ret = i2c_write_bytes(i2c_client_point, 0x8040, &flag, 1);   //gavin
    ret = gtp_fw_startup(i2c_client_point);
    
recovery_gesture_exit:
...
static u8 gup_download_fw_dsp(struct i2c_client *client, u8 dwn_mode)
{
    ...
    /*if (GTP_FL_FW_BURN == dwn_mode)   //gavin
    {
