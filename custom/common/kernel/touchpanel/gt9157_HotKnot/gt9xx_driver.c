...
#ifdef LENOVO_TP_GESTURE_REBOOT_MACHINE
...
/*Lenovo-sw yexm1 modify 20140428 begin */
#if GTP_SUPPORT_I2C_DMA
s32 i2c_dma_read(struct i2c_client *client, u16 addr, u8 *rxbuf, s32 len)
{
    int ret;
...
/*lenovo-sw xuwen1 add 20131225 for read FW & ID version begin*/
#ifdef LENOVO_READ_FW_ID
...
#endif
/*lenovo-sw xuwen1 add 20131225 for read FW & ID version end*/
...
static s32 gtp_init_panel(struct i2c_client *client)
{
    ...
    {
        u8 have_key = 0;
        if (!memcmp(&gtp_default_FW_fl[4],"950",3))  //modify wisdom is_950)
        {
...
static u8 gtp_bak_ref_proc(struct i2c_client *client, u8 mode)
{
    ...
    if (!memcmp(&gtp_default_FW_fl[4],"950",3))//modify by wisdom is_950)
    {
...
exit_ref_proc:
    ...
    if(flp && !IS_ERR(flp))  //modify by wisdom
    {
        filp_close(flp, NULL);
    }
    return ret;
...
send_main_clk:
    ...
    ret = SUCCESS;
   // return SUCCESS;   modify by wisdom
exit_clk_proc:
    if(flp && !IS_ERR(flp))  //modify by wisdom
    {
     filp_close(flp,NULL);
...
/*lenovo-sw xuwen1 add for read begin*/
#ifdef LENOVO_READ_FW_ID
 ...
 get_tpd_info();
#endif
/*lenovo-sw xuwen1 add for read end*/
...
/*lenovo-sw xuwen1 add 20140109 for area touch begin*/
    #if defined(LENOVO_AREA_TOUCH)
    input_report_abs(tpd->dev, ABS_MT_POSITION_X_W, size);
    input_report_abs(tpd->dev, ABS_MT_POSITION_Y_W, size);
    #endif
/*lenovo-sw xuwen1 add 20140109 for area touch end*/
...
static int touch_event_handler(void *unused)
{
   ...
   u8  rqst_data[3] = {(u8)(GTP_REG_RQST >> 8), (u8)(GTP_REG_RQST & 0xFF), 0};
    ...
    u8 doze_buf_double[3] = {0x81,0x4D};//lenovo-sw add for 
#endif
    ...
    //	  mutex_lock(&i2c_access);
        ...
        if (DOZE_ENABLED == doze_status)
        {
            ...
            {   
                ...
                if (((doze_buf[2] == 'c') ||(doze_buf[2] == 'e') ||(doze_buf[2] == 'm') || (doze_buf[2] == 'v') ||
                    (doze_buf[2] == 0x5E) /* ^ */
                    )&& (lpwg_int_flag == 1))
                {
                    if (doze_buf[2] != 0x5E)
                 ...
                 if(letter_char == 'm')
                      ...
                      letter = 0x34;
             else if(letter_char == 'v')
                      letter = 0x35;
                  GTP_INFO("[TSP-xw]letter is 0x%2x", letter);
                     #endif
                    }
                    else
                    {
             ...
             else  if ((0xAA == doze_buf[2]) && (lpwg_int_flag == 1))
                {
                ...
                else if ((0xBB == doze_buf[2]) && (lpwg_int_flag == 1))
                {
                 ...
                 else if (0xCC == doze_buf[2]) // double click wakeup
                {
                    ...
                    if(ret > 0)
                      {
                       if((doze_buf_double[2]!=0x01)&&(doze_buf_double[2]!=0x02)&&(doze_buf_double[2]!=0x04)&&(doze_buf_double[2]!=0x08))//key
                         { 
                        GTP_INFO("Double screen to light up the screen!");
                  #ifdef LENOVO_GESTURE_WAKEUP
                     letter = 0x24;
	           #endif
                    doze_status = DOZE_WAKEUP;
                    ...
                    gtp_i2c_write(i2c_client_point, doze_buf, 3);
                       ...
                       else if (doze_buf_double[2]==0x02)
                         {
                    GTP_INFO("Double home key to light up the screen!");
                     #ifdef LENOVO_GESTURE_WAKEUP
                     letter = 0x50;
	               #endif
                     doze_status = DOZE_WAKEUP;
                    ...
                    tpd_touch_down_hook();//chenglogn1 for leSnapshot

                    input_report_key(tpd->dev, KEY_SLIDE, 0);
                    input_sync(tpd->dev);

                    tpd_touch_up_hook();//chenglong1 for leSnapshot
                    // clear 0x814B
                    doze_buf_double[2] = 0x00;
                    gtp_i2c_write(i2c_client_point, doze_buf_double, 3);
                     doze_buf[2] = 0x00;
...
/*lenovo-sw xuwen1 add 20140506 for upgesture, begin*/
#ifdef LENOVO_TP_GESTURE_REBOOT_MACHINE	 			
                  else if ((0xBA == doze_buf[2])  && (((get_boot_mode() == KERNEL_POWER_OFF_CHARGING_BOOT) 
			|| (get_boot_mode() == LOW_POWER_OFF_CHARGING_BOOT))))  { 
	             printk("shone detect tp gesture\n");			  	
                    letter = 0x22;
//lenovo-sw zhangrc2 add for low-power wakeup machine 								      
                    g_tp_poweron = 0x1;
                    ...
                    doze_buf[2] = 0x00;
                    gtp_i2c_write(i2c_client_point, doze_buf, 3);
			gtp_reset_guitar(i2c_client_point, 20);//force_reset_guitar();		
//lenovo-sw zhangrc2 add for low-power wakeup machine 			
			arch_reset(0,NULL);
		}					 
#endif		
...
/*lenovo-xw xuwen1 add 20140506 for start work begin*/
    ...
    #endif
	printk("[GTP-xw]after tpd_local_init\n");
     gtp_enter_doze(i2c_client_point);
	 msleep(58);
    }
#endif
...
/*lenovo-sw xuwen1 add 20140506 for upgesture begin*/
    ...
    if (((get_boot_mode() == KERNEL_POWER_OFF_CHARGING_BOOT) 
	|| (get_boot_mode() == LOW_POWER_OFF_CHARGING_BOOT))) {
	printk("[GTP-xw tpd_suspend wakeup \n]");
	return 0;
 	// gtp_enter_doze(i2c_client_point);
    	}
#endif
/*lenovo-sw xuwen1 add 20140506 for upgesture end*/	
...
/*lenovo-sw zhangrc2 add function for reboot machine by tp gesture  2014-05-01 begin*/
    ...
    if ((get_boot_mode() == KERNEL_POWER_OFF_CHARGING_BOOT) 
	|| (get_boot_mode() == LOW_POWER_OFF_CHARGING_BOOT)){
	/*lenovo-sw xuwen1 add for up gesture begin*/
	printk("[GTP-xw tpd_resume wakeup \n]");
	msleep(58);
	return 0;
...
/*Lenovo-sw yexm1 modify 20140428 end */
