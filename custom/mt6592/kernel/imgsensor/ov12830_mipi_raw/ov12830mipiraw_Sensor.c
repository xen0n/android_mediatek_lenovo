...
#define OV12830_ORIENTATION IMAGE_V_MIRROR
//#define OV12830_DEBUG_SOFIA
#define OV12830_TEST_PATTERN_CHECKSUM (0xa6096115) //V_MIRROR
//#define OV12830_DEBUG_SOFIA
...
void OV12830_write_shutter(kal_uint32 shutter)
{
	...
	if(ov12830.OV12830AutoFlickerMode == KAL_TRUE)
	{
		...
		switch(OV12830CurrentScenarioId)
		{
			...
			case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
				...
				break;
			default:
				min_framelength = (OV12830MIPI_PREVIEW_CLK) /(OV12830_PV_PERIOD_PIXEL_NUMS + ov12830.DummyPixels)/294*10 ;
    			break;
		}
		...
		if (shutter > (max_shutter-4))
			...
			extra_lines = 0;
		OV12830DBSOFIA("extra_lines 0=%d!!\n",extra_lines);
	...
	OV12830DBSOFIA("frame_length %d,shutter %d!!\n",frame_length,shutter);
...
void OV12830PreviewSetting(void)
{
	...
	OV12830_write_cmos_sensor(0x0102,0x01);
	mDELAY(2);
	...
	OV12830_write_cmos_sensor(0x3808,0x08);//width 2112
	OV12830_write_cmos_sensor(0x3809,0x40);//
	OV12830_write_cmos_sensor(0x380a,0x05);//height 1500
	...
	OV12830_write_cmos_sensor(0x5002,0x08);//00->08 09/23
	mDELAY(2);
...
void OV12830VideoSetting(void)
{
	...
	OV12830_write_cmos_sensor(0x0100,0x00);//
	OV12830_write_cmos_sensor(0x0102,0x01);//add fast standby
	mDELAY(2);
	OV12830_write_cmos_sensor(0x30b4,0x03);
	OV12830_write_cmos_sensor(0x30b3,0x54);
	OV12830_write_cmos_sensor(0x3106,0x21);
...
void OV12830CaptureSetting24fps(void)
{
	OV12830DB("OV12830OV12830CaptureSetting24fps:\n ");
	...
	OV12830_write_cmos_sensor(0x30b4,0x03);
	OV12830_write_cmos_sensor(0x3090,0x02);
	OV12830_write_cmos_sensor(0x3091,0x1b);//0x11
	...
	OV12830_write_cmos_sensor(0x380d,0x50);//
	OV12830_write_cmos_sensor(0x380e,0x0B);//
	OV12830_write_cmos_sensor(0x380f,0xE4);//
	...
	OV12830_write_cmos_sensor(0x5002,0x08);//00->08 9/23
	OV12830_write_cmos_sensor(0x0100,0x01);
    mDELAY(66);
...
void OV12830CaptureSetting18fps(void)
{
	...
	OV12830_write_cmos_sensor(0x30b4,0x03);
	OV12830_write_cmos_sensor(0x3090,0x02);
	OV12830_write_cmos_sensor(0x3091,0x14);//0x11
	OV12830_write_cmos_sensor(0x3106,0x01);
	OV12830_write_cmos_sensor(0x3708,0xe3);
	OV12830_write_cmos_sensor(0x3709,0xc3);
	OV12830_write_cmos_sensor(0x3801,0x00);
	...
	OV12830_write_cmos_sensor(0x380c,0x11);
	OV12830_write_cmos_sensor(0x380d,0x00);
	OV12830_write_cmos_sensor(0x380e,0x0B);//0x0c
	OV12830_write_cmos_sensor(0x380f,0xF8);//0x00
	OV12830_write_cmos_sensor(0x3811,0x10);
	OV12830_write_cmos_sensor(0x3813,0x08);
	OV12830_write_cmos_sensor(0x3814,0x11);
	OV12830_write_cmos_sensor(0x3815,0x11);
...
0x40,0x01,0x06,
0x40,0x02,0x45,
0x40,0x05,0x19,//0x18->0x19 modify 09/23 blc
0x40,0x07,0x90,
0x40,0x08,0x24,//am10a 
0x40,0x4e,0x37,
0x40,0x4f,0x8f,
...
0x50,0x13,0x80,
0x50,0x1f,0x00,
0x57,0x80,0xfc,//add DPC update start 09/23
0x57,0x81,0x13,
0x57,0x82,0x03,
0x57,0x86,0x20,
0x57,0x87,0x40,
...
0x57,0x8e,0x02,
0x57,0x8f,0x01,
0x57,0x90,0x01,//add DPC update end 09/23
0x5e,0x00,0x00,
0x5a,0x01,0x00,
0x5a,0x02,0x00,
0x5a,0x03,0x00,
...
0x40,0x04,0x08,
0x48,0x37,0x0b,
0x50,0x02,0x08,//0x00->0x08 09/23
0x01,0x00,0x01};
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if( SENSOR_MODE_CAPTURE== ov12830.sensorMode)
	{
		...
		// Full size setting
		#ifdef OV12830_CAP_18FPS
		OV12830CaptureSetting18fps();
		#else
		#ifdef OV12830_CAP_24FPS
		OV12830CaptureSetting24fps();
		#else
                                                                ...
                                                                UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
    ...
    switch (FeatureId)
    {
		...
		case SENSOR_FEATURE_GET_TEST_PATTERN_CHECKSUM_VALUE:
			*pFeatureReturnPara32=OV12830_TEST_PATTERN_CHECKSUM;
			*pFeatureParaLen=4;
			break;
