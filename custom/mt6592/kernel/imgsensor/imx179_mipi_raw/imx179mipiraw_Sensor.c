...
#define IMX179_TEST_PATTERN_CHECKSUM (0x9e08861c)
...
void IMX179MIPI_write_shutter(kal_uint16 shutter)
{
	...
	if(IMX179MIPI_Auto_Flicker_mode== KAL_TRUE)
	{
		realtime_fp = IMX179MIPI_sensor.pv_pclk *10 / (line_length * frame_length);
		if((realtime_fp >= 298)&&(realtime_fp <= 302))
		{
			realtime_fp = 297;
			frame_length = IMX179MIPI_sensor.pv_pclk *10 / (line_length * realtime_fp);
			SENSORDB("[autofliker realtime_fp=30,extern heights slowdown to 29.6fps][height:%d]",frame_length);
		}
		else if((realtime_fp >= 147)&&(realtime_fp <= 153))
		{
			realtime_fp = 146;
			frame_length = IMX179MIPI_sensor.pv_pclk *10 / (line_length * realtime_fp);
			SENSORDB("[autofliker realtime_fp=15,extern heights slowdown to 14.6fps][height:%d]",frame_length);
		}
	}
	if (IMX179MIPI_sensor.pv_mode == KAL_TRUE) 
	{
...
{
    SENSORDB("[IMX179MIPI] enter IMX179MIPISetTestPatternMode function enable:%d\n", bEnable);
	...
	SENSORDB("[IMX179MIPI] exit IMX179MIPISetTestPatternMode function \n");
    return ERROR_NONE;
}
                                                                ...
                                                                UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
    ...
    switch (FeatureId)
    {
		...
		case SENSOR_FEATURE_GET_TEST_PATTERN_CHECKSUM_VALUE:
			*pFeatureReturnPara32 = IMX179_TEST_PATTERN_CHECKSUM;
			*pFeatureParaLen=4;
			break;
