...
#define IMX111_TEST_PATTERN_CHECKSUM (0x42406a84)
...
KAL_FALSE,KAL_FALSE,KAL_FALSE,134400000,134400000,134400000,800,0,0,64,64,64,3536,1254,3536,1254,3536,2502,0,0,0,0,0,0,30};
...
void IMX111MIPI_write_shutter(kal_uint16 shutter)
{
	...
	IMX111MIPI_sensor.pv_shutter=shutter;
	spin_unlock_irqrestore(&imx111_drv_lock,flags);
	SENSORDB(" write shutter IMX111MIPI_write_shutter\n"); 
...
void IMX111MIPI_SetGain(UINT16 iGain)
{
	...
	SENSORDB("set gain IMX111MIPI_SetGain\n"); 
	IMX111MIPI_write_cmos_sensor(0x0104, 1);
...
#if 0  //1080p  30fps mipi clock  600Mhz  H 1.67 bining V 1.67 Bining 
		IMX111MIPI_write_cmos_sensor(0x0104,1);  
		IMX111MIPI_write_cmos_sensor(0x0100,0x00);// STREAM STop
		IMX111MIPI_write_cmos_sensor(0x0305,0x02);
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	SENSORDB(" enter the preview function IMX111MIPIPreview\n"); 
	...
	//	IMX111MIPI_SetShutter(IMX111MIPI_sensor.pv_shutter);
		...
		image_window->ExposureWindowHeight= IMX111MIPI_IMAGE_SENSOR_PV_HEIGHT -200- 2*iStartY;
		...
		SENSORDB(" exit the preview function IMX111MIPIPreview\n"); 
	return ERROR_NONE;
}	/* IMX111MIPIPreview() */
...
{
    kal_uint16 iStartX = 0, iStartY = 0;
	SENSORDB("enter the video function IMX111MIPIPreview\n"); 
    	spin_lock(&imx111_drv_lock);    
		...
		image_window->GrabStartX= iStartX;
		image_window->GrabStartY= iStartY;
		image_window->ExposureWindowWidth= IMX111MIPI_IMAGE_SENSOR_VIDEO_WIDTH - 2*iStartX;
		image_window->ExposureWindowHeight= IMX111MIPI_IMAGE_SENSOR_VIDEO_HEIGHT - 2*iStartY;
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
  ...
  //  kal_uint32 shutter=IMX111MIPI_sensor.pv_shutter;
	...
	SENSORDB(" enter the capture function IMX111MIPICapture\n"); 
	spin_lock(&imx111_drv_lock);	
	IMX111MIPI_sensor.video_mode=KAL_FALSE;
	IMX111MIPI_sensor.pv_mode=KAL_FALSE;
    ...
    memcpy(&IMX111MIPISensorConfigData, sensor_config_data, sizeof(MSDK_SENSOR_CONFIG_STRUCT));
	spin_unlock(&imx111_drv_lock);
	SENSORDB(" exit the capture function IMX111MIPICapture\n"); 
...
UINT32 IMX111MIPIGetResolution(MSDK_SENSOR_RESOLUTION_INFO_STRUCT *pSensorResolution)
{
    ...
    pSensorResolution->SensorVideoWidth		= IMX111MIPI_REAL_VIDEO_WIDTH;//IMX111MIPI_REAL_VIDEO_WIDTH;
		...
		if (IMX111MIPI_Video_Max_Expourse_Time > IMX111MIPI_VIDEO_FRAME_LENGTH_LINES/*IMX111MIPI_sensor.pv_frame_length*/) 
			{
				...
				spin_unlock(&imx111_drv_lock);
				if(IMX111MIPI_sensor.video_frame_length<IMX111MIPI_sensor.pv_shutter+5)
					{
					spin_lock(&imx111_drv_lock);	
					IMX111MIPI_sensor.video_frame_length=IMX111MIPI_sensor.pv_shutter+5;
					spin_unlock(&imx111_drv_lock);
					}
					spin_lock(&imx111_drv_lock);	
    ...
    switch (FeatureId)
    {
        ...
        case SENSOR_FEATURE_GET_PIXEL_CLOCK_FREQ:
        		switch(CurrentScenarioId)
        		{
        			case MSDK_SCENARIO_ID_CAMERA_ZSD:
        			case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		            *pFeatureReturnPara32 = IMX111MIPI_sensor.cp_pclk; //134400000
		            *pFeatureParaLen=4;		         	
					
		            SENSORDB("Sensor CPCLK:%dn",IMX111MIPI_sensor.cp_pclk); 
		         		break; //
					case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
						*pFeatureReturnPara32 = IMX111MIPI_sensor.video_pclk;//134400000; 
						*pFeatureParaLen=4;
		...
		case SENSOR_FEATURE_GET_TEST_PATTERN_CHECKSUM_VALUE:
			*pFeatureReturnPara32 = IMX111_TEST_PATTERN_CHECKSUM;
			*pFeatureParaLen=4;
			break;
