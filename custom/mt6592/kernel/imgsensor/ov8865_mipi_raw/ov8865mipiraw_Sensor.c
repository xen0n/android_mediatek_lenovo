...
#define OV8865_TEST_PATTERN_CHECKSUM 0xa2230d9f    //0xf5e2f1ce
...
void OV8865PreviewSetting(void)
{
	...
	OV8865_write_cmos_sensor(0x3700,0x24);//; sensor control  AM24
	...
	OV8865_write_cmos_sensor(0x3706,0x38);//AM24
	...
	OV8865_write_cmos_sensor(0x370b,0xb8);//AM24
	...
	OV8865_write_cmos_sensor(0x37a8,0x58);//AM24
	OV8865_write_cmos_sensor(0x37a9,0x58);//AM24
...
void OV8865VideoSetting(void)
{
	...
	OV8865_write_cmos_sensor(0x3700,0x48);// ; sensor control AM24
	...
	OV8865_write_cmos_sensor(0x3706,0x70);//AM24
	...
	OV8865_write_cmos_sensor(0x370b,0x70);//AM24
	...
	OV8865_write_cmos_sensor(0x37a8,0xb0);//AM24
	OV8865_write_cmos_sensor(0x37a9,0xb0);//AM24
...
static void OV8865_Sensor_Init(void)
{
	...
	OV8865_write_cmos_sensor(0x3612,0x86);//AM24  power save ov_Revise
	OV8865_write_cmos_sensor(0x3613,0x58);//AM24
	...
	OV8865_write_cmos_sensor(0x3636,0x10);//AM24
	...
	OV8865_write_cmos_sensor(0x3d85,0x06);//AM24a
	OV8865_write_cmos_sensor(0x3d8c,0x75);//AM24a
	OV8865_write_cmos_sensor(0x3d8d,0xef);//AM24a
	...
	OV8865_write_cmos_sensor(0x5b00,0x02);//
	OV8865_write_cmos_sensor(0x5b01,0xd0);// 
	OV8865_write_cmos_sensor(0x5b02,0x03);// 
	OV8865_write_cmos_sensor(0x5b03,0xff);// 
	OV8865_write_cmos_sensor(0x5b05,0x6c);// 
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
		...
		OV8865DB("OV8865CurrentScenarioId=%d\n",OV8865CurrentScenarioId);
...
UINT32 OV8865SetTestPatternMode(kal_bool bEnable)
{
    ...
    if(bEnable == KAL_TRUE)
    {
		...
		OV8865_write_cmos_sensor(0x5000,0x16);// ; LENC off, MWB on, BPC on, WPC on
