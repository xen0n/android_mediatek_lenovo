...
#define GC5004_TEST_PATTERN_CHECKSUM (0x9657b9f1)
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	SENSORDB(" enter the preview function GC5004MIPIPreview\n"); 
    
	if(GC5004MIPI_sensor.video_mode==TRUE)
	 {
		spin_lock(&gc5004_drv_lock);	
		GC5004MIPI_sensor.video_mode=KAL_FALSE;
		GC5004MIPI_sensor.pv_mode=KAL_TRUE;
	...
	//	image_window->ExposureWindowHeight= GC5004MIPI_IMAGE_SENSOR_PV_HEIGHT - 2*iStartY;
		...
		SENSORDB(" exit the preview function GC5004MIPIPreview\n"); 
	return ERROR_NONE;
}	/* GC5004MIPIPreview() */
...
{
    kal_uint16 iStartX = 0, iStartY = 0;
	SENSORDB("enter the video function GC5004MIPIPreview\n"); 
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
  ...
  //  kal_uint32 shutter=GC5004MIPI_sensor.pv_shutter;
	...
	SENSORDB(" enter the capture function GC5004MIPICapture\n"); 
	spin_lock(&gc5004_drv_lock);	
	GC5004MIPI_sensor.video_mode=KAL_FALSE;
	GC5004MIPI_sensor.pv_mode=KAL_FALSE;
    ...
    memcpy(&GC5004MIPISensorConfigData, sensor_config_data, sizeof(MSDK_SENSOR_CONFIG_STRUCT));
	spin_unlock(&gc5004_drv_lock);
	SENSORDB(" exit the capture function GC5004MIPICapture\n"); 
...
UINT32 GC5004MIPISetVideoMode(UINT16 u2FrameRate)
{
	...
	//	VideoFullSizeSetting()
		if(u2FrameRate==0)//dynamic frame rate
			return TRUE;
		//spin_lock(&gc5004_drv_lock);
    ...
    switch (FeatureId)
    {
        ...
        case SENSOR_FEATURE_GET_PIXEL_CLOCK_FREQ:
        		switch(CurrentScenarioId)
        		{
        			case MSDK_SCENARIO_ID_CAMERA_ZSD:
        			case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		            *pFeatureReturnPara32 = GC5004MIPI_sensor.cp_pclk; 
		            *pFeatureParaLen=4;		         	
					
		            SENSORDB("Sensor CPCLK:%dn",GC5004MIPI_sensor.cp_pclk); 
		         		break;
					case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
						*pFeatureReturnPara32 = GC5004MIPI_sensor.video_pclk;
						*pFeatureParaLen=4;
						SENSORDB("Sensor videoCLK:%d\n",GC5004MIPI_sensor.video_pclk); 
						break;
		         		default:
		            *pFeatureReturnPara32 = GC5004MIPI_sensor.pv_pclk;
		            *pFeatureParaLen=4;
