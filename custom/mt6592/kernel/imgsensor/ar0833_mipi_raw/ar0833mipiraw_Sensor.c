...
kal_bool AR0833DuringTestPattern = KAL_FALSE;
#define AR0833_TEST_PATTERN_CHECKSUM (0x681f6f08)
...
static void AR0833_SetDummy( const kal_uint32 iPixels, const kal_uint32 iLines )
{
	...
	AR0833DB("[AR0833MIPI_SetDummy] frame_length is :%d \n ",frame_length);
	AR0833DB("[AR0833MIPI_SetDummy] line_lengthis :%d \n ",line_length);
...
static void AR0833_Sensor_Init(void)
{
	  ...
	  AR0833DB("AR0833_Sensor_Init 4lane_OB:\n ");			
	  ...
	  AR0833_write_cmos_sensor(0x31B4, 0x4C36);  // MIPI_TIMING_0
...
void AR0833VideoSetting(void)
{
	AR0833DB("AR0833VideoSetting_16:9 exit :\n ");
	  ...
	  AR0833_write_cmos_sensor(0x301A, 0x0218);   // RESET_REGISTER
	  AR0833_write_cmos_sensor(0x0342, 0x139E);   // LINE_LENGTH_PCK
	  AR0833_write_cmos_sensor(0x0340, 0x079D);   // FRAME_LENGTH_LINES
	  AR0833_write_cmos_sensor(0x0202, 0x0700);   // COARSE_INTEGRATION_TIME
	  AR0833_write_cmos_sensor(0x0344, 0x0008);   // X_ADDR_START
	  AR0833_write_cmos_sensor(0x0348, 0x0CC7);   // X_ADDR_END
	  ...
	  AR0833_write_cmos_sensor(0x034E, 0x072C);   // Y_OUTPUT_SIZE
	  AR0833_write_cmos_sensor(0x3040, 0x4041);   // READ_MODE
...
UINT32 AR0833Open(void)
{
	...
	AR0833DuringTestPattern = KAL_FALSE;
...
UINT32 AR0833SetVideoMode(UINT16 u2FrameRate)
{
    ...
    if(ar0833.sensorMode == SENSOR_MODE_VIDEO)//video ScenarioId recording
    {
		...
		if((MIN_Frame_length <=AR0833_VIDEO_PERIOD_LINE_NUMS))
		{
			...
			AR0833DB("[AR0833SetVideoMode]current fps = %d\n", (ar0833.pvPclk*100000)  /(AR0833_VIDEO_PERIOD_PIXEL_NUMS)/AR0833_VIDEO_PERIOD_LINE_NUMS);
		}
		AR0833DB("[AR0833SetVideoMode]current fps (10 base)= %d\n", (ar0833.pvPclk*100000)*10/(AR0833_VIDEO_PERIOD_PIXEL_NUMS + ar0833.DummyPixels)/MIN_Frame_length);
		extralines = MIN_Frame_length - AR0833_VIDEO_PERIOD_LINE_NUMS;
		AR0833DB("[AR0833SetVideoMode]extralines= %d\n", extralines);
		spin_lock(&ar0833mipiraw_drv_lock);
		ar0833.DummyPixels = 0;//define dummy pixels and lines
...
UINT32 AR0833SetAutoFlickerMode(kal_bool bEnable, UINT16 u2FrameRate)
{
    ...
    if (KAL_TRUE == AR0833DuringTestPattern) return ERROR_NONE;
...
UINT32 AR0833SetTestPatternMode(kal_bool bEnable)
{
	...
	{
        spin_lock(&ar0833mipiraw_drv_lock);
	    AR0833DuringTestPattern = KAL_TRUE;
	    spin_unlock(&ar0833mipiraw_drv_lock);
		AR0833_write_cmos_sensor(0x0600,0x0002);
	}
   else		
	{
		AR0833_write_cmos_sensor(0x0600,0x0000);	
	}
                                                                ...
                                                                UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
	   ...
	   switch (FeatureId)
	   {
		   ...
		   case SENSOR_FEATURE_GET_TEST_PATTERN_CHECKSUM_VALUE://for factory mode auto testing 			
               *pFeatureReturnPara32= AR0833_TEST_PATTERN_CHECKSUM;
			   *pFeatureParaLen=4; 							
			break;	
