...
#define S5K4H5YX_TEST_PATTERN_CHECKSUM (0x82256eb5)
#define SHUTTER_FRAMELENGTH_MARGIN 16

#ifdef S5K4H5YX_DEBUG
	#define LOG_TAG (__FUNCTION__)
	#define S5K4H5YXDB(fmt, arg...) xlog_printk(ANDROID_LOG_DEBUG, LOG_TAG, fmt, ##arg)
#else
	#define S5K4H5YXDB(x,...)
#endif
...
static kal_uint16 S5K4H5YX_burst_write_cmos_sensor(kal_uint16* para, kal_uint32 len)
{   
	...
	{       
		...
		{           
			iBurstWriteReg(puSendCmd , tosend, S5K4H5YXMIPI_WRITE_ID);           
			tosend = 0;       
		}   
	}   
...
void S5K4H5YX_MIPI_read_otp_wb(struct S5K4H5YX_MIPI_otp_struct *otp)
{
	...
	{
		S5K4H5YXDB("PageCount=%d\n", PageCount);	
		
		S5K4H5YX_write_cmos_sensor(0x3a02, PageCount); 
		S5K4H5YX_write_cmos_sensor(0x3a00, 0x01); 
		...
		S5K4H5YX_write_cmos_sensor(0x3a00, 0x00); 
	...
	S5K4H5YXDB("otp->R_to_G=0x%x, otp->B_to_G=0x%x, otp->G_to_G=0x%x\n", otp->R_to_G, otp->B_to_G, otp->G_to_G);	
}



void S5K4H5YX_MIPI_algorithm_otp_wb(struct S5K4H5YX_MIPI_otp_struct *otp)
{
	...
	S5K4H5YXDB("R_to_G=%d, B_to_G=%d, G_to_G=%d\n", R_to_G, B_to_G, G_to_G);	
	...
	S5K4H5YXDB("R_gain=0x%x, B_gain=0x%, G_gain=0x%x\n", otp->R_Gain, otp->B_Gain, otp->G_Gain);
}
...
void S5K4H5YX_MIPI_write_otp_wb(struct S5K4H5YX_MIPI_otp_struct *otp)
{
	...
	S5K4H5YXDB("R_GainH=0x%x, R_GainL=0x%x, B_GainH=0x%x, B_GainL=0x%x, G_GainH=0x%x, G_GainL=0x%x\n", R_GainH, R_GainL, B_GainH, B_GainL, G_GainH, G_GainL);	
...
void S5K4H5YX_MIPI_update_wb_register_from_otp(void)
{
	...
	S5K4H5YX_MIPI_algorithm_otp_wb(&current_otp);
...
//			if this value is false, fix frameratee, count min frame length limit in S5K4H5YXSetVideoMode function call S5K4H5YX_SetDummy function
//
//////////////////////////////////////////////
kal_uint16 S5K4H5YX_get_framelength(void)
{
	kal_uint16 Framelength = s5k4h5yx_preview_frame_length;

	if(s5k4h5yx.sensorMode == SENSOR_MODE_PREVIEW)
		Framelength = s5k4h5yx_preview_frame_length;
	else if(s5k4h5yx.sensorMode == SENSOR_MODE_CAPTURE)
		Framelength = s5k4h5yx_capture_frame_length;
	else if(s5k4h5yx.sensorMode == SENSOR_MODE_VIDEO)
		{
		if(s5k4h5yx.DynamicVideoSupport == KAL_TRUE)
			Framelength = s5k4h5yx_video_frame_length;
		else
			Framelength = (S5K4H5YX_read_cmos_sensor(0x0340)<<8)|S5K4H5YX_read_cmos_sensor(0x0341);
		}
	return Framelength;
}
...
kal_uint16 S5K4H5YX_get_linelength(void)
{
	kal_uint16 Linelength = s5k4h5yx_preview_line_length;
...
kal_uint32 S5K4H5YX_get_pixelclock(void)
{
	kal_uint32 PixelClock = s5k4h5yx_preview_pixelclock;

	if(s5k4h5yx.sensorMode == SENSOR_MODE_PREVIEW)
		PixelClock = s5k4h5yx_preview_pixelclock;
	else if(s5k4h5yx.sensorMode == SENSOR_MODE_CAPTURE)
		PixelClock = s5k4h5yx_capture_pixelclock;
	else if(s5k4h5yx.sensorMode == SENSOR_MODE_VIDEO)
		PixelClock = s5k4h5yx_video_pixelclock;
...
void S5K4H5YX_write_shutter(kal_uint32 shutter)
{
	...
	frame_length_min = S5K4H5YX_get_framelength();

	if(s5k4h5yx.sensorMode == SENSOR_MODE_VIDEO && s5k4h5yx.DynamicVideoSupport == KAL_FALSE)
	{
	...
	else
	{
		...
		{
			line_length = S5K4H5YX_get_linelength();
			pixelclock = S5K4H5YX_get_pixelclock();
	...
	shutter_count = S5K4H5YX_read_shutter();
	...
	S5K4H5YXDB("shutter=%d, frame_length=%d, frame_length_min=%d, framerate=%d\n", shutter,frame_length, frame_length_min, framerate);
} 
...
static void S5K4H5YX_SetDummy( const kal_uint32 iPixels, const kal_uint32 iLines )
{
	...
	frame_length = S5K4H5YX_get_framelength() + iLines;
	line_length = S5K4H5YX_get_linelength() + iPixels;
	shutter = frame_length - SHUTTER_FRAMELENGTH_MARGIN;
	...
	S5K4H5YXDB("frame_length=%d, line_length=%d\n", frame_length, line_length);
}
...
void S5K4H5YXPreviewSetting(void)
{
	...
	chip_id = S5K4H5YX_read_cmos_sensor(0x0002);

	S5K4H5YXDB("chip_id=%x\n",chip_id);
	...
	if(chip_id == 0x01)
	{
	    ...
	    S5K4H5YX_burst_write_cmos_sensor(addr_data_pair, sizeof(addr_data_pair)/sizeof(kal_uint16));
	#else
		S5K4H5YX_write_cmos_sensor(0x0100,0x00);
		S5K4H5YX_write_cmos_sensor(0x0101,0x00);
		S5K4H5YX_write_cmos_sensor(0x0204,0x00);
		S5K4H5YX_write_cmos_sensor(0x0205,0x20);
		S5K4H5YX_write_cmos_sensor(0x0200,0x0D);
		S5K4H5YX_write_cmos_sensor(0x0201,0x78);
		S5K4H5YX_write_cmos_sensor(0x0202,0x04);
		S5K4H5YX_write_cmos_sensor(0x0203,0xE2);
		S5K4H5YX_write_cmos_sensor(0x0340,0x09);
		S5K4H5YX_write_cmos_sensor(0x0341,0xD0);
		...
		S5K4H5YX_write_cmos_sensor(0x030F,0xAF);
		S5K4H5YX_write_cmos_sensor(0x3C5A,0x00);
		S5K4H5YX_write_cmos_sensor(0x0310,0x01);
		S5K4H5YX_write_cmos_sensor(0x3C50,0x53);
		S5K4H5YX_write_cmos_sensor(0x3C62,0x02);
		S5K4H5YX_write_cmos_sensor(0x3C63,0xBC);
		...
		S5K4H5YX_write_cmos_sensor(0x3002,0x0D);
		S5K4H5YX_write_cmos_sensor(0x300A,0x0D);
		S5K4H5YX_write_cmos_sensor(0x3037,0x12);
		S5K4H5YX_write_cmos_sensor(0x3045,0x04);
		S5K4H5YX_write_cmos_sensor(0x300C,0x78);
		S5K4H5YX_write_cmos_sensor(0x300D,0x80);
		S5K4H5YX_write_cmos_sensor(0x305C,0x82);
		S5K4H5YX_write_cmos_sensor(0x3010,0x0A);
		S5K4H5YX_write_cmos_sensor(0x305E,0x11);
		S5K4H5YX_write_cmos_sensor(0x305F,0x11);
		S5K4H5YX_write_cmos_sensor(0x3060,0x10);
		S5K4H5YX_write_cmos_sensor(0x3091,0x04);
		S5K4H5YX_write_cmos_sensor(0x3092,0x07);
		S5K4H5YX_write_cmos_sensor(0x303D,0x05);
		S5K4H5YX_write_cmos_sensor(0x3038,0x99);
	...
	{ 
		...
		S5K4H5YX_write_cmos_sensor(0x3C64,0x00);
		S5K4H5YX_write_cmos_sensor(0x3C65,0x00);
		S5K4H5YX_write_cmos_sensor(0x0114,0x03);
		S5K4H5YX_write_cmos_sensor(0x3C1E,0x00);
		...
		S5K4H5YX_write_cmos_sensor(0x300C,0x54);
		S5K4H5YX_write_cmos_sensor(0x300D,0x65);
		S5K4H5YX_write_cmos_sensor(0x3010,0x00);
		S5K4H5YX_write_cmos_sensor(0x3012,0x1D);
		S5K4H5YX_write_cmos_sensor(0x3014,0x19);
		S5K4H5YX_write_cmos_sensor(0x3017,0x0F);
		S5K4H5YX_write_cmos_sensor(0x3018,0x09);
		S5K4H5YX_write_cmos_sensor(0x3019,0x5B);
		...
		S5K4H5YX_write_cmos_sensor(0x3086,0x7A);
		S5K4H5YX_write_cmos_sensor(0x30A6,0x01);
		S5K4H5YX_write_cmos_sensor(0x30A7,0x06);
		S5K4H5YX_write_cmos_sensor(0x3032,0x01);
		S5K4H5YX_write_cmos_sensor(0x304A,0x02);
		S5K4H5YX_write_cmos_sensor(0x3054,0xF0);
		...
		S5K4H5YX_write_cmos_sensor(0x304B,0x39);
		S5K4H5YX_write_cmos_sensor(0x303A,0xA0);
		S5K4H5YX_write_cmos_sensor(0x302D,0x7F);
		S5K4H5YX_write_cmos_sensor(0x305E,0x11);
		S5K4H5YX_write_cmos_sensor(0x3038,0x50);
		S5K4H5YX_write_cmos_sensor(0x3097,0x55);
		S5K4H5YX_write_cmos_sensor(0x3096,0x05);
		S5K4H5YX_write_cmos_sensor(0x308F,0x00);
		S5K4H5YX_write_cmos_sensor(0x3230,0x00);
		S5K4H5YX_write_cmos_sensor(0x3042,0x01);
		S5K4H5YX_write_cmos_sensor(0x3053,0x01);
		S5K4H5YX_write_cmos_sensor(0x320D,0x00);
		S5K4H5YX_write_cmos_sensor(0x0100,0x01);
	#endif
	}	
...
void S5K4H5YXVideoSetting(void)
{
	...
	if(chip_id == 0x01)
	{
		...
		S5K4H5YX_write_cmos_sensor(0x0343,0x68);
		S5K4H5YX_write_cmos_sensor(0x0344,0x00);
		S5K4H5YX_write_cmos_sensor(0x0345,0x08);
		S5K4H5YX_write_cmos_sensor(0x0346,0x01);
		S5K4H5YX_write_cmos_sensor(0x0347,0x3A);
		S5K4H5YX_write_cmos_sensor(0x0348,0x0C);
		S5K4H5YX_write_cmos_sensor(0x0349,0xC7);
		S5K4H5YX_write_cmos_sensor(0x034A,0x08);
		S5K4H5YX_write_cmos_sensor(0x034B,0x65);
		S5K4H5YX_write_cmos_sensor(0x034C,0x0C);
		S5K4H5YX_write_cmos_sensor(0x034D,0xC0);
		S5K4H5YX_write_cmos_sensor(0x034E,0x07);
		S5K4H5YX_write_cmos_sensor(0x034F,0x2C);
		S5K4H5YX_write_cmos_sensor(0x0390,0x00);
		S5K4H5YX_write_cmos_sensor(0x0391,0x00);
		S5K4H5YX_write_cmos_sensor(0x0381,0x01);
		S5K4H5YX_write_cmos_sensor(0x0383,0x01);
...
UINT32 S5K4H5YXOpen(void)
{
	...
	s5k4h5yx.DynamicVideoSupport = KAL_FALSE;
	spin_unlock(&s5k4h5yxmipiraw_drv_lock);
...
UINT32 S5K4H5YXGetSensorID(UINT32 *sensorID)
{
    ...
    do {
        *sensorID = (S5K4H5YX_read_cmos_sensor(0x0000)<<8)|S5K4H5YX_read_cmos_sensor(0x0001);
        ...
        {
        	S5K4H5YXDB("Sensor ID = 0x%04x\n", *sensorID);
			break;
        }
		
        S5K4H5YXDB("Read Sensor ID Fail = 0x%04x\n", *sensorID);
...
UINT32 S5K4H5YXPreview(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window, MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	S5K4H5YX_FeatureControl_PERIOD_LineNum = s5k4h5yx_preview_frame_length;
...
UINT32 S5K4H5YXVideo(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window, MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
...
UINT32 S5K4H5YXCapture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window, MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5K4H5YXDB("Enter\n");
	...
	if( SENSOR_MODE_CAPTURE != s5k4h5yx.sensorMode)
	{
		...
		S5K4H5YX_FeatureControl_PERIOD_LineNum = s5k4h5yx_capture_frame_length;
		spin_unlock(&s5k4h5yxmipiraw_drv_lock);
...
UINT32 S5K4H5YXGetResolution(MSDK_SENSOR_RESOLUTION_INFO_STRUCT *pSensorResolution)
{
	...
	pSensorResolution->SensorPreviewWidth	= s5k4h5yx_preview_width;
    pSensorResolution->SensorPreviewHeight	= s5k4h5yx_preview_height;
	pSensorResolution->SensorFullWidth		= s5k4h5yx_capture_width;
    pSensorResolution->SensorFullHeight		= s5k4h5yx_capture_height;
    pSensorResolution->SensorVideoWidth		= s5k4h5yx_video_width; 
    pSensorResolution->SensorVideoHeight    = s5k4h5yx_video_height;
...
UINT32 S5K4H5YXGetInfo(MSDK_SCENARIO_ID_ENUM ScenarioId, MSDK_SENSOR_INFO_STRUCT *pSensorInfo, MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	S5K4H5YXDB("Enter\n");

	pSensorInfo->SensorPreviewResolutionX= s5k4h5yx_preview_width;
	pSensorInfo->SensorPreviewResolutionY= s5k4h5yx_preview_height;
	pSensorInfo->SensorFullResolutionX= s5k4h5yx_capture_width;
    pSensorInfo->SensorFullResolutionY= s5k4h5yx_capture_height;
    ...
    pSensorInfo->AEShutDelayFrame = 1;
    ...
    switch (ScenarioId)
    {
        case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
            pSensorInfo->SensorClockFreq=s5k4h5yx_master_clock;
            pSensorInfo->SensorClockRisingCount= 0;
			pSensorInfo->SensorGrabStartX = s5k4h5yx_preview_startx;
            pSensorInfo->SensorGrabStartY = s5k4h5yx_preview_starty;
        ...
        case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
            pSensorInfo->SensorClockFreq=s5k4h5yx_master_clock;
            pSensorInfo->SensorClockRisingCount= 0;
            pSensorInfo->SensorGrabStartX = s5k4h5yx_video_startx;
            pSensorInfo->SensorGrabStartY = s5k4h5yx_video_starty;
            pSensorInfo->SensorMIPILaneNumber = SENSOR_MIPI_4_LANE;
            pSensorInfo->MIPIDataLowPwr2HighSpeedTermDelayCount = 0;
	     	pSensorInfo->MIPIDataLowPwr2HighSpeedSettleDelayCount = 6;
	    	pSensorInfo->MIPICLKLowPwr2HighSpeedTermDelayCount = 0;
		...
		case MSDK_SCENARIO_ID_CAMERA_ZSD:
            pSensorInfo->SensorClockFreq=s5k4h5yx_master_clock;
            pSensorInfo->SensorClockRisingCount= 0;
			pSensorInfo->SensorGrabStartX = s5k4h5yx_capture_startx;	
            pSensorInfo->SensorGrabStartY = s5k4h5yx_capture_starty;	
            pSensorInfo->SensorMIPILaneNumber = SENSOR_MIPI_4_LANE;
            pSensorInfo->MIPIDataLowPwr2HighSpeedTermDelayCount = 0;
            pSensorInfo->MIPIDataLowPwr2HighSpeedSettleDelayCount = 6;
            pSensorInfo->MIPICLKLowPwr2HighSpeedTermDelayCount = 0;
...
UINT32 S5K4H5YXControl(MSDK_SCENARIO_ID_ENUM ScenarioId, MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *pImageWindow, MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	spin_lock(&s5k4h5yxmipiraw_drv_lock);
...
UINT32 S5K4H5YXSetVideoMode(UINT16 u2FrameRate)
{
    ...
    S5K4H5YXDB("frame rate = %d\n", u2FrameRate);
	
	if(u2FrameRate==0)
	{
		S5K4H5YXDB("Disable Video Mode or dynimac fps\n");
		
		spin_lock(&s5k4h5yxmipiraw_drv_lock);
		s5k4h5yx.DynamicVideoSupport = KAL_TRUE;
		spin_unlock(&s5k4h5yxmipiraw_drv_lock);
	...
	if(u2FrameRate >30 || u2FrameRate <5)
	    S5K4H5YXDB("error frame rate seting\n");
	...
	if(s5k4h5yx.S5K4H5YXAutoFlickerMode == KAL_TRUE)
    {
		...
		MIN_Frame_length = (s5k4h5yx_video_pixelclock)/(s5k4h5yx_video_line_length+ s5k4h5yx.DummyPixels)/frameRate*10;
    }
	else
		MIN_Frame_length = (s5k4h5yx_video_pixelclock) /(s5k4h5yx_video_line_length + s5k4h5yx.DummyPixels)/u2FrameRate;
	...
	S5K4H5YXDB("MIN_Frame_length=%d,s5k4h5yx.DummyLines=%d\n",MIN_Frame_length,s5k4h5yx.DummyLines);
...
UINT32 S5K4H5YXSetTestPatternMode(kal_bool bEnable)
{
	...
	if(bEnable) 	 
		S5K4H5YX_write_cmos_sensor(0x0601, 0x02);
	else		  
		S5K4H5YX_write_cmos_sensor(0x0601, 0x00);  
...
UINT32 S5K4H5YXMIPISetMaxFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 frameRate) 
{
	...
	switch (scenarioId) {
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
			S5K4H5YXDB("MSDK_SCENARIO_ID_CAMERA_PREVIEW\n");
			lineLength = s5k4h5yx_preview_line_length;
			frameHeight = (10 * s5k4h5yx_preview_pixelclock) / frameRate / lineLength;
			...
			break;			
		case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
			S5K4H5YXDB("MSDK_SCENARIO_ID_VIDEO_PREVIEW\n");
			lineLength = s5k4h5yx_video_line_length;
			frameHeight = (10*s5k4h5yx_video_pixelclock) / frameRate / lineLength;
		...
		case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		case MSDK_SCENARIO_ID_CAMERA_ZSD:	
			S5K4H5YXDB("MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG or MSDK_SCENARIO_ID_CAMERA_ZSD\n");
			lineLength = s5k4h5yx_capture_line_length;
			frameHeight =  (10*s5k4h5yx_capture_pixelclock) / frameRate / lineLength;
...
UINT32 S5K4H5YXMIPIGetDefaultFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 *pframeRate) 
{
	switch (scenarioId) 
	{
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
		case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
			 *pframeRate = s5k4h5yx_video_max_framerate;
			 break;
		case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		case MSDK_SCENARIO_ID_CAMERA_ZSD:
			 *pframeRate = s5k4h5yx_capture_max_framerate;
			break;		
        case MSDK_SCENARIO_ID_CAMERA_3D_PREVIEW: 
        case MSDK_SCENARIO_ID_CAMERA_3D_VIDEO:
        case MSDK_SCENARIO_ID_CAMERA_3D_CAPTURE:    
			 *pframeRate = s5k4h5yx_preview_max_framerate;
			break;		
		default:
			break;
	}
...
UINT32 S5K4H5YXFeatureControl(MSDK_SENSOR_FEATURE_ENUM FeatureId, UINT8 *pFeaturePara, UINT32 *pFeatureParaLen)
{
    ...
    switch (FeatureId)
    {
        case SENSOR_FEATURE_GET_RESOLUTION:
            *pFeatureReturnPara16++= s5k4h5yx_capture_width;
            *pFeatureReturnPara16= s5k4h5yx_capture_height;
            *pFeatureParaLen=4;
            break;
        ...
        case SENSOR_FEATURE_GET_PIXEL_CLOCK_FREQ:
			...
			{
				case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
					*pFeatureReturnPara32 = s5k4h5yx_preview_pixelclock;
					*pFeatureParaLen=4;
					break;
				case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
					*pFeatureReturnPara32 = s5k4h5yx_video_pixelclock;
					*pFeatureParaLen=4;
					break;	 
				case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
				case MSDK_SCENARIO_ID_CAMERA_ZSD:
					*pFeatureReturnPara32 = s5k4h5yx_capture_pixelclock;
					*pFeatureParaLen=4;
					break;
				default:
					*pFeatureReturnPara32 = s5k4h5yx_preview_pixelclock;
		...
		case SENSOR_FEATURE_GET_TEST_PATTERN_CHECKSUM_VALUE:		           
			*pFeatureReturnPara32= S5K4H5YX_TEST_PATTERN_CHECKSUM;			
			*pFeatureParaLen=4; 										
			break;
