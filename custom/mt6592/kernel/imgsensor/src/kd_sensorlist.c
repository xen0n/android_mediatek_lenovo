...
int iBurstReadRegEx(u32 phyAddr, u16 regAddr, u32 bytes, u16 i2cId)
{
	u32 old_addr = 0;
	int ret = 0;
	int retry = 0;
	u8 addr[2] = {regAddr>>8, regAddr&0xff};
	...
	ret = i2c_master_send(g_pstI2Cclient, (u8*)addr, 2);
	if (ret != 2) {
		PK_DBG("[CAMERA SENSOR] I2C send failed, addr = 0x%x!! \n", regAddr);
		return -1;
	}
	...
	ret = i2c_master_recv(g_pstI2Cclient, (char *)phyAddr, bytes);
	spin_lock(&kdsensor_drv_lock);
	g_pstI2Cclient->addr = old_addr;
	spin_unlock(&kdsensor_drv_lock);
...
int iBurstWriteRegEx(u32 phyAddr, u32 bytes, u16 i2cId)
{
	u32 old_addr = 0;
	int ret = 0;
	int retry = 0;
	...
	old_addr = g_pstI2Cclient->addr;
	spin_lock(&kdsensor_drv_lock);
	/*lenovo-sw zhangjiano modify i2c burst error 2013.11.05 begin*/
#if 1
	g_pstI2Cclient->addr = (g_pstI2Cclient->addr & (~0xff)) |i2cId;
	g_pstI2Cclient->addr = ( (((g_pstI2Cclient->addr|i2cId) >> 1) &  I2C_MASK_FLAG) | I2C_DMA_FLAG );
#else
