...
kal_uint16 Last_framerate_30=30;
kal_uint16 Last_framerate_15=15;
kal_uint16 Last_framerate=30;
...
// printk("[HI542raw]:Fix_framerate :%d\n", Fix_framerate);
 if(Fix_framerate==0)
 	{
...
UINT32 HI542MIPIOpen(void)
{
	...
	Fix_framerate=0;
	 Last_framerate_30=30;
	 Last_framerate_15=15;
    spin_unlock(&hi542mipi_drv_lock);	
...
//	fixed mode setting
	...
	SENSORDB("Last_framerate_30=%d,Last_framerate_15=%d,Fix_framerate=%d\n",Last_framerate_30,Last_framerate_15,Fix_framerate);
	spin_lock(&hi542mipi_drv_lock);
	Fix_framerate=1;
	spin_unlock(&hi542mipi_drv_lock);
    if (u2FrameRate == 30 && Last_framerate_30 !=0)
    { 
       spin_lock(&hi542mipi_drv_lock);
			Fix_framerate=1;
		    HI542MIPI_MPEG4_encode_mode = KAL_TRUE; 
    ...
    else if (u2FrameRate == 15 && Last_framerate_15 !=0)       
    {
    	spin_lock(&hi542mipi_drv_lock);
			Fix_framerate=1;
		    HI542MIPI_MPEG4_encode_mode = KAL_TRUE; 
    ...
    else if(u2FrameRate == 0)
	{
		    ...
		    HI542MIPI_MPEG4_encode_mode = KAL_FALSE; 
			spin_unlock(&hi542mipi_drv_lock);
			if(Last_framerate==30)
			{
				spin_lock(&hi542mipi_drv_lock);
 				Last_framerate_30=u2FrameRate;
				Last_framerate_15=15;
				spin_unlock(&hi542mipi_drv_lock);
			}
			if(Last_framerate==15)
			{
				spin_lock(&hi542mipi_drv_lock);
 				Last_framerate_15=u2FrameRate;
				Last_framerate_30=30;
				spin_unlock(&hi542mipi_drv_lock);
			}
...
UINT32 HI542MIPISetTestPatternMode(kal_bool bEnable)
{
    ...
    if(bEnable) {   // enable color bar   
	 ...
	 HI542MIPI_write_cmos_sensor(0x0050, 0x04);    
    } else 
    {
        HI542MIPI_write_cmos_sensor(0x0050, 0x00);  // disable color bar test pattern
    }
    return TRUE;
}
...
{    
    ...
    switch (FeatureId)
    {
		...
		case SENSOR_FEATURE_GET_TEST_PATTERN_CHECKSUM_VALUE:			
			*pFeatureReturnPara32=HI542_TEST_PATTERN_CHECKSUM;			
			*pFeatureParaLen=4;			
