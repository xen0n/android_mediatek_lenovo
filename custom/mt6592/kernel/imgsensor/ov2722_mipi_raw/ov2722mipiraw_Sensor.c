...
static OV2722MIPI_sensor_struct OV2722MIPI_sensor =
{
  ...
  .frame_height   = OV2722MIPI_PV_PERIOD_LINE_NUMS  + OV2722MIPI_DEFAULT_DUMMY_LINES,
  .line_length    = OV2722MIPI_PV_PERIOD_PIXEL_NUMS + OV2722MIPI_DEFAULT_DUMMY_PIXELS,
  .default_height = OV2722MIPI_PV_PERIOD_LINE_NUMS  + OV2722MIPI_DEFAULT_DUMMY_LINES,
};
...
kal_uint8  ov2722test_pattern_flag=0;

#define OV2722_TEST_PATTERN_CHECKSUM (0x8b04298)
...
static void OV2722MIPI_Write_Shutter(kal_uint16 iShutter)
{
	...
	unsigned long flags;
	SENSORDB("[OV2722MIPI]enter OV2722MIPI_Write_Shutter function iShutter=%d\n",iShutter);	    
	...
	SENSORDB("[OV2722MIPI]exit OV2722MIPI_Write_Shutter function 2722MIPI_frame_length=%x\n",OV2722MIPI_sensor.frame_height);
...
static void OV2722MIPI_Set_Dummy(const kal_uint16 iPixels, const kal_uint16 iLines)
{
	...
	SENSORDB("[OV2722MIPI]enter OV2722MIPI_Set_Dummy function iPixels:%x; iLines:%x \n",iPixels,iLines);
...
#endif
	SENSORDB("[OV2722MIPI]exit OV2722MIPI_Set_Dummy function \n");
...
#if 1
	unsigned long flags;
	SENSORDB("[OV2722MIPI]enter set_OV2722MIPI_shutter function iShutter=%x\n",iShutter);
    spin_lock_irqsave(&ov2722mipiraw_drv_lock,flags);
	OV2722MIPI_sensor.shutter = iShutter;
	spin_unlock_irqrestore(&ov2722mipiraw_drv_lock,flags);
    OV2722MIPI_Write_Shutter(iShutter);
	SENSORDB("[OV2722MIPI]exit set_OV2722MIPI_shutter function \n");
#endif
...
{
	#if 1
	SENSORDB("[OV2722MIPI]enter OV2722MIPIGain2Reg function \n");
	kal_uint16 iReg = 0x00;
	iReg = ((iGain / BASEGAIN) << 4) + ((iGain % BASEGAIN) * 16 / BASEGAIN);
	iReg = iReg & 0xFF;
	return (kal_uint8)iReg;
	SENSORDB("[OV2722MIPI]exit OV2722MIPIGain2Reg function \n");
	#endif
}
...
void OV2722MIPI_SetGain(UINT16 iGain)
{
	...
	/* [4:9] = M meams M X	*/
	/* Total gain = M + N /16 X */
	SENSORDB("[OV2722MIPI]enter OV2722MIPIGain2Reg function iGain=%x\n",iGain);
	iReg = OV2722MIPIGain2Reg(iGain);
	SENSORDB("[ylf]OV2722MIPI_SetGain,iReg:%x",iReg);
	if (iReg < 0x10) iReg = 0x10;
	//OV5647MIPI_write_cmos_sensor(0x3508, iReg);
	 OV2722MIPI_write_cmos_sensor(0x3509,iReg);
	SENSORDB("[OV2722MIPI]exit OV2722MIPI_SetGain function \n");
	return iGain;		
	#endif
}	/*	OV2722MIPI_SetGain	*/
...
void OV2722MIPI_night_mode(kal_bool enable)
{
	...
	SENSORDB("[OV2722MIPI]enter OV2722MIPI_night_mode function enable=%x\n",enable);

	if (!OV2722MIPI_sensor.video_mode) 
		return;
	...
	SENSORDB("[OV2722MIPI]exit OV2722MIPI_night_mode function \n");
}   /*  OV2722MIPI_NightMode    */
...
{
	kal_uint32 i;
	SENSORDB("[OV2722MIPI]enter OV2722MIPI_camera_para_to_sensor function \n");
	...
	SENSORDB("[OV2722MIPI]exit OV2722MIPI_camera_para_to_sensor function \n");
}
...
static void OV2722MIPI_sensor_to_camera_para(void)
{
	...
	SENSORDB("[OV2722MIPI]enter OV2722MIPI_sensor_to_camera_para function \n");
	for (i = 0; 0xFFFFFFFF != OV2722MIPI_sensor.eng.reg[i].Addr; i++)
	{
	...
	SENSORDB("[OV2722MIPI]exit OV2722MIPI_sensor_to_camera_para function \n");
}
/* ------------------------ Engineer mode ------------------------ */
inline static void OV2722MIPI_get_sensor_group_count(kal_int32 *sensor_count_ptr)
{
	SENSORDB("[OV2722MIPI]enter OV2722MIPI_get_sensor_group_count function \n");
...
{
	SENSORDB("[OV2722MIPI]enter OV2722MIPI_get_sensor_group_info function \n");
	...
	SENSORDB("[OV2722MIPI]exit OV2722MIPI_get_sensor_group_info function \n");
}
...
inline static void OV2722MIPI_get_sensor_item_info(MSDK_SENSOR_ITEM_INFO_STRUCT *para)
{
	...
	SENSORDB("[OV2722MIPI]enter OV2722MIPI_get_sensor_item_info function \n");
	switch (para->GroupIdx)
	{
		case OV2722MIPI_PRE_GAIN:
			switch (para->ItemIdx)
	...
	SENSORDB("[OV2722MIPI]exit OV2722MIPI_get_sensor_item_info function \n");
}

inline static kal_bool OV2722MIPI_set_sensor_item_info(MSDK_SENSOR_ITEM_INFO_STRUCT *para)
{
	kal_uint16 temp_para;
   	SENSORDB("[OV2722MIPI]enter OV2722MIPI_set_sensor_item_info function \n");
  	switch (para->GroupIdx)
	{
		case OV2722MIPI_PRE_GAIN:
			switch (para->ItemIdx)
	...
	SENSORDB("[OV2722MIPI]exit OV2722MIPI_set_sensor_item_info function \n");
  	return KAL_TRUE;
}
void OV2722MIPI_init()
{
	SENSORDB("[OV2722MIPI]enter OV2722MIPI_init function \n");
	...
	OV2722MIPI_write_cmos_sensor(0x3801,0x06);//08
	OV2722MIPI_write_cmos_sensor(0x3802,0x00); 
	OV2722MIPI_write_cmos_sensor(0x3803,0x00);//02
	OV2722MIPI_write_cmos_sensor(0x3804,0x07);//9b  
	OV2722MIPI_write_cmos_sensor(0x3805,0x9d);
	OV2722MIPI_write_cmos_sensor(0x3806,0x04);  
	OV2722MIPI_write_cmos_sensor(0x3807,0x47);//45
	OV2722MIPI_write_cmos_sensor(0x3808,0x07);  
	OV2722MIPI_write_cmos_sensor(0x3809,0x84);//80
	OV2722MIPI_write_cmos_sensor(0x380a,0x04);  
	OV2722MIPI_write_cmos_sensor(0x380b,0x3C);//38
	...
	OV2722MIPI_write_cmos_sensor(0x3a0e,0x03);//03
	...
	OV2722MIPI_write_cmos_sensor(0x4003,0x01);
	//OV2722MIPI_write_cmos_sensor(0x4003,0x41);
	...
	OV2722MIPI_write_cmos_sensor(0x3503,0x07);//00
	...
	OV2722MIPI_write_cmos_sensor(0x3035,0x10);//00 for 1 lane
	OV2722MIPI_write_cmos_sensor(0x3036,0x1e);
	OV2722MIPI_write_cmos_sensor(0x3037,0x21);//a1 for 1 lane
	...
	OV2722MIPI_write_cmos_sensor(0x5000,0xfd);
	OV2722MIPI_write_cmos_sensor(0x0100,0x01);
	mdelay(50);
	SENSORDB("[OV2722MIPI]exit OV2722MIPI_init function \n");
}
...
{
	kal_uint32 id=0;
	SENSORDB("[OV2722MIPI]enter OV2722MIPIOpen function \n");
	...
	SENSORDB("[ylf]OV2722MIPI Sensor Read ID:0x%04x OK\n",OV2722MIPI_sensor_id);
	OV2722MIPI_init();
	spin_lock(&ov2722mipiraw_drv_lock);
	OV2722MIPI_state=1;
	...
	spin_unlock(&ov2722mipiraw_drv_lock);
	SENSORDB("[OV2722MIPI]exit OV2722MIPIOpen function \n");
	return ERROR_NONE;
}
...
{
	int  retry = 3; 
	SENSORDB("[OV2722MIPI]enter OV2722MIPIGetSensorID function \n");
	...
	SENSORDB("[OV2722MIPI]exit OV2722MIPIGetSensorID function \n");
    return ERROR_NONE;
}
...
{
   SENSORDB("[OV2722MIPI]enter OV2722MIPIClose function \n");
  //CISModulePowerOn(FALSE);
//	DRV_I2CClose(OV2722MIPIhDrvI2C);
	return ERROR_NONE;
}   /* OV2722MIPIClose */
UINT32 OV2722MIPISetTestPatternMode(kal_bool bEnable)
{
    SENSORDB("[OV2722MIPI]enter OV2722MIPISetTestPatternMode function bEnable=%d \n",bEnable);   
    if(bEnable) 
	{   // enable color bar
		ov2722test_pattern_flag=TRUE; 
		OV2722MIPI_write_cmos_sensor(0x5040,0x80);
    } 
	else 
	{   
		ov2722test_pattern_flag=FALSE;
		OV2722MIPI_write_cmos_sensor(0x5040,0x00);
    }
	SENSORDB("[OV2722MIPI]exit OV2722MIPISetTestPatternMode function \n");  
    return ERROR_NONE;
}
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	SENSORDB("[OV2722MIPI]enter OV2722MIPIPreview function \n");
	...
	SENSORDB("[OV2722MIPI]exit OV2722MIPIPreview function \n");
	return ERROR_NONE;	
}   /*  OV2722MIPIPreview   */
...
{
	SENSORDB("[OV2722MIPI]enter OV2722MIPIGetResolution function pSensorResolution=%x\n",*pSensorResolution);
	...
	SENSORDB("[OV2722MIPI]exit OV2722MIPIGetResolution function SensorPreviewWidth=%d,SensorFullWidth=%d \n",pSensorResolution->SensorPreviewWidth,pSensorResolution->SensorFullWidth);
	return ERROR_NONE;
...
{
	SENSORDB("[OV2722MIPI]enter OV2722MIPIGetInfo function FeatureId:%d\n",ScenarioId);
    switch(ScenarioId)
    {
...
#endif	
   SENSORDB("[OV2722MIPI]exit OV2722MIPIGetInfo function \n");	
 	return ERROR_NONE;
}	/* OV2722MIPIGetInfo() */
UINT32 OV2722MIPIControl(MSDK_SCENARIO_ID_ENUM ScenarioId, MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *pImageWindow,
					  MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	SENSORDB("[OV2722MIPI]enter OV2722MIPIControl function FeatureId:%d\n",ScenarioId);
	...
	SENSORDB("[OV2722MIPI]exit OV2722MIPIControl function\n");
	return TRUE;
}	/* OV2722MIPIControl() */
...
UINT32 OV2722MIPISetVideoMode(UINT16 u2FrameRate)
{
	...
	SENSORDB("[OV2722MIPI]enter OV2722MIPISetVideoMode functionu 2FrameRate=%d\n",u2FrameRate);
	if(u2FrameRate==0)
		return TRUE;
	...
	OV2722MIPI_sensor.video_mode = KAL_TRUE;
	spin_unlock(&ov2722mipiraw_drv_lock);
	SENSORDB("[OV2722MIPI]exit OV2722MIPISetVideoMode functionu \n");
    return TRUE;
}

UINT32 OV2722MIPISetAutoFlickerMode(kal_bool bEnable,UINT16 u2FrameRate)
{
	SENSORDB("[OV2722MIPI]enter OV2722MIPISetAutoFlickerMode functionu bEnable=%d,u2FrameRate=%d\n",bEnable,u2FrameRate);
	if(bEnable)
	{
		spin_lock(&ov2722mipiraw_drv_lock);
	...
	SENSORDB("[OV2722MIPI]exit OV2722MIPISetAutoFlickerMode functionu \n");
}
							 ...
							 UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
	...
	SENSORDB("[OV2722MIPI]enter OV2722MIPIFeatureControl functionu FeatureId:%d\n",FeatureId);
	switch (FeatureId)
	{
		...
		case SENSOR_FEATURE_SET_TEST_PATTERN:
             OV2722MIPISetTestPatternMode((BOOL)*pFeatureData16);        	
            break;
		case SENSOR_FEATURE_GET_TEST_PATTERN_CHECKSUM_VALUE:
			*pFeatureReturnPara32 =  OV2722_TEST_PATTERN_CHECKSUM;
			 *pFeatureParaLen=4;
			break;
		default:
			break;
	}
	SENSORDB("[OV2722MIPI]exit OV2722MIPIFeatureControl functionu FeatureId:%d\n",FeatureId);
	return ERROR_NONE;
